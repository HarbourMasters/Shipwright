...
void RegisterImGuiItemIcons() {
    ...
    std::vector<ModdedItem> moddedItems = ModdedItems_GetRegisteredModdedItems();

    for (const auto& moddedItem : moddedItems) {
        std::string moddedItemName = ModdedItems_GetModdedItemName(moddedItem.modId, moddedItem.itemId);
        moddedItemMapping[moddedItem] = { moddedItem, moddedItemName, moddedItemName + "_Faded"};
    }
