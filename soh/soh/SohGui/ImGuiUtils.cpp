...
void RegisterImGuiItemIcons() {
    for (const auto& entry : itemMapping) {
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.second.name, entry.second.texturePath,
                                                                            ImVec4(1, 1, 1, 1));
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(
    ...
    for (const auto& entry : gregMapping) {
        ...
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.second.name, entry.second.texturePath,
                                                                            gregGreen);
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.second.nameFaded,
    ...
    for (const auto& [quest, entry] : songMapping) {
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.name, gSongNoteTex, entry.color);
        ImVec4 fadedCol = entry.color;
        fadedCol.w = 0.3f;
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.nameFaded, gSongNoteTex, fadedCol);
    }
    ...
    for (const auto& entry : gSeedTextures) {
        std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture(entry.tex, entry.tex, ImVec4(1, 1, 1, 1));
    }
}