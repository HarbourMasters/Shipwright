...
static void OTRMessage_LoadCustom(const std::string& folderPath, MessageTableEntry*& table, size_t tableSize) {
    auto lst = *ShipDK::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->ListFiles(folderPath).get();

    for (auto& tPath : lst) {
        auto file = std::static_pointer_cast<SOH::Text>(ShipDK::Context::GetInstance()->GetResourceManager()->LoadResource(tPath));
...
MessageTableEntry* OTRMessage_LoadTable(const std::string& filePath, bool isNES) {
    auto file = std::static_pointer_cast<SOH::Text>(ShipDK::Context::GetInstance()->GetResourceManager()->LoadResource(filePath));
...
extern "C" void OTRMessage_Init()
{
    ...
    if (sStaffMessageEntryTablePtr == NULL) {
        auto file2 =
            std::static_pointer_cast<SOH::Text>(ShipDK::Context::GetInstance()->GetResourceManager()->LoadResource(
