...
void LUS::SetAlternateHeadersFactoryV0::ParseFileBinary(std::shared_ptr<BinaryReader> reader,
    ...
    for (uint32_t i = 0; i < setAlternateHeaders->numHeaders; i++) {
        ...
        if (!headerName.empty()) {
            setAlternateHeaders->headers.push_back(std::static_pointer_cast<LUS::Scene>(
                LUS::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(headerName.c_str())));
        } else {
