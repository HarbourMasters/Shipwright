...
namespace SOH {
std::shared_ptr<ShipDK::IResource> ResourceFactoryBinaryAudioSoundFontV2::ReadResource(std::shared_ptr<ShipDK::File> file) {
    ...
    for (uint32_t i = 0; i < audioSoundFont->soundFont.numDrums; i++) {
        ...
        if (sampleFileName.empty()) {
            ...
            auto res = ShipDK::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(sampleFileName.c_str());
