...
namespace SOH {
std::shared_ptr<ShipDK::IResource> ResourceFactoryBinarySkeletonV0::ReadResource(std::shared_ptr<ShipDK::File> file) {
    ...
    for (size_t i = 0; i < skeleton->limbTable.size(); i++) {
        ...
        auto limb = ShipDK::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(limbStr.c_str());
...
std::shared_ptr<ShipDK::IResource> ResourceFactoryXMLSkeletonV0::ReadResource(std::shared_ptr<ShipDK::File> file) {
    if (!FileHasValidFormatAndReader(file)) {
        return nullptr;
    }
    ...
    while (child != nullptr) {
        ...
        if (childName == "SkeletonLimb") {
            ...
            auto limb = ShipDK::Context::GetInstance()->GetResourceManager()->LoadResourceProcess(limbName.c_str());
