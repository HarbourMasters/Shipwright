...
void SkeletonPatcher::RegisterSkeleton(std::string& path, SkelAnime* skelAnime) {
    ...
    if (path.starts_with(ShipDK::IResource::gAltAssetPrefix)) {
        info.vanillaSkeletonPath = path.substr(ShipDK::IResource::gAltAssetPrefix.length(),
                                               path.size() - ShipDK::IResource::gAltAssetPrefix.length());
    } else {
...
void SkeletonPatcher::UpdateSkeletons() {
    ...
    for (auto skel : skeletons) {
        Skeleton* newSkel =
            (Skeleton*)ShipDK::Context::GetInstance()->GetResourceManager()
                ->LoadResource((isHD ? ShipDK::IResource::gAltAssetPrefix : "") + skel.vanillaSkeletonPath, true)
                .get();
