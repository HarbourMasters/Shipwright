...
extern "C" int CustomMessage_RetrieveIfExists(PlayState* play) {
    ...
    if (IS_RANDO) {
        ...
        else if (textId == TEXT_MEDIGORON && !Flags_GetRandomizerInf(RAND_INF_MERCHANTS_CARPET_SALESMAN) && Randomizer_GetSettingValue(RSK_SHUFFLE_MERCHANTS) != RO_SHUFFLE_MERCHANTS_OFF){
    ...
    } else if (textId == TEXT_HEART_PIECE && CVarGetInteger("gInjectItemCounts", 0)) {
    ...
    } else if (textId == TEXT_MARKET_GUARD_NIGHT && CVarGetInteger("gMarketSneak", 0) && play->sceneNum == SCENE_MARKET_ENTRANCE_NIGHT) {
