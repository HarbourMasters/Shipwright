        ...
        if (ImGui::BeginMenu("Controller")) {
            ...
            if (mGameControlEditorWindow) {
                if (ImGui::Button(GetWindowButtonText("Additional Controller Options",
                                                      CVarGetInteger("gGameControlEditorEnabled", 0))
                                      .c_str(),
                                  ImVec2(-1.0f, 0.0f))) {
            ...
            UIWidgets::Tooltip(
                ...
                "disable game inputs while the menubar is visible.\n\nD-pad to move between items, A to select, and X "
...
#ifndef __APPLE__
            UIWidgets::EnhancementSliderFloat("Internal Resolution: %d %%", "##IMul", "gInternalResolution", 0.5f, 2.0f,
                                              "", 1.0f, true);
...
#endif
                if (LUS::Context::GetInstance()->GetWindow()->GetWindowBackend() == LUS::WindowBackend::DX11) {
                    UIWidgets::Tooltip("Uses Matrix Interpolation to create extra frames, resulting in smoother "
                                       "graphics. This is purely "
    ...
    if (ImGui::BeginMenu("Enhancements")) {
        ...
        if (ImGui::BeginMenu("Gameplay")) {
            if (ImGui::BeginMenu("Time Savers")) {
                UIWidgets::PaddedEnhancementSliderInt("Text Speed: %dx", "##TEXTSPEED", "gTextSpeed", 1, 5, "", 1, true,
                                                      false, true);
                UIWidgets::PaddedEnhancementCheckbox("Skip Text", "gSkipText", false, true);
                UIWidgets::Tooltip("Holding down B skips text");
                UIWidgets::PaddedEnhancementSliderInt("King Zora Speed: %dx", "##MWEEPSPEED", "gMweepSpeed", 1, 5, "",
                                                      1, true, false, true);
                UIWidgets::PaddedEnhancementSliderInt("Biggoron Forge Time: %d days", "##FORGETIME", "gForgeTime", 0, 3,
                                                      "", 3, true, false, true);
                UIWidgets::Tooltip(
                    "Allows you to change the number of days it takes for Biggoron to forge the Biggoron Sword");
                UIWidgets::PaddedEnhancementSliderInt("Vine/Ladder Climb speed +%d", "##CLIMBSPEED", "gClimbSpeed", 0,
                                                      12, "", 0, true, false, true);
                UIWidgets::PaddedEnhancementSliderInt("Block pushing speed +%d", "##BLOCKSPEED", "gFasterBlockPush", 0,
                                                      5, "", 0, true, false, true);
                ...
                if (UIWidgets::EnhancementCombobox("gChestSizeAndTextureMatchesContents",
                                                   chestStyleMatchesContentsOptions, CSMC_DISABLED)) {
                ...
                UIWidgets::Tooltip("Chest sizes and textures are changed to help identify the item inside.\n"
                ...
                if (CVarGetInteger("gChestSizeAndTextureMatchesContents", CSMC_DISABLED) != CSMC_DISABLED) {
                    UIWidgets::PaddedEnhancementCheckbox("Chests of Agony", "gChestSizeDependsStoneOfAgony", true,
                                                         false);
                ...
                UIWidgets::PaddedEnhancementCheckbox("Skip Scarecrow Song", "gSkipScarecrow", true, false,
                                                     forceSkipScarecrow, forceSkipScarecrowText,
                ...
                UIWidgets::PaddedEnhancementCheckbox("Skip Magic Arrow Equip Animation", "gSkipArrowAnimation", true,
                                                     false);
            ...
            if (ImGui::BeginMenu("Items")) {
                ...
                UIWidgets::PaddedEnhancementCheckbox("Prevent Dropped Ocarina Inputs", "gDpadNoDropOcarinaInput", true,
                                                     false);
                ...
                UIWidgets::Tooltip("Wearing the Bunny Hood grants a speed increase like in Majora's Mask. The longer "
                                   "jump option is not accounted for in randomizer logic.\n\n"
                ...
                UIWidgets::Tooltip("After completing the mask trading sub-quest, press A and any direction on the mask "
                                   "slot to change masks");
                UIWidgets::PaddedEnhancementCheckbox("Nuts explode bombs", "gNutsExplodeBombs", true, false);
                UIWidgets::Tooltip("Makes nuts explode bombs, similar to how they interact with bombchus. This does "
                                   "not affect bombflowers.");
                ...
                UIWidgets::PaddedEnhancementCheckbox("Bow as Child/Slingshot as Adult", "gBowSlingShotAmmoFix", true,
                                                     false);
                UIWidgets::Tooltip("Allows child to use bow with arrows.\nAllows adult to use slingshot with "
                ...
                UIWidgets::Tooltip("Helps FW persist between ages, gives child and adult separate FW points, and can "
                                   "be used in more places.");
                UIWidgets::PaddedEnhancementCheckbox("Static Explosion Radius", "gStaticExplosionRadius", true, false);
                UIWidgets::Tooltip("Explosions are now a static size, like in Majora's Mask and OoT3D. Makes bombchu "
                                   "hovering much easier.");
                UIWidgets::PaddedEnhancementCheckbox("Prevent Bombchus Forcing First-Person", "gDisableFirstPersonChus",
                                                     true, false);
                UIWidgets::Tooltip("Prevent bombchus from forcing the camera into first-person mode when released.");
                UIWidgets::PaddedEnhancementCheckbox("Aiming reticle for the bow/slingshot", "gBowReticle", true,
                                                     false);
                UIWidgets::Tooltip("Aiming with a bow or slingshot will display a reticle as with the hookshot when "
                                   "the projectile is ready to fire.");
                ImGui::EndMenu();
            }
            ...
            if (ImGui::BeginMenu("Difficulty Options")) {
                ...
                UIWidgets::Tooltip("Modifies all sources of damage not affected by other sliders\n"
                ...
                UIWidgets::Tooltip("Modifies all fall damage\n"
                ...
                UIWidgets::Tooltip("Modifies damage taken after falling into a void\n"
                ...
                UIWidgets::PaddedEnhancementCheckbox("Enable Bombchu Drops", "gBombchuDrops", true, false,
                                                     forceEnableBombchuDrops, forceEnableBombchuDropsText,
                                                     UIWidgets::CheckboxGraphics::Checkmark);
                ...
                UIWidgets::Tooltip("Disables heart drops, but not heart placements, like from a Deku Scrub running "
                ...
                UIWidgets::PaddedEnhancementCheckbox(
                    "Always Win Dampe Digging Game", "gDampeWin", true, false, SaveManager::Instance->IsRandoFile(),
                    "This setting is always enabled in randomizer files", UIWidgets::CheckboxGraphics::Checkmark);
                UIWidgets::Tooltip("Always win the heart piece/purple rupee on the first dig in Dampe's grave digging "
                                   "game, just like in rando\nIn a rando file, this is unconditionally enabled");
                UIWidgets::Spacer(0);
                ...
                if (ImGui::BeginMenu("Potion Values")) {
                    ...
                    static const char* disabledTooltipRedPotion =
                        "This option is disabled because \"Change Red Potion Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Red Potion Health: %d", "##REDPOTIONHEALTH", "gRedPotionHealth", 1,
                                                    100, "", 0, true, disabledRedPotion, disabledTooltipRedPotion);
                    ...
                    UIWidgets::EnhancementCheckbox("Red Potion Percent Restore", "gRedPercentRestore",
                                                   disabledRedPotion, disabledTooltipRedPotion);
                    UIWidgets::Tooltip("Toggles from Red Potions restoring a fixed amount of health to a percent of "
                    ...
                    static const char* disabledTooltipGreenPotion =
                        "This option is disabled because \"Change Green Potion Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Green Potion Mana: %d", "##GREENPOTIONMANA", "gGreenPotionMana", 1,
                                                    100, "", 0, true, disabledGreenPotion, disabledTooltipGreenPotion);
                    UIWidgets::Tooltip("Changes the amount of mana restored by Green Potions, base max mana is 48, max "
                                       "upgraded mana is 96");
                    ...
                    UIWidgets::Tooltip("Toggles from Green Potions restoring a fixed amount of mana to a percent of "
                                       "the player's current max mana");

                    ImGui::Separator();

                    UIWidgets::EnhancementCheckbox("Change Blue Potion Effects", "gBluePotionEffects");
                    UIWidgets::Tooltip(
                        "Enable the following changes to the amount of health and mana restored by Blue Potions");
                    bool               disabledBluePotion = !CVarGetInteger("gBluePotionEffects", 0);
                    static const char* disabledTooltipBluePotion =
                        "This option is disabled because \"Change Blue Potion Effects\" is turned off";
                    UIWidgets::EnhancementSliderInt("Blue Potion Health: %d", "##BLUEPOTIONHEALTH", "gBluePotionHealth",
                                                    1, 100, "", 0, true, disabledBluePotion, disabledTooltipBluePotion);
                    ...
                    UIWidgets::EnhancementCheckbox("Blue Potion Health Percent Restore", "gBlueHealthPercentRestore",
                                                   disabledBluePotion, disabledTooltipBluePotion);
                    UIWidgets::Tooltip("Toggles from Blue Potions restoring a fixed amount of health to a percent of "
                                       "the player's current max health");

                    ImGui::Separator();

                    UIWidgets::EnhancementSliderInt("Blue Potion Mana: %d", "##BLUEPOTIONMANA", "gBluePotionMana", 1,
                                                    100, "", 0, true, disabledBluePotion, disabledTooltipBluePotion);
                    UIWidgets::Tooltip("Changes the amount of mana restored by Blue Potions, base max mana is 48, max "
                                       "upgraded mana is 96");
                    UIWidgets::EnhancementCheckbox("Blue Potion Mana Percent Restore", "gBlueManaPercentRestore",
                                                   disabledBluePotion, disabledTooltipBluePotion);
                    UIWidgets::Tooltip("Toggles from Blue Potions restoring a fixed amount of mana to a percent of the "
                                       "player's current max mana");
                    ...
                    static const char* disabledTooltipMilk =
                        "This option is disabled because \"Change Milk Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Milk Health: %d", "##MILKHEALTH", "gMilkHealth", 1, 100, "", 0,
                                                    true, disabledMilk, disabledTooltipMilk);
                    UIWidgets::Tooltip("Changes the amount of health restored by Milk");
                    UIWidgets::EnhancementCheckbox("Milk Percent Restore", "gMilkPercentRestore", disabledMilk,
                                                   disabledTooltipMilk);
                    UIWidgets::Tooltip("Toggles from Milk restoring a fixed amount of health to a percent of the "
                                       "player's current max health");

                    ImGui::Separator();

                    UIWidgets::EnhancementCheckbox("Separate Half Milk Effect", "gSeparateHalfMilkEffect", disabledMilk,
                                                   disabledTooltipMilk);
                    UIWidgets::Tooltip("Enable the following changes to the amount of health restored by Half Milk\nIf "
                                       "this is disabled, Half Milk will behave the same as Full Milk.");
                    bool               disabledHalfMilk = disabledMilk || !CVarGetInteger("gSeparateHalfMilkEffect", 0);
                    static const char* disabledTooltipHalfMilk =
                        "This option is disabled because \"Separate Half Milk Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Half Milk Health: %d", "##HALFMILKHEALTH", "gHalfMilkHealth", 1,
                                                    100, "", 0, true, disabledHalfMilk, disabledTooltipHalfMilk);
                    ...
                    UIWidgets::EnhancementCheckbox("Half Milk Percent Restore", "gHalfMilkPercentRestore",
                                                   disabledHalfMilk, disabledTooltipHalfMilk);
                    UIWidgets::Tooltip("Toggles from Half Milk restoring a fixed amount of health to a percent of the "
                                       "player's current max health");
                    ...
                    static const char* disabledTooltipFairy =
                        "This option is disabled because \"Change Fairy Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Fairy: %d", "##FAIRYHEALTH", "gFairyHealth", 1, 100, "", 0, true,
                                                    disabledFairy, disabledTooltipFairy);
                    UIWidgets::Tooltip("Changes the amount of health restored by Fairies");
                    UIWidgets::EnhancementCheckbox("Fairy Percent Restore", "gFairyPercentRestore", disabledFairy,
                                                   disabledTooltipFairy);
                    UIWidgets::Tooltip("Toggles from Fairies restoring a fixed amount of health to a percent of the "
                                       "player's current max health");
                    ...
                    UIWidgets::Tooltip(
                        "Enable the following changes to the amount of health restored by Fairy Revivals");
                    bool               disabledFairyRevive = !CVarGetInteger("gFairyReviveEffect", 0);
                    static const char* disabledTooltipFairyRevive =
                        "This option is disabled because \"Change Fairy Revive Effect\" is turned off";
                    UIWidgets::EnhancementSliderInt("Fairy Revival: %d", "##FAIRYREVIVEHEALTH", "gFairyReviveHealth", 1,
                                                    100, "", 0, true, disabledFairyRevive, disabledTooltipFairyRevive);
                    ...
                    UIWidgets::EnhancementCheckbox("Fairy Revive Percent Restore", "gFairyRevivePercentRestore",
                                                   disabledFairyRevive, disabledTooltipFairyRevive);
                    UIWidgets::Tooltip("Toggles from Fairy Revivals restoring a fixed amount of health to a percent of "
                                       "the player's current max health");
                ...
                if (ImGui::BeginMenu("Shooting Gallery")) {
                    ...
                    UIWidgets::PaddedEnhancementCheckbox("Instant Win", "gInstantShootingGalleryWin", true, false,
                                                         disabled, disabledTooltip);
                    UIWidgets::Tooltip("Skips the shooting gallery minigame");
                    UIWidgets::PaddedEnhancementCheckbox("No Rupee Randomization", "gConstantAdultGallery", true, false,
                                                         disabled, disabledTooltip);
                    UIWidgets::Tooltip(
                        "Forces the rupee order to not be randomized as adult, making it the same as chlid");
                    UIWidgets::PaddedEnhancementSliderInt("Child Starting Ammunition: %d",
                                                          "##cShootingGalleryAmmunition",
                                                          "gChildShootingGalleryAmmunition", 10, 30, "", 15, true, true,
                                                          false, disabled, disabledTooltip);
                    UIWidgets::Tooltip("The ammunition at the start of the shooting gallery minigame as a child");
                    UIWidgets::PaddedEnhancementSliderInt("Adult Starting Ammunition: %d",
                                                          "##aShootingGalleryAmmunition",
                                                          "gAdultShootingGalleryAmmunition", 10, 30, "", 15, true, true,
                                                          false, disabled, disabledTooltip);
                ...
                if (ImGui::BeginMenu("Bombchu Bowling")) {
                    ...
                    static const char* disabledTooltip =
                        "This option is disabled because \"Customize Behavior\" is turned off";
                    UIWidgets::PaddedEnhancementCheckbox("Remove Small Cucco", "gBombchuBowlingNoSmallCucco", true,
                                                         false, disabled, disabledTooltip);
                    UIWidgets::Tooltip("Prevents the small cucco from appearing in the bombchu bowling minigame");
                    UIWidgets::PaddedEnhancementCheckbox("Remove Big Cucco", "gBombchuBowlingNoBigCucco", true, false,
                                                         disabled, disabledTooltip);
                    UIWidgets::Tooltip("Prevents the big cucco from appearing in the bombchu bowling minigame");
                    UIWidgets::PaddedEnhancementSliderInt("Bombchu Count: %d", "##cBombchuBowlingAmmunition",
                                                          "gBombchuBowlingAmmunition", 3, 20, "", 10, true, true, false,
                                                          disabled, disabledTooltip);
                ...
                if (ImGui::BeginMenu("Fishing")) {
                    ...
                    static const char* disabledTooltip =
                        "This option is disabled because \"Customize Behavior\" is turned off";
                    UIWidgets::PaddedEnhancementCheckbox("Instant Fishing", "gInstantFishing", true, false, disabled,
                                                         disabledTooltip);
                    ...
                    UIWidgets::PaddedEnhancementCheckbox("Fish Never Escape", "gFishNeverEscape", true, false, disabled,
                                                         disabledTooltip);
                    UIWidgets::Tooltip("Once a hook has been set, fish will never let go while being reeled in.");
                    UIWidgets::PaddedEnhancementSliderInt("Child Minimum Weight: %d", "##cMinimumWeight",
                                                          "gChildMinimumWeightFish", 3, 10, "", 10, true, true, false,
                                                          disabled, disabledTooltip);
                    UIWidgets::Tooltip("The minimum weight for the unique fishing reward as a child");
                    UIWidgets::PaddedEnhancementSliderInt("Adult Minimum Weight: %d", "##aMinimumWeight",
                                                          "gAdultMinimumWeightFish", 6, 13, "", 13, true, true, false,
                                                          disabled, disabledTooltip);
            ...
            if (ImGui::BeginMenu("Reduced Clutter")) {
                ...
                UIWidgets::PaddedEnhancementCheckbox("Disable Hot/Underwater Warning Text", "gDisableTunicWarningText",
                                                     true, false);
                UIWidgets::Tooltip(
            ...
            UIWidgets::PaddedEnhancementCheckbox("Assignable Tunics and Boots", "gAssignableTunicsAndBoots", true,
                                                 false);
            ...
            UIWidgets::PaddedEnhancementCheckbox("Enable passage of time on file select", "gTimeFlowFileSelect", true,
                                                 false);
            ...
            UIWidgets::PaddedEnhancementCheckbox("Don't require input for Credits sequence", "gNoInputForCredits", true,
                                                 false);
            UIWidgets::Tooltip("Removes the input requirement on textboxes after defeating Ganon, allowing Credits "
                               "sequence to continue to progress");

            // Blue Fire Arrows
            bool forceEnableBlueFireArrows =
                gSaveContext.n64ddFlag && OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_BLUE_FIRE_ARROWS);
            ...
            UIWidgets::PaddedEnhancementCheckbox("Blue Fire Arrows", "gBlueFireArrows", true, false,
                                                 forceEnableBlueFireArrows, forceEnableBlueFireArrowsText,
                                                 UIWidgets::CheckboxGraphics::Checkmark);
            UIWidgets::Tooltip(
                "Allows Ice Arrows to melt red ice.\nMay require a room reload if toggled during gameplay.");

            // Sunlight Arrows
            bool forceEnableSunLightArrows =
                gSaveContext.n64ddFlag && OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_SUNLIGHT_ARROWS);
            ...
            UIWidgets::PaddedEnhancementCheckbox("Sunlight Arrows", "gSunlightArrows", true, false,
                                                 forceEnableSunLightArrows, forceEnableSunLightArrowsText,
                                                 UIWidgets::CheckboxGraphics::Checkmark);
            UIWidgets::Tooltip(
        ...
        if (ImGui::BeginMenu("Graphics")) {
            ...
            UIWidgets::Tooltip("Displays a tick in the top center of the screen to help with glitch line-ups in SoH, "
                               "as traditional UI based line-ups do not work outside of 4:3");
            ...
            UIWidgets::PaddedEnhancementCheckbox("Always show dungeon entrances", "gAlwaysShowDungeonMinimapIcon", true,
                                                 false);
            ...
            if (ImGui::BeginMenu("Animated Link in Pause Menu")) {
                ...
                UIWidgets::Tooltip("Allow you to rotate Link on the Equipment menu with the D-pad\nUse D-pad Up or "
                                   "D-pad Down to reset Link's rotation");
                ...
                UIWidgets::Tooltip("Allow you to rotate Link on the Equipment menu with the C-buttons\nUse C-Up or "
                                   "C-Down to reset Link's rotation");
                ...
                UIWidgets::Tooltip("Allow you to rotate Link on the Equipment menu with the Right Stick\nYou can zoom "
                                   "in by pointing up and reset Link's rotation by pointing down");
                if (CVarGetInteger("gPauseLiveLinkRotation", 0) != 0) {
                    UIWidgets::EnhancementSliderInt("Rotation Speed: %d", "##MinRotationSpeed",
                                                    "gPauseLiveLinkRotationSpeed", 1, 20, "", 1);
                }
                UIWidgets::PaddedSeparator();
                ...
                UIWidgets::Tooltip(
                    "Randomize the animation played on the menu after a certain time (Idle animations only)");
                if (CVarGetInteger("gPauseLiveLink", 0) >= 16) {
                    UIWidgets::EnhancementSliderInt("Frame to wait: %d", "##MinFrameCount", "gMinFrameCount", 1, 1000,
                                                    "", 0);
                }
            ...
            if (UIWidgets::EnhancementCombobox("gSceneSpecificDirtPathFix", zFightingOptions, ZFIGHT_FIX_DISABLED) &&
                gPlayState != NULL) {
            ...
            UIWidgets::PaddedEnhancementSliderInt("Text Spacing: %d", "##TEXTSPACING", "gTextSpacing", 4, 6, "", 6,
                                                  true, true, true);
        ...
        if (ImGui::BeginMenu("Fixes")) {
            ...
            UIWidgets::PaddedEnhancementCheckbox("Fix L&Z Page switch in Pause menu", "gNGCKaleidoSwitcher", true,
                                                 false);
            ...
            UIWidgets::Tooltip("Removes the dungeon entrance icon on the top-left corner of the screen when no dungeon "
                               "is present on the current map");
            UIWidgets::PaddedEnhancementCheckbox("Fix Two Handed idle animations", "gTwoHandedIdle", true, false);
            UIWidgets::Tooltip("Re-enables the two-handed idle animation, a seemingly finished animation that was "
                               "disabled on accident in the original game");
            UIWidgets::PaddedEnhancementCheckbox("Fix the Gravedigging Tour Glitch", "gGravediggingTourFix", true,
                                                 false);
            ...
            UIWidgets::Tooltip("Prevents the Forest Stage Deku Nut upgrade from becoming unobtainable after receiving "
                               "the Poacher's Saw");
            ...
            if (UIWidgets::PaddedEnhancementCheckbox("Fix Megaton Hammer crouch stab", "gCrouchStabHammerFix", true,
                                                     false)) {
            ...
            UIWidgets::PaddedEnhancementCheckbox("Fix Gerudo Warrior's clothing colors", "gGerudoWarriorClothingFix",
                                                 true, false);
            UIWidgets::Tooltip("Prevent the Gerudo Warrior's clothes changing color when changing Link's tunic or "
            ...
            UIWidgets::Tooltip("Fixes camera getting stuck on collision when standing still, also fixes slight shift "
                               "back in camera when stop moving");
            UIWidgets::PaddedEnhancementCheckbox("Fix Hanging Ledge Swing Rate", "gFixHangingLedgeSwingRate", true,
                                                 false);
            UIWidgets::Tooltip("Fixes camera swing rate when player falls off a ledge and camera swings around");
            UIWidgets::PaddedEnhancementCheckbox("Fix Missing Jingle after 5 Silver Rupees", "gSilverRupeeJingleExtend",
                                                 true, false);
            UIWidgets::Tooltip(
        ...
        if (ImGui::BeginMenu("Restoration")) {
            ...
            UIWidgets::Tooltip("Hide the game version and build details and display the authentic model and texture on "
                               "the boot logo start screen");
            ...
            UIWidgets::Tooltip("Restore a bug from NTSC 1.0 that allows putting away an item without an animation and "
                               "performing Putaway Ocarina Items");
        ...
        if (ImGui::BeginMenu("Extra Modes")) {
            ...
            UIWidgets::Tooltip("Will attempt to connect to the Crowd Control server. Check out crowdcontrol.live for "
                               "more information.");
#endif

            UIWidgets::PaddedText("Mirrored World", true, false);
            if (UIWidgets::EnhancementCombobox("gMirroredWorldMode", mirroredWorldModes, MIRRORED_WORLD_OFF) &&
                gPlayState != NULL) {
            ...
            UIWidgets::Tooltip("Replaces fixed enemies throughout the game with a random enemy. Bosses, mini-bosses "
                               "and a few specific regular enemies are excluded.\n"
                               "Enemies that need more than Deku Nuts + either Deku Sticks or a sword to kill are "
                               "excluded from spawning in \"clear enemy\" rooms.\n\n"
        ...
        // Autosave enum value of 1 is the default in presets and the old checkbox "on" state for backwards
        // compatibility
        ...
        UIWidgets::Tooltip("Automatically save the game when changing locations and/or obtaining items\n"
                           "Major items exclude rupees and health/magic/ammo refills (but include bombchus unless "
                           "bombchu drops are enabled)");
        ...
        if (mCosmeticsEditorWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Cosmetics Editor", CVarGetInteger("gCosmeticsEditorEnabled", 0)).c_str(),
                    ImVec2(-1.0f, 0.0f))) {
        ...
        if (mAudioEditorWindow) {
            if (ImGui::Button(GetWindowButtonText("Audio Editor", CVarGetInteger("gAudioEditor.WindowOpen", 0)).c_str(),
                              ImVec2(-1.0f, 0.0f))) {
        ...
        if (mGameplayStatsWindow) {
            if (ImGui::Button(GetWindowButtonText("Gameplay Stats", CVarGetInteger("gGameplayStatsEnabled", 0)).c_str(),
                              ImVec2(-1.0f, 0.0f))) {
        ...
        if (UIWidgets::EnhancementCombobox("gSwitchPerfMode", SWITCH_CPU_PROFILES, (int)LUS::SwitchProfiles::STOCK)) {
            SPDLOG_INFO("Profile:: %s",
                        SWITCH_CPU_PROFILES[CVarGetInteger("gSwitchPerfMode", (int)LUS::SwitchProfiles::STOCK)]);
...
void DrawCheatsMenu() {
    if (ImGui::BeginMenu("Cheats")) {
        ...
        UIWidgets::EnhancementSliderFloat("Hookshot Reach Multiplier: %.1fx", "##gCheatHookshotReachMultiplier",
                                          "gCheatHookshotReachMultiplier", 1.0f, 5.0f, "", 1.0f, false);
        ...
        UIWidgets::PaddedEnhancementCheckbox("Easy Input Buffering", "gCheatEasyInputBufferingEnabled", true, false,
                                             bEasyFrameAdvanceEnabled,
                                             "Forced enabled when Easy Frame Advancing is enabled");
        UIWidgets::Tooltip(
            "Inputs that are held down while the Subscreen is closing will be pressed when the game is resumed");
...

        {
            ...
            if (betaQuestEnabled) {
                ...
                UIWidgets::Tooltip("Set the Beta Quest world to explore. *WARNING* Changing this will reset your "
            ...
            if (betaQuestEnabled != lastBetaQuestEnabled || betaQuestWorld != lastBetaQuestWorld) {
                // Reset the game if the beta quest state or world changed because beta quest happens on redirecting the
...
void DrawDeveloperToolsMenu() {
    if (ImGui::BeginMenu("Developer Tools")) {
        ...
        if (mColViewerWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Collision Viewer", CVarGetInteger("gCollisionViewerEnabled", 0)).c_str(),
                    ImVec2(-1.0f, 0.0f))) {
        ...
        UIWidgets::Spacer(0);
        if (mDLViewerWindow) {
            if (ImGui::Button(GetWindowButtonText("Display List Viewer", CVarGetInteger("gDLViewerEnabled", 0)).c_str(),
                              ImVec2(-1.0f, 0.0f))) {
...
void DrawRandomizerMenu() {
    if (ImGui::BeginMenu("Randomizer")) {
        ...
        if (mRandomizerSettingsWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Randomizer Settings", CVarGetInteger("gRandomizerSettingsEnabled", 0)).c_str(),
                    buttonSize)) {
        ...
        if (mItemTrackerWindow) {
            if (ImGui::Button(GetWindowButtonText("Item Tracker", CVarGetInteger("gItemTrackerEnabled", 0)).c_str(),
                              buttonSize)) {
        ...
        if (mItemTrackerSettingsWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Item Tracker Settings", CVarGetInteger("gItemTrackerSettingsEnabled", 0))
                        .c_str(),
                    buttonSize)) {
        ...
        if (mEntranceTrackerWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Entrance Tracker", CVarGetInteger("gEntranceTrackerEnabled", 0)).c_str(),
                    buttonSize)) {
        ...
        if (mCheckTrackerWindow) {
            if (ImGui::Button(GetWindowButtonText("Check Tracker", CVarGetInteger("gCheckTrackerEnabled", 0)).c_str(),
                              buttonSize)) {
        ...
        if (mCheckTrackerSettingsWindow) {
            if (ImGui::Button(
                    GetWindowButtonText("Check Tracker Settings", CVarGetInteger("gCheckTrackerSettingsEnabled", 0))
                        .c_str(),
                    buttonSize)) {
        ...
        if (ImGui::BeginMenu("Rando Enhancements")) {
            UIWidgets::EnhancementCheckbox("Rando-Relevant Navi Hints", "gRandoRelevantNavi", false, "",
                                           UIWidgets::CheckboxGraphics::Cross, true);
            UIWidgets::Tooltip("Replace Navi's overworld quest hints with rando-related gameplay hints.");
            UIWidgets::PaddedEnhancementCheckbox("Random Rupee Names", "gRandomizeRupeeNames", true, false, false, "",
                                                 UIWidgets::CheckboxGraphics::Cross, true);
            UIWidgets::Tooltip("When obtaining rupees, randomize what the rupee is called in the textbox.");

            // Only disable the key colors checkbox when none of the keysanity settings are set to "Any Dungeon",
            // "Overworld" or "Anywhere"
            bool disableKeyColors = true;

            if (OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_KEYSANITY) ==
                ...
                OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_BOSS_KEYSANITY) ==
                    RO_DUNGEON_ITEM_LOC_ANY_DUNGEON ||
                OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_BOSS_KEYSANITY) ==
                    RO_DUNGEON_ITEM_LOC_OVERWORLD ||
                OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_BOSS_KEYSANITY) ==
                    RO_DUNGEON_ITEM_LOC_ANYWHERE ||
                (OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_GANONS_BOSS_KEY) !=
            ...
            UIWidgets::Tooltip("Play unique fanfares when obtaining quest items "
