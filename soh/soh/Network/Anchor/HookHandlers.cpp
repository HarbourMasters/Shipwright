...
void Anchor::RegisterHooks() {
    ...
    COND_HOOK(OnItemReceive, isConnected, [&](GetItemEntry itemEntry) {
        ...
        // Handle custom items (range 0x9C-0xB5)
        // For custom items, send the itemId directly instead of getItemId
        if (itemEntry.modIndex == MOD_NONE && itemEntry.itemId >= 0x9C && itemEntry.itemId <= 0xB5) {
            SendPacket_GiveItem(MOD_NONE, itemEntry.itemId);
        } else {
