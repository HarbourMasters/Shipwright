...
void RegionTable_Init() {
    ...
    areaTable[RR_ROOT] = Region("Root", SCENE_ID_MAX, TIME_DOESNT_PASS, {RA_LINKS_POCKET}, {
        ...
        LOCATION(RC_TRIFORCE_COMPLETED, GetShipSaveContextData(logic->GetSaveContext())->quest.data.randomizer.triforcePiecesCollected >= ctx->GetOption(RSK_TRIFORCE_HUNT_PIECES_REQUIRED).Get() + 1;),
