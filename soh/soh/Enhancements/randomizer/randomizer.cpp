...
static const char* englishRupeeNames[170] = { "[P]",
                                              "Bad RNG Rolls",
                                              "Bananas",
                                              "Beanbean Coins",
                                              "Beans",
...
static const char* frenchRupeeNames[36] = {
    "Rubis",   "Bitcoin", "Bananes", "Euros",      "Dollars",  "Émeraudes", "Joyaux",      "Diamants",    "Balles",
    "Pokémon", "Pièces",  "Lunes",   "Étoiles",    "Dogecoin", "Anneaux",   "Radis",       "Pokédollars", "Zennies",
    "Pépètes", "Mailles", "Éthers",  "Clochettes", "Capsules", "Gils",      "Champignons", "Blés",        "Halos",
...
Randomizer::Randomizer() {
    for (auto& [randomizerCheck, rcObject] : RandomizerCheckObjects::GetAllRCObjects()) {
        ...
        checkFromActorMultimap.emplace(
            std::make_tuple((s16)rcObject.actorId, (s16)rcObject.sceneId, rcObject.actorParams), rcObject);
    }
...
std::unordered_map<std::string, SceneID> spoilerFileDungeonToScene = { { "Deku Tree", SCENE_YDAN },
...
std::unordered_map<s16, s16> getItemIdToItemId = { { GI_BOW, ITEM_BOW },
...
void Randomizer::LoadHintLocations(const char* spoilerFileName) {
    ...
    for (const auto& hintLocation : gSaveContext.hintLocations) {
        if (hintLocation.check == RC_LINKS_POCKET)
            break;
        this->hintLocations[hintLocation.check] = hintLocation.hintText;
        CustomMessageManager::Instance->CreateMessage(
            Randomizer::hintMessageTableID, hintLocation.check,
            CustomMessage(hintLocation.hintText, hintLocation.hintText, hintLocation.hintText));
    }
    ...
    CustomMessageManager::Instance->CreateMessage(
        ...
        CustomMessage("Yeaaarrgh! I'm cursed!!^Please save me by destroying&%r{{params}} Spiders of the Curse%w&and I "
                      "will give you my&%b{{check}}%w!",
                      "Yeaaarrgh! Ich bin verflucht!^Bitte rette mich, indem du %r{{params}} Skulltulas&%wzerstörst "
                      "und ich werde dir dafür&%b{{check}} %wgeben!",
                      "Yeaaarrgh! Je suis maudit!^Détruit encore %r{{params}} Araignées de&la Malédiction%w et j'aurai "
                      "quelque&chose à te donner!&%b({{check}})"));
    CustomMessageManager::Instance->CreateMessage(
    ...
    CustomMessageManager::Instance->CreateMessage(
        ...
        CustomMessage("Warp to&{{location}}?\x1B&%gOK&No%w\x02", "Zu {{location}}?\x1B&%gOK&No%w\x02",
...
void Randomizer::LoadMerchantMessages(const char* spoilerFileName) {
    ...
    CustomMessageManager::Instance->CreateMessage(
        ...
        CustomMessage(
            "\x12\x38\x82"
            "All right! You win! In return for&sparing me, I will give you a&%g{{item}}%w!&Please, take "
            "it!\x07\x10\xA3",
            "\x12\x38\x82"
            "In Ordnung! Du gewinnst! Im Austausch&dafür, dass du mich verschont hast,&werde ich dir einen "
            "&%g{{item}}%w geben!\x07\x10\xA3",
            "\x12\x38\x82"
            "J'me rends! Laisse-moi partir et en&échange, je te donne un &%g{{item}}%w! Vas-y prends le!\x07\x10\xA3"));
    CustomMessageManager::Instance->CreateMessage(
        Randomizer::merchantMessageTableID, TEXT_SCRUB_RANDOM,
        CustomMessage("\x12\x38\x82"
                      "All right! You win! In return for&sparing me, I will sell you a&%g{{item}}%w!&%r{{price}} "
                      "Rupees%w it is!\x07\x10\xA3",
                      "\x12\x38\x82"
                      "Aufgeben! Ich verkaufe dir einen&%g{{item}}%w&für %r{{price}} Rubine%w!\x07\x10\xA3",
                      "\x12\x38\x82"
                      "J'abandonne! Tu veux bien m'acheter&un %g{{item}}%w?&Ça fera %r{{price}} Rubis%w!\x07\x10\xA3"));
    CustomMessageManager::Instance->CreateMessage(
    ...
    if (Randomizer::GetRandoSettingValue(RSK_SHUFFLE_MERCHANTS) == RO_SHUFFLE_MERCHANTS_ON_HINT) {
        cgBoxTwoText = { "!%w&It's real, I promise!&A lonely man such as myself&wouldn't %rlie%w to you, hmm?^",
                         "!%w&Ich kann versichern es ist ein&aufrichtiges Angebot!^Ein einsamer Mann wie ich würde "
                         "dich&doch nicht %ranlügen%w, oder?^",
                         "!%w&C'est vrai! J'te jure!&Un gars comme moi ne te %rmentirai%w pas&tu ne crois pas?^" };
    } else {
        cgBoxTwoText = { "!%w&I won't tell you what it is until I see&the money...^",
...
void Randomizer::ParseHintLocationsFile(const char* spoilerFileName) {
    ...
    try {
        ...
        gSaveContext.rewardCheck[0] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["childAltar"]["rewards"]["emeraldLoc"]];
        gSaveContext.rewardCheck[1] = SpoilerfileCheckNameToEnum[spoilerFileJson["childAltar"]["rewards"]["rubyLoc"]];
        gSaveContext.rewardCheck[2] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["childAltar"]["rewards"]["sapphireLoc"]];
        ...
        gSaveContext.rewardCheck[3] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["forestMedallionLoc"]];
        gSaveContext.rewardCheck[4] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["fireMedallionLoc"]];
        gSaveContext.rewardCheck[5] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["waterMedallionLoc"]];
        gSaveContext.rewardCheck[6] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["shadowMedallionLoc"]];
        gSaveContext.rewardCheck[7] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["spiritMedallionLoc"]];
        gSaveContext.rewardCheck[8] =
            SpoilerfileCheckNameToEnum[spoilerFileJson["adultAltar"]["rewards"]["lightMedallionLoc"]];
...
ItemObtainability Randomizer::GetItemObtainabilityFromRandomizerGet(RandomizerGet randoGet) {
    ...
    switch (randoGet) {
        ...
        case RG_FOREST_TEMPLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_BMORI1] < FOREST_TEMPLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_FIRE_TEMPLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_HIDAN] < FIRE_TEMPLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_WATER_TEMPLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_MIZUSIN] < WATER_TEMPLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_SPIRIT_TEMPLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_JYASINZOU] < SPIRIT_TEMPLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_SHADOW_TEMPLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_HAKADAN] < SHADOW_TEMPLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        ...
        case RG_GERUDO_TRAINING_GROUNDS_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_MEN] < GERUDO_TRAINING_GROUNDS_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_GERUDO_FORTRESS_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_GERUDOWAY] < GERUDO_FORTRESS_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_GANONS_CASTLE_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_GANONTIKA] < GANONS_CASTLE_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
        case RG_TREASURE_GAME_SMALL_KEY:
            return gSaveContext.inventory.dungeonKeys[SCENE_TAKARAYA] < TREASURE_GAME_SMALL_KEY_MAX
                       ? CAN_OBTAIN
                       : CANT_OBTAIN_ALREADY_HAVE;
...
RandomizerCheckObject Randomizer::GetCheckObjectFromActor(s16 actorId, s16 sceneNum, s32 actorParams = 0x00) {
        ...
        case SCENE_TAKARAYA:
            ...
            if (GetRandoSettingValue(RSK_SHUFFLE_CHEST_MINIGAME)) {
                if ((actorParams & 0xF) < 2)
                    specialRc = RC_MARKET_TREASURE_CHEST_GAME_ITEM_1;
                if ((actorParams & 0xF) < 4)
                    specialRc = RC_MARKET_TREASURE_CHEST_GAME_ITEM_2;
                if ((actorParams & 0xF) < 6)
                    specialRc = RC_MARKET_TREASURE_CHEST_GAME_ITEM_3;
                if ((actorParams & 0xF) < 8)
                    specialRc = RC_MARKET_TREASURE_CHEST_GAME_ITEM_4;
                if ((actorParams & 0xF) < 10)
                    specialRc = RC_MARKET_TREASURE_CHEST_GAME_ITEM_5;
            }
            break;
        case SCENE_SHRINE:
        case SCENE_SHRINE_N:
    ...
    for (auto it = range.first; it != range.second; ++it) {
        if (it->second.vOrMQ == RCVORMQ_BOTH ||
...
ScrubIdentity Randomizer::IdentifyScrub(s32 sceneNum, s32 actorParams, s32 respawnData) {
    ...
    if (rcObject.rc != RC_UNKNOWN_CHECK) {
        ...
        if (rcObject.rc == RC_HF_DEKU_SCRUB_GROTTO || rcObject.rc == RC_LW_DEKU_SCRUB_GROTTO_FRONT ||
            rcObject.rc == RC_LW_DEKU_SCRUB_NEAR_BRIDGE) {
...
ShopItemIdentity Randomizer::IdentifyShopItem(s32 sceneNum, u8 slotIndex) {
    ...
    RandomizerCheckObject rcObject = GetCheckObjectFromActor(
        ACTOR_EN_GIRLA,
...
GetItemEntry Randomizer::GetItemEntryFromRGData(RandomizerGetData rgData, GetItemID ogItemId, bool checkObtainability) {
    ...
    if (checkObtainability &&
        OTRGlobals::Instance->gRandomizer->GetItemObtainabilityFromRandomizerGet(rgData.rgID) != CAN_OBTAIN) {
    ...
    if (IsItemVanilla(rgData.rgID)) {
        return ItemTableManager::Instance->RetrieveItemEntry(MOD_NONE,
                                                             GetItemIdFromRandomizerGet(rgData.rgID, ogItemId));
    }
    ...
    if (rgData.rgID == RG_ICE_TRAP) {
        ...
        GetItemEntry fakeGiEntry = ItemTableManager::Instance->RetrieveItemEntry(
            modIndex, GetItemIdFromRandomizerGet(rgData.fakeRgID, ogItemId));
...
void GenerateRandomizerImgui(std::string seed = "") {
    ...
    cvarSettings[RSK_SHUFFLE_OCARINA] =
        CVarGetInteger("gRandomizeShuffleOcarinas", 0) || CVarGetInteger("gRandomizeStartingOcarina", 0);
    cvarSettings[RSK_STARTING_KOKIRI_SWORD] = CVarGetInteger("gRandomizeStartingKokiriSword", 0);
    cvarSettings[RSK_SHUFFLE_KOKIRI_SWORD] =
        CVarGetInteger("gRandomizeShuffleKokiriSword", 0) || CVarGetInteger("gRandomizeStartingKokiriSword", 0);
    ...
    cvarSettings[RSK_STARTING_MAPS_COMPASSES] =
        CVarGetInteger("gRandomizeStartingMapsCompasses", RO_DUNGEON_ITEM_LOC_OWN_DUNGEON);
    cvarSettings[RSK_SHUFFLE_DUNGEON_REWARDS] =
    ...
    cvarSettings[RSK_SHOPSANITY_PRICES_AFFORDABLE] =
        CVarGetInteger("gRandomizeShopsanityPricesAffordable", RO_SHOPSANITY_OFF);
    ...
    cvarSettings[RSK_SHUFFLE_WEIRD_EGG] =
        ((CVarGetInteger("gRandomizeSkipChildZelda", 0) == 0) && CVarGetInteger("gRandomizeShuffleWeirdEgg", 0));
    ...
    cvarSettings[RSK_KEYRINGS_RANDOM_COUNT] =
        std::min(CVarGetInteger("gRandomizeShuffleKeyRingsRandomCount", maxKeyringCount), maxKeyringCount);
    ...
    cvarSettings[RSK_SHUFFLE_ENTRANCES] =
    ...
    cvarSettings[RSK_SHUFFLE_DUNGEON_ENTRANCES] =
        CVarGetInteger("gRandomizeShuffleDungeonsEntrances", RO_DUNGEON_ENTRANCE_SHUFFLE_OFF);
    cvarSettings[RSK_SHUFFLE_BOSS_ENTRANCES] =
        CVarGetInteger("gRandomizeShuffleBossEntrances", RO_BOSS_ROOM_ENTRANCE_SHUFFLE_OFF);
    cvarSettings[RSK_SHUFFLE_OVERWORLD_ENTRANCES] =
        CVarGetInteger("gRandomizeShuffleOverworldEntrances", RO_GENERIC_OFF);
    cvarSettings[RSK_SHUFFLE_INTERIOR_ENTRANCES] =
        CVarGetInteger("gRandomizeShuffleInteriorsEntrances", RO_INTERIOR_ENTRANCE_SHUFFLE_OFF);
...
void RandomizerSettingsWindow::DrawElement() {
    ...
    static const char* randoShopsanityPrices[5] = { "Balanced", "Starter Wallet", "Adult Wallet", "Giant's Wallet",
                                                    "Tycoon's Wallet" };
    ...
    bool disableEditingRandoSettings =
        CVarGetInteger("gRandoGenerating", 0) || CVarGetInteger("gOnFileSelectNameEntry", 0);
    ...
    if (ImGui::BeginTabBar("Randomizer Settings", ImGuiTabBarFlags_NoCloseWithMiddleMouseButton)) {
        if (ImGui::BeginTabItem("World")) {
            ...
            if (ImGui::BeginTable("tableRandoWorld", 3, ImGuiTableFlags_BordersH | ImGuiTableFlags_BordersV)) {
                ...
                UIWidgets::InsertHelpHoverText("Closed - The Ocarina of Time, the Song of Time and all "
                ...
                static const char* disableRandoStartingAgeText =
                ...
                UIWidgets::EnhancementCombobox("gRandomizeStartingAge", randoStartingAge, RO_AGE_CHILD,
                                               disableRandoStartingAge, disableRandoStartingAgeText, RO_AGE_CHILD);
                        ...
                        switch (CVarGetInteger("gRandomizeBridgeRewardOptions", RO_BRIDGE_STANDARD_REWARD)) {
                            case RO_BRIDGE_STANDARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Stone Count: %d", "##RandoStoneCount",
                                                                      "gRandomizeStoneCount", 1, 3, "", 3, true, true,
                                                                      false);
                                break;
                            case RO_BRIDGE_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Stone Count: %d", "##RandoStoneCount",
                                                                      "gRandomizeStoneCount", 1, 4, "", 4, true, true,
                                                                      false);
                                break;
                        ...
                        switch (CVarGetInteger("gRandomizeBridgeRewardOptions", RO_BRIDGE_STANDARD_REWARD)) {
                            case RO_BRIDGE_STANDARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Reward Count: %d", "##RandoRewardCount",
                                                                      "gRandomizeRewardCount", 1, 9, "", 9, true, true,
                                                                      false);
                                break;
                            case RO_BRIDGE_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Reward Count: %d", "##RandoRewardCount",
                        ...
                        switch (CVarGetInteger("gRandomizeBridgeRewardOptions", RO_BRIDGE_STANDARD_REWARD)) {
                            case RO_BRIDGE_STANDARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Dungeon Count: %d", "##RandoDungeonCount",
                                                                      "gRandomizeDungeonCount", 1, 8, "", 8, true, true,
                                                                      false);
                                break;
                            case RO_BRIDGE_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Dungeon Count: %d", "##RandoDungeonCount",
                                                                      "gRandomizeDungeonCount", 1, 9, "", 9, true, true,
                                                                      false);
                                break;
                            case RO_BRIDGE_WILDCARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Dungeon Count: %d", "##RandoDungeonCount",
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "- Bottom of the Well will be open for adult after playing Song of Storms to the Windmill guy as "
                    "child.\n"
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "\n"
                    "Full - Shuffle the entrances of all boss rooms together. Child may be expected to defeat Phantom "
                    "Ganon and/or Bongo Bongo.");
                ...
                UIWidgets::InsertHelpHoverText("Shuffle the pool of Overworld entrances, which corresponds to almost "
                ...
                UIWidgets::InsertHelpHoverText("Randomize where Kaepora Gaebora (the Owl) drops you at when you talk "
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "be able use the spawn positions, especially if they are the only logical way to get to certain "
                    "areas.");
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "vice versa, while overworld entrances are shuffled in their own separate pool and indoors stay "
                    "vanilla.");
        ...
        if (ImGui::BeginTabItem("Items")) {
            ...
            if (ImGui::BeginTable("tableRandoStartingInventory", 3,
                ...
                UIWidgets::InsertHelpHoverText("Shuffles Golden Skulltula Tokens into the item pool. This means "
                ...
                UIWidgets::PaddedEnhancementCheckbox("Nighttime GS expect Sun's Song", "gRandomizeGsExpectSunsSong",
                                                     true, false);
                UIWidgets::InsertHelpHoverText("All Golden Skulltulas that require nighttime to appear will only be "
                ...
                static const char* disableShuffleKokiriSwordText =
                    "This option is disabled because \"Start with Kokiri Sword\" is enabled.";
                UIWidgets::EnhancementCheckbox(Settings::ShuffleKokiriSword.GetName().c_str(),
                                               "gRandomizeShuffleKokiriSword", disableShuffleKokiriSword,
                                               disableShuffleKokiriSwordText);
                UIWidgets::InsertHelpHoverText("Shuffles the Kokiri Sword into the item pool.\n"
                                               "\n"
                ...
                static const char* disableShuffleOcarinasText =
                    "This option is disabled because \"Start with Fairy Ocarina\" is enabled.";
                ...
                static const char* disableShuffleWeirdEggText =
                ...
                UIWidgets::InsertHelpHoverText("Shuffles the Weird Egg from Malon in to the item pool. Enabling "
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "\n"
                    "1-4 Items - Vanilla shop items will be shuffled among different shops, and each shop will contain "
                    "1-4 non-vanilla shop items.\n"
                    "\n"
                    "Random - Vanilla shop items will be shuffled among different shops, and each shop will contain a "
                    "random number(1-4) of non-vanilla shop items.\n");
                ...
                switch (CVarGetInteger("gRandomizeShopsanity", RO_SHOPSANITY_OFF)) {
                    ...
                    default:
                        ...
                        UIWidgets::InsertHelpHoverText(
                            "Balanced - The default randomization. Shop prices for shopsanity items will range between "
                            "0 to 300 rupees, "
                        ...
                        UIWidgets::EnhancementCombobox("gRandomizeShopsanityPrices", randoShopsanityPrices,
                                                       RO_SHOPSANITY_PRICE_BALANCED);
                        UIWidgets::EnhancementCheckbox(
                            Settings::ShopsanityPricesAffordable.GetName().c_str(),
                            "gRandomizeShopsanityPricesAffordable",
                            CVarGetInteger("gRandomizeShopsanityPrices", RO_SHOPSANITY_PRICE_BALANCED) ==
                                RO_SHOPSANITY_PRICE_BALANCED,
                            "This can only apply to a wallet range.");
                        UIWidgets::InsertHelpHoverText(
                            "Random selection between the selected wallet tier's affordable price and the affordable "
                            "prices of the preceding wallet tiers.\n\n"
                            "Affordable prices per tier: starter = 10, adult = 105, giant = 205, tycoon = 505\n\n"
                            "Use this to enable wallet tier locking, but make shop items not as expensive as they "
                            "could be.");
                }
                ...
                UIWidgets::EnhancementCheckbox(Settings::ShuffleFrogSongRupees.GetName().c_str(),
                ...
                static const char* disable100GSRewardText =
                    "This option is forcefully enabled because \"Ganon's Boss Key\" is set to \"100 GS Reward.\"";
                UIWidgets::EnhancementCheckbox(Settings::Shuffle100GSReward.GetName().c_str(),
                                               "gRandomizeShuffle100GSReward", forceEnable100GSShuffle,
                                               disable100GSRewardText, UIWidgets::CheckboxGraphics::Checkmark);
                UIWidgets::InsertHelpHoverText(
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "You start with 3 keys in Spirit Temple MQ because the vanilla key layout is not beatable in "
                    "logic.\n"
                    "\n"
                ...
                UIWidgets::InsertHelpHoverText(
                    "Keyrings will replace all small keys from a particular dungeon with a single keyring that awards "
                    "all keys for it's associated dungeon\n"
                    "\n"
                    ...
                    "\n"
                    "Count - A specified amount of randomly selected dungeons will have their keys replaced with "
                    "keyrings.\n"
                    "\n"
                    "Selection - Hand select which dungeons will have their keys replaced with keyrings.\n"
                    "\n"
                    "Selecting key ring for dungeons will have no effect if Small Keys are set to Start With or "
                    "Vanilla.\n"
                    "\n"
                ...
                switch (CVarGetInteger("gRandomizeShuffleKeyRings", RO_KEYRINGS_OFF)) {
                    ...
                    case RO_KEYRINGS_SELECTION:
                        ...
                        UIWidgets::EnhancementCheckbox(
                            ...
                            "Gerudo Fortress Keys setting is incompatible with \nhaving a Gerudo Fortress keyring.");
                        UIWidgets::EnhancementCheckbox("Forest Temple##RandomizeShuffleKeyRings",
                                                       "gRandomizeShuffleKeyRingsForestTemple");
                        UIWidgets::EnhancementCheckbox("Fire Temple##RandomizeShuffleKeyRings",
                                                       "gRandomizeShuffleKeyRingsFireTemple");
                        UIWidgets::EnhancementCheckbox("Water Temple##RandomizeShuffleKeyRings",
                                                       "gRandomizeShuffleKeyRingsWaterTemple");
                        UIWidgets::EnhancementCheckbox("Spirit Temple##RandomizeShuffleKeyRings",
                                                       "gRandomizeShuffleKeyRingsSpiritTemple");
                        UIWidgets::EnhancementCheckbox("Shadow Temple##RandomizeShuffleKeyRings",
                                                       "gRandomizeShuffleKeyRingsShadowTemple");
                        UIWidgets::EnhancementCheckbox("Bottom of the Well##RandomizeShuffleKeyRings",
                        ...
                        UIWidgets::EnhancementCheckbox("Ganon's Castle##RandomizeShuffleKeyRings",
                ...
                UIWidgets::InsertHelpHoverText("Start with - You will start with Boss keys from all dungeons.\n"
                                               "\n"
                ...
                UIWidgets::InsertHelpHoverText(
                    ...
                    "\n"
                    "LACS - These settings put the boss key on the Light Arrow Cutscene location, from Zelda in Temple "
                    ...
                    "- Dungeons: Complete the specified amount of dungeons. Dungeons are considered complete after "
                    "stepping in to the blue warp after the boss.\n"
                    "- Tokens: Obtain the specified amount of Skulltula tokens.\n"
                    "\n"
                    "100 GS Reward - Ganon's Boss Key will be awarded by the cursed rich man after you collect 100 "
                ...
                switch (CVarGetInteger("gRandomizeShuffleGanonBossKey", RO_GANON_BOSS_KEY_VANILLA)) {
                    case RO_GANON_BOSS_KEY_LACS_STONES:
                        ...
                        switch (CVarGetInteger("gRandomizeLacsRewardOptions", RO_LACS_STANDARD_REWARD)) {
                            ...
                            case RO_LACS_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Stone Count: %d", "##RandoLacsStoneCount",
                                                                      "gRandomizeLacsStoneCount", 1, 4, "", 4, true,
                                                                      true, false);
                                break;
                    ...
                    case RO_GANON_BOSS_KEY_LACS_MEDALLIONS:
                        ...
                        switch (CVarGetInteger("gRandomizeLacsRewardOptions", RO_LACS_STANDARD_REWARD)) {
                            case RO_LACS_STANDARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt(
                                    "Medallion Count: %d", "##RandoLacsMedallionCount", "gRandomizeLacsMedallionCount",
                                    1, 6, "", 6, true, true, false);
                                break;
                            case RO_LACS_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt(
                                    "Medallion Count: %d", "##RandoLacsMedallionCount", "gRandomizeLacsMedallionCount",
                                    1, 7, "", 7, true, true, false);
                                break;
                            case RO_LACS_WILDCARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt(
                                    "Medallion Count: %d", "##RandoLacsMedallionCount", "gRandomizeLacsMedallionCount",
                    ...
                    case RO_GANON_BOSS_KEY_LACS_REWARDS:
                        ...
                        switch (CVarGetInteger("gRandomizeLacsRewardOptions", RO_LACS_STANDARD_REWARD)) {
                            ...
                            case RO_LACS_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Reward Count: %d", "##RandoLacsRewardCount",
                                                                      "gRandomizeLacsRewardCount", 1, 10, "", 10, true,
                                                                      true, false);
                                break;
                            case RO_LACS_WILDCARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Reward Count: %d", "##RandoLacsRewardCount",
                    ...
                    case RO_GANON_BOSS_KEY_LACS_DUNGEONS:
                        ...
                        switch (CVarGetInteger("gRandomizeLacsRewardOptions", RO_LACS_STANDARD_REWARD)) {
                            ...
                            case RO_LACS_GREG_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Dungeon Count: %d", "##RandoLacsDungeonCount",
                                                                      "gRandomizeLacsDungeonCount", 1, 9, "", 9, true,
                                                                      true, false);
                                break;
                            case RO_LACS_WILDCARD_REWARD:
                                UIWidgets::PaddedEnhancementSliderInt("Dungeon Count: %d", "##RandoLacsDungeonCount",
        ...
        if (ImGui::BeginTabItem("Gameplay")) {
            ...
            if (ImGui::BeginTable("tableRandoGameplay", 3, ImGuiTableFlags_BordersH | ImGuiTableFlags_BordersV)) {
                ...
                ImGui::SameLine();
                UIWidgets::EnhancementSliderInt("", "##RandoBigPoeTargetCount", "gRandomizeBigPoeTargetCount", 1, 10,
                                                "", 10);
                UIWidgets::PaddedSeparator();
                ...
                static const char* disableChildStealthText =
                    "This option is disabled because \"Skip Child Zelda\" is enabled";
                UIWidgets::EnhancementCheckbox(Settings::SkipChildStealth.GetName().c_str(),
                                               "gRandomizeSkipChildStealth", disableChildStealth,
                                               disableChildStealthText);
                UIWidgets::InsertHelpHoverText(
                ...
                // Skip tower escape
                UIWidgets::EnhancementCheckbox(Settings::SkipTowerEscape.GetName().c_str(),
                ...
                UIWidgets::InsertHelpHoverText("Start with the ability to summon Pierre the scarecrow. Pulling out an "
                                               "ocarina in the usual locations will automatically summon him.");
                ...
                UIWidgets::InsertHelpHoverText("Sets how many major items appear in the item pool.\n"
                                               "\n"
                ...
                UIWidgets::InsertHelpHoverText("Sets how many items are replaced by ice traps.\n"
                                               "\n"
                                               "Off - No ice traps.\n"
                                               "\n"
                ...
                if (CVarGetInteger("gRandomizeGossipStoneHints", RO_GOSSIP_STONES_NEED_NOTHING) !=
                    ...
                    UIWidgets::InsertHelpHoverText("Sets how many hints will be useful.\n"
                                                   "\n"
                    ...
                    UIWidgets::EnhancementCombobox("gRandomizeHintDistribution", randoHintDistribution,
                ...
                UIWidgets::PaddedEnhancementCheckbox("Altar Text", "gRandomizeAltarHint", true, false, false, "",
                                                     UIWidgets::CheckboxGraphics::Cross, true);
                UIWidgets::InsertHelpHoverText(
                    "Reading the Temple of Time altar as child will tell you the locations of the Spiritual Stones.\n"
                    "Reading the Temple of Time altar as adult will tell you the locations of the Medallions, as well "
                    "as the conditions for building the Rainbow Bridge and getting the Boss Key for Ganon's Castle.");
                UIWidgets::PaddedEnhancementCheckbox("Ganondorf (Light Arrows)", "gRandomizeLAHint", true, false, false,
                                                     "", UIWidgets::CheckboxGraphics::Cross, true);
                UIWidgets::InsertHelpHoverText("Talking to Ganondorf in his boss room will tell you the location of "
                                               "the Light Arrows. If this option is enabled and Ganondorf is reachable "
                                               "without Light Arrows, Gossip Stones will never hint the Light Arrows.");
                UIWidgets::PaddedEnhancementCheckbox("Dampe's Diary (Hookshot)", "gRandomizeDampeHint", true, false);
                UIWidgets::InsertHelpHoverText("Reading the diary of Dampé the gravekeeper as adult will tell you the "
                                               "location of one of the Hookshots.");
                UIWidgets::PaddedEnhancementCheckbox("Greg the Green Rupee", "gRandomizeGregHint", true, false);
                UIWidgets::InsertHelpHoverText("Talking to the chest game owner after buying a key will tell you the "
                                               "location of Greg the Green Rupee.");
                UIWidgets::PaddedEnhancementCheckbox("Warp Song text", "gRandomizeWarpSongText", true, false,
                                                     !CVarGetInteger("gRandomizeShuffleWarpSongs", RO_GENERIC_OFF),
                                                     "This option is disabled since warp songs are not shuffled.",
                                                     UIWidgets::CheckboxGraphics::Cross, true);
                UIWidgets::InsertHelpHoverText("Playing a warp song will tell you where it leads. (If warp song "
                                               "destinations are vanilla, this is always enabled.)");
                UIWidgets::PaddedEnhancementCheckbox("Scrub Item text", "gRandomizeScrubText", true, false, false, "",
                ...
                // Bombchus in Logic
                UIWidgets::EnhancementCheckbox(Settings::BombchusInLogic.GetName().c_str(),
                                               "gRandomizeBombchusInLogic");
                UIWidgets::InsertHelpHoverText("Bombchus are properly considered in logic.\n"
                                               "\n"
                ...
                UIWidgets::InsertHelpHoverText("Once you obtain bombchus for the first time, refills can be found in "
                                               "bushes and other places where bomb drops can normally spawn.");
        ...
        if (ImGui::BeginTabItem("Tricks/Glitches")) {
            ...
            if (ImGui::BeginTable("tableRandoLogic", 1, ImGuiTableFlags_BordersH | ImGuiTableFlags_BordersV)) {
                ...
                UIWidgets::InsertHelpHoverText(
                    "Glitchless - No glitches are required, but may require some minor tricks. Additional tricks may "
                    "be enabled and disabled below.\n"
                    "\n"
                    //"Glitched - Glitches may be required to beat the game. You can disable and enable glitches
                    // below.\n"
                    //"\n"
                ...
                if (CVarGetInteger("gRandomizeLogicRules", RO_LOGIC_GLITCHLESS) != RO_LOGIC_NO_LOGIC) {
                    ImGui::SameLine();
                    UIWidgets::EnhancementCheckbox(Settings::LocationsReachable.GetName().c_str(),
                                                   "gRandomizeAllLocationsReachable", false, "",
                                                   UIWidgets::CheckboxGraphics::Cross, RO_GENERIC_ON);
                    UIWidgets::InsertHelpHoverText("When this options is enabled, the randomizer will "
            ...
            static std::unordered_map<RandomizerTrickTag, bool> showTag{ { RTTAG_NOVICE, true },
                ...
                if (CVarGetInteger("gRandomizeLogicRules", RO_LOGIC_GLITCHLESS) != RO_LOGIC_NO_LOGIC) {
                    ...
                    if (ImGui::Button("Collapse All##disabled")) {
                        for (auto [rtArea, rtObjects] : RandomizerTricks::GetAllRTObjectsByArea()) {
                            ...
                            for (auto [randomizerTrick, rtObject] : rtObjects) {
                                if (rtObject.visibleInImgui && !enabledTricks.count(rtObject.rt) &&
                                    !rtObject.rtGlitch) {
                    ...
                    if (ImGui::Button("Enable Visible")) {
                        for (auto [rtArea, rtObjects] : RandomizerTricks::GetAllRTObjectsByArea()) {
                            for (auto [randomizerTrick, rtObject] : rtObjects) {
                                if (!rtObject.rtGlitch && !enabledTricks.count(rtObject.rt) &&
                                    trickSearch.PassFilter(rtObject.rtShortName.c_str()) && areaTreeDisabled[rtArea] &&
                    ...
                    for (auto [rtArea, rtObjects] : RandomizerTricks::GetAllRTObjectsByArea()) {
                        bool hasTricks = false;
                        for (auto [randomizerTrick, rtObject] : rtObjects) {
                            if (rtObject.visibleInImgui && trickSearch.PassFilter(rtObject.rtShortName.c_str()) &&
                                !enabledTricks.count(rtObject.rt) &&
                                RandomizerTricks::CheckRTTags(showTag, *rtObject.rtTags) && !rtObject.rtGlitch) {
                        ...
                        if (hasTricks) {
                            ImGui::TreeNodeSetOpen(ImGui::GetID(RandomizerTricks::GetRTAreaName(rtArea).c_str()),
                                                   areaTreeDisabled[rtArea]);
                            ImGui::SetNextItemOpen(true, ImGuiCond_Once);
                    ...
                    if (ImGui::Button("Collapse All##enabled")) {
                        for (auto [rtArea, rtObjects] : RandomizerTricks::GetAllRTObjectsByArea()) {
                            bool hasTricks = false;
                            for (auto [randomizerTrick, rtObject] : rtObjects) {
                                if (rtObject.visibleInImgui && enabledTricks.count(rtObject.rt) && !rtObject.rtGlitch) {
                    ...
                    if (ImGui::Button("Disable Visible")) {
                        for (auto [rtArea, rtObjects] : RandomizerTricks::GetAllRTObjectsByArea()) {
                            for (auto [randomizerTrick, rtObject] : rtObjects) {
                                auto etfound = enabledTricks.find(randomizerTrick);
                                if (!rtObject.rtGlitch && etfound != enabledTricks.end() &&
                                    trickSearch.PassFilter(rtObject.rtShortName.c_str()) && areaTreeEnabled[rtArea] &&
                                    RandomizerTricks::CheckRTTags(showTag, *rtObject.rtTags)) {
                                    enabledTricks.erase(etfound);
                                }
                            }
        ...
        if (ImGui::BeginTabItem("Starting Inventory")) {
            ...
            if (ImGui::BeginTable("tableRandoStartingInventory", 3,
                ...
                if (CVarGetInteger("gRandomizeShuffleDungeonReward", RO_DUNGEON_REWARDS_END_OF_DUNGEON) !=
                    RO_DUNGEON_REWARDS_END_OF_DUNGEON) {
                    ImGui::Text(Settings::LinksPocketItem.GetName().c_str());
                    UIWidgets::EnhancementCombobox("gRandomizeLinksPocket", randoLinksPocket,
                                                   RO_LINKS_POCKET_DUNGEON_REWARD);
                    UIWidgets::PaddedSeparator();
                }
                ...
                UIWidgets::EnhancementCheckbox(Settings::StartingOcarina.GetName().c_str(),
                ...
                UIWidgets::PaddedSeparator();
                UIWidgets::EnhancementSliderInt("Gold Skulltula Tokens: %d", "##RandoStartingSkulltulaToken",
                                                "gRandomizeStartingSkulltulaToken", 0, 100, "", 0);
                UIWidgets::PaddedSeparator();
...
CustomMessage Randomizer::GetWarpSongMessage(u16 textId, bool mysterious) {
    CustomMessage messageEntry =
        CustomMessageManager::Instance->RetrieveMessage(Randomizer::hintMessageTableID, TEXT_WARP_RANDOM_REPLACED_TEXT);
    if (mysterious) {
        ...
        messageEntry.Replace("{{location}}", locationName[0], locationName[1], locationName[2]);
        return messageEntry;
    }
...
CustomMessage Randomizer::GetMerchantMessage(RandomizerInf randomizerInf, u16 textId, bool mysterious) {
    CustomMessage messageEntry =
        CustomMessageManager::Instance->RetrieveMessage(Randomizer::merchantMessageTableID, textId);
    ...
    if (mysterious) {
        ...
        shopItemName = { std::string(this->itemLocations[rc].trickName), std::string(this->itemLocations[rc].trickName),
    ...
    messageEntry.Replace("{{item}}", std::move(shopItemName[0]), std::move(shopItemName[1]),
                         std::move(shopItemName[2]));
    messageEntry.Replace("{{price}}", std::to_string(shopItemPrice));
    return messageEntry;
}

CustomMessage Randomizer::GetCursedSkullMessage(s16 params) {
    CustomMessage   messageEntry = CustomMessageManager::Instance->RetrieveMessage(Randomizer::randoMiscHintsTableID,
    ...
    if (itemGet == RG_ICE_TRAP) {
        ...
        itemName = { std::string(this->itemLocations[rc].trickName), std::string(this->itemLocations[rc].trickName),
                     std::string(this->itemLocations[rc].trickName) };
    } else {
...
CustomMessage Randomizer::GetRupeeMessage(u16 rupeeTextId) {
    ...
    messageEntry.Replace("{{rupee}}", RandomElement(englishRupeeNames), RandomElement(germanRupeeNames),
...
void CreateIceTrapRandoMessages() {
    ...
    for (u8 i = 0; i <= (NUM_ICE_TRAP_MESSAGES - 1); i++) {
        customMessageManager->CreateMessage(Randomizer::IceTrapRandoMessageTableID, i, IceTrapMessages[i]);
    }
    ...
    const std::array<GetItemMessage, 56> getItemMessages = {
        { GIMESSAGE(RG_GREG_RUPEE, ITEM_MASK_GORON, "You found %gGreg%w!", "%gGreg%w! Du hast ihn wirklich gefunden!",
          ...
          GIMESSAGE(RG_BOTTLE_WITH_BLUE_FIRE, ITEM_BLUE_FIRE,
                    ...
                    "Vous obtenez une %rBouteille avec&une Flamme Bleue%w! Utilisez-la&pour faire fondre la "
                    "%rGlace&Rouge%w!"),
          ...
          GIMESSAGE(RG_BOTTLE_WITH_BLUE_POTION, ITEM_POTION_BLUE,
                    ...
                    "Du erhältst ein %rBlaues Elexier%w!&Nutze es, um deine %rMagie- und&Energieleiste%w "
          ...
          GIMESSAGE(RG_BOTTLE_WITH_FISH, ITEM_FISH,
                    ...
                    "Vous obtenez une %rBouteille avec&un Poisson%w! Il a l'air délicieux!&Il paraîtrait que "
                    "%bJabu-Jabu %wen&serait friand!"),
          ...
          GIMESSAGE(
              ...
              "Vous obtenez une %rBouteille avec&une Potion Rouge%w! Buvez-la pour&restaurer votre %rénergie "
              "vitale%w!"),
          GIMESSAGE(
          ...
          GIMESSAGE(RG_MAGIC_BEAN_PACK, ITEM_BEAN,
                    ...
                    "Du erhältst eine %rPackung&Wundererbsen%w! Suche nach einer&Stelle um sie einzupflanzen.&Warte "
                    "ab, was passiert!",
...
void InitRandoItemTable() {
    ...
    static GetItemEntry extendedVanillaGetItemTable[] = {
        GET_ITEM(ITEM_MEDALLION_LIGHT, OBJECT_GI_MEDAL, GID_MEDALLION_LIGHT, 0x40, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_LIGHT_MEDALLION),
        GET_ITEM(ITEM_MEDALLION_FOREST, OBJECT_GI_MEDAL, GID_MEDALLION_FOREST, 0x3E, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_FOREST_MEDALLION),
        GET_ITEM(ITEM_MEDALLION_FIRE, OBJECT_GI_MEDAL, GID_MEDALLION_FIRE, 0x3C, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_FIRE_MEDALLION),
        GET_ITEM(ITEM_MEDALLION_WATER, OBJECT_GI_MEDAL, GID_MEDALLION_WATER, 0x3D, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_WATER_MEDALLION),
        GET_ITEM(ITEM_MEDALLION_SHADOW, OBJECT_GI_MEDAL, GID_MEDALLION_SHADOW, 0x41, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_SHADOW_MEDALLION),
        GET_ITEM(ITEM_MEDALLION_SPIRIT, OBJECT_GI_MEDAL, GID_MEDALLION_SPIRIT, 0x3F, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_SPIRIT_MEDALLION),

        GET_ITEM(ITEM_KOKIRI_EMERALD, OBJECT_GI_JEWEL, GID_KOKIRI_EMERALD, 0x80, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_KOKIRI_EMERALD),
        GET_ITEM(ITEM_GORON_RUBY, OBJECT_GI_JEWEL, GID_GORON_RUBY, 0x81, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_GORON_RUBY),
        GET_ITEM(ITEM_ZORA_SAPPHIRE, OBJECT_GI_JEWEL, GID_ZORA_SAPPHIRE, 0x82, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_ZORA_SAPPHIRE),

        GET_ITEM(ITEM_SONG_LULLABY, OBJECT_GI_MELODY, GID_SONG_ZELDA, 0xD4, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_ZELDAS_LULLABY),
        GET_ITEM(ITEM_SONG_SUN, OBJECT_GI_MELODY, GID_SONG_SUN, 0xD3, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_SUNS_SONG),
        GET_ITEM(ITEM_SONG_EPONA, OBJECT_GI_MELODY, GID_SONG_EPONA, 0xD2, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_EPONAS_SONG),
        GET_ITEM(ITEM_SONG_STORMS, OBJECT_GI_MELODY, GID_SONG_STORM, 0xD6, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_SONG_OF_STORMS),
        GET_ITEM(ITEM_SONG_TIME, OBJECT_GI_MELODY, GID_SONG_TIME, 0xD5, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_SONG_OF_TIME),
        GET_ITEM(ITEM_SONG_SARIA, OBJECT_GI_MELODY, GID_SONG_SARIA, 0xD1, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_SARIAS_SONG),

        GET_ITEM(ITEM_SONG_MINUET, OBJECT_GI_MELODY, GID_SONG_MINUET, 0x73, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_MINUET_OF_FOREST),
        GET_ITEM(ITEM_SONG_BOLERO, OBJECT_GI_MELODY, GID_SONG_BOLERO, 0x74, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_NONE, RG_BOLERO_OF_FIRE),
        GET_ITEM(ITEM_SONG_SERENADE, OBJECT_GI_MELODY, GID_SONG_SERENADE, 0x75, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_SERENADE_OF_WATER),
        GET_ITEM(ITEM_SONG_NOCTURNE, OBJECT_GI_MELODY, GID_SONG_NOCTURNE, 0x77, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_NOCTURNE_OF_SHADOW),
        GET_ITEM(ITEM_SONG_REQUIEM, OBJECT_GI_MELODY, GID_SONG_REQUIEM, 0x76, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_REQUIEM_OF_SPIRIT),
        GET_ITEM(ITEM_SONG_PRELUDE, OBJECT_GI_MELODY, GID_SONG_PRELUDE, 0x78, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_NONE, RG_PRELUDE_OF_LIGHT),
    };
    ...
    static GetItemEntry randoGetItemTable[] = {
        GET_ITEM(RG_ICE_TRAP, OBJECT_GI_RUPY, GID_RUPEE_GOLD, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_ICE_TRAP),
        GET_ITEM(RG_MAGIC_SINGLE, OBJECT_GI_MAGICPOT, GID_MAGIC_SMALL, 0xE4, 0x80, CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR,
                 MOD_RANDOMIZER, RG_MAGIC_SINGLE),
        GET_ITEM(RG_MAGIC_DOUBLE, OBJECT_GI_MAGICPOT, GID_MAGIC_LARGE, 0xE8, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_MAGIC_DOUBLE),
        GET_ITEM(RG_DOUBLE_DEFENSE, OBJECT_GI_HEARTS, GID_HEART_CONTAINER, 0xE9, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_DOUBLE_DEFENSE),
        GET_ITEM(RG_GREG_RUPEE, OBJECT_GI_RUPY, GID_RUPEE_GREEN, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_GREG_RUPEE),
        GET_ITEM(RG_BOTTLE_WITH_RED_POTION, OBJECT_GI_LIQUID, GID_POTION_RED, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_RED_POTION),
        GET_ITEM(RG_BOTTLE_WITH_GREEN_POTION, OBJECT_GI_LIQUID, GID_POTION_GREEN, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_GREEN_POTION),
        GET_ITEM(RG_BOTTLE_WITH_BLUE_POTION, OBJECT_GI_LIQUID, GID_POTION_BLUE, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_BLUE_POTION),
        GET_ITEM(RG_BOTTLE_WITH_FAIRY, OBJECT_GI_BOTTLE, GID_BOTTLE, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_FAIRY),
        GET_ITEM(RG_BOTTLE_WITH_FISH, OBJECT_GI_FISH, GID_FISH, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_FISH),
        GET_ITEM(RG_BOTTLE_WITH_BLUE_FIRE, OBJECT_GI_FIRE, GID_BLUE_FIRE, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_BLUE_FIRE),
        GET_ITEM(RG_BOTTLE_WITH_BUGS, OBJECT_GI_INSECT, GID_BUG, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_BUGS),
        GET_ITEM(RG_BOTTLE_WITH_POE, OBJECT_GI_GHOST, GID_POE, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_POE),
        GET_ITEM(RG_BOTTLE_WITH_BIG_POE, OBJECT_GI_GHOST, GID_BIG_POE, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_BOTTLE_WITH_BIG_POE),
        GET_ITEM(RG_GERUDO_FORTRESS_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GERUDO_FORTRESS_SMALL_KEY),
        GET_ITEM(RG_FOREST_TEMPLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_FOREST_TEMPLE_SMALL_KEY),
        GET_ITEM(RG_FIRE_TEMPLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_FIRE_TEMPLE_SMALL_KEY),
        GET_ITEM(RG_WATER_TEMPLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_WATER_TEMPLE_SMALL_KEY),
        GET_ITEM(RG_SPIRIT_TEMPLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_SPIRIT_TEMPLE_SMALL_KEY),
        GET_ITEM(RG_SHADOW_TEMPLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_SHADOW_TEMPLE_SMALL_KEY),
        GET_ITEM(RG_BOTTOM_OF_THE_WELL_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_BOTTOM_OF_THE_WELL_SMALL_KEY),
        GET_ITEM(RG_GERUDO_TRAINING_GROUNDS_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GERUDO_TRAINING_GROUNDS_SMALL_KEY),
        GET_ITEM(RG_GANONS_CASTLE_SMALL_KEY, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GANONS_CASTLE_SMALL_KEY),
        GET_ITEM(RG_GERUDO_FORTRESS_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GERUDO_FORTRESS_KEY_RING),
        GET_ITEM(RG_FOREST_TEMPLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_FOREST_TEMPLE_KEY_RING),
        GET_ITEM(RG_FIRE_TEMPLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_FIRE_TEMPLE_KEY_RING),
        GET_ITEM(RG_WATER_TEMPLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_WATER_TEMPLE_KEY_RING),
        GET_ITEM(RG_SPIRIT_TEMPLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_SPIRIT_TEMPLE_KEY_RING),
        GET_ITEM(RG_SHADOW_TEMPLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_SHADOW_TEMPLE_KEY_RING),
        GET_ITEM(RG_BOTTOM_OF_THE_WELL_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_BOTTOM_OF_THE_WELL_KEY_RING),
        GET_ITEM(RG_GERUDO_TRAINING_GROUNDS_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GERUDO_TRAINING_GROUNDS_KEY_RING),
        GET_ITEM(RG_GANONS_CASTLE_KEY_RING, OBJECT_GI_KEY, GID_KEY_SMALL, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_SHORT, ITEM_CATEGORY_SMALL_KEY, MOD_RANDOMIZER, RG_GANONS_CASTLE_KEY_RING),
        GET_ITEM(RG_FOREST_TEMPLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_FOREST_TEMPLE_BOSS_KEY),
        GET_ITEM(RG_FIRE_TEMPLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_FIRE_TEMPLE_BOSS_KEY),
        GET_ITEM(RG_WATER_TEMPLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_WATER_TEMPLE_BOSS_KEY),
        GET_ITEM(RG_SPIRIT_TEMPLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_SPIRIT_TEMPLE_BOSS_KEY),
        GET_ITEM(RG_SHADOW_TEMPLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_SHADOW_TEMPLE_BOSS_KEY),
        GET_ITEM(RG_GANONS_CASTLE_BOSS_KEY, OBJECT_GI_BOSSKEY, GID_KEY_BOSS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_BOSS_KEY, MOD_RANDOMIZER, RG_GANONS_CASTLE_BOSS_KEY),
        GET_ITEM(RG_DEKU_TREE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_DEKU_TREE_MAP),
        GET_ITEM(RG_DODONGOS_CAVERN_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_DODONGOS_CAVERN_MAP),
        GET_ITEM(RG_JABU_JABUS_BELLY_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_JABU_JABUS_BELLY_MAP),
        GET_ITEM(RG_FOREST_TEMPLE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_FOREST_TEMPLE_MAP),
        GET_ITEM(RG_FIRE_TEMPLE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_FIRE_TEMPLE_MAP),
        GET_ITEM(RG_WATER_TEMPLE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_WATER_TEMPLE_MAP),
        GET_ITEM(RG_SPIRIT_TEMPLE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_SPIRIT_TEMPLE_MAP),
        GET_ITEM(RG_SHADOW_TEMPLE_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_SHADOW_TEMPLE_MAP),
        GET_ITEM(RG_BOTTOM_OF_THE_WELL_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_BOTTOM_OF_THE_WELL_MAP),
        GET_ITEM(RG_ICE_CAVERN_MAP, OBJECT_GI_MAP, GID_DUNGEON_MAP, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_ICE_CAVERN_MAP),
        GET_ITEM(RG_DEKU_TREE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_DEKU_TREE_COMPASS),
        GET_ITEM(RG_DODONGOS_CAVERN_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_DODONGOS_CAVERN_COMPASS),
        GET_ITEM(RG_JABU_JABUS_BELLY_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_JABU_JABUS_BELLY_COMPASS),
        GET_ITEM(RG_FOREST_TEMPLE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_FOREST_TEMPLE_COMPASS),
        GET_ITEM(RG_FIRE_TEMPLE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_FIRE_TEMPLE_COMPASS),
        GET_ITEM(RG_WATER_TEMPLE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_WATER_TEMPLE_COMPASS),
        GET_ITEM(RG_SPIRIT_TEMPLE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_SPIRIT_TEMPLE_COMPASS),
        GET_ITEM(RG_SHADOW_TEMPLE_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_SHADOW_TEMPLE_COMPASS),
        GET_ITEM(RG_BOTTOM_OF_THE_WELL_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_BOTTOM_OF_THE_WELL_COMPASS),
        GET_ITEM(RG_ICE_CAVERN_COMPASS, OBJECT_GI_COMPASS, GID_COMPASS, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_ICE_CAVERN_COMPASS),
        GET_ITEM(RG_MAGIC_BEAN_PACK, OBJECT_GI_BEAN, GID_BEAN, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_MAGIC_BEAN_PACK),
        GET_ITEM(RG_TYCOON_WALLET, OBJECT_GI_PURSE, GID_WALLET_GIANT, TEXT_RANDOMIZER_CUSTOM_ITEM, 0x80,
                 CHEST_ANIM_LONG, ITEM_CATEGORY_LESSER, MOD_RANDOMIZER, RG_TYCOON_WALLET),
        GET_ITEM(RG_PROGRESSIVE_BOMBCHUS, OBJECT_GI_BOMB_2, GID_BOMBCHU, 0x33, 0x80, CHEST_ANIM_LONG,
                 ITEM_CATEGORY_MAJOR, MOD_RANDOMIZER, RG_PROGRESSIVE_BOMBCHUS),
    };
    ItemTableManager::Instance->AddItemTable(MOD_RANDOMIZER);
    for (int i = 0; i < ARRAY_COUNT(extendedVanillaGetItemTable); i++) {
        ItemTableManager::Instance->AddItemEntry(MOD_RANDOMIZER, extendedVanillaGetItemTable[i].getItemId,
                                                 extendedVanillaGetItemTable[i]);
    }
    for (int i = 0; i < ARRAY_COUNT(randoGetItemTable); i++) {
        if (randoGetItemTable[i].itemId >= RG_FOREST_TEMPLE_SMALL_KEY &&
            randoGetItemTable[i].itemId <= RG_GANONS_CASTLE_SMALL_KEY &&
        ...
        } else if (randoGetItemTable[i].itemId >= RG_FOREST_TEMPLE_KEY_RING &&
                   randoGetItemTable[i].itemId <= RG_GANONS_CASTLE_KEY_RING) {
            randoGetItemTable[i].drawFunc = (CustomDrawFunc)Randomizer_DrawKeyRing;
        } else if (randoGetItemTable[i].itemId >= RG_FOREST_TEMPLE_BOSS_KEY &&
                   randoGetItemTable[i].itemId <= RG_GANONS_CASTLE_BOSS_KEY) {
