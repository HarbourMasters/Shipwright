...
void RegionTable_Init_Kakariko() {
    ...
    areaTable[RR_KAKARIKO_VILLAGE] = Region("Kakariko Village", "Kakariko Village", {RA_KAKARIKO_VILLAGE}, NO_DAY_NIGHT_CYCLE, {
        ...
        Entrance(RR_KAK_WELL,                 []{return logic->IsAdult || logic->DrainWell || logic->CanUse(RG_IRON_BOOTS) || (ctx->GetTrickOption(RT_BOTTOM_OF_THE_WELL_NAVI_DIVE) && logic->CanUse(RG_STICKS) && logic->HasItem(RG_BRONZE_SCALE)) || (ctx->GetTrickOption(RT_BOTTOM_OF_THE_WELL_NAVI_DIVE) && logic->CanUse(RG_KOKIRI_SWORD) && logic->HasItem(RG_BRONZE_SCALE));}),
