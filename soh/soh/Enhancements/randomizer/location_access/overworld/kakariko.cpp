...
void RegionTable_Init_Kakariko() {
    ...
    areaTable[RR_KAKARIKO_VILLAGE] = Region("Kakariko Village", SCENE_KAKARIKO_VILLAGE, {
        ...
        LOCATION_ROUTES(RC_KAK_SILVER_BOULDER, ROUTE(logic->IsAdult && logic->CanUse(RG_SILVER_GAUNTLETS) && (logic->HasItem(RG_CLIMB) || logic->CanUse(RG_HOVER_BOOTS) || (logic->HasItem(RG_LONGSHOT) && (logic->AtDay && logic->HasItem(RG_POWER_BRACELET))))), ROUTE(logic->IsAdult && logic->CanUse(RG_SILVER_GAUNTLETS) && logic->HasItem(RG_LONGSHOT) && ctx->GetTrickOption(RT_UNINTUITIVE_JUMPS) && logic->CanJumpslash(), logic->HitCost())),
