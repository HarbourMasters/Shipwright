...
namespace Settings {
  ...
  Option ShuffleChildWallet     = Option::Bool("Shuffle Child's Wallet", {"Off", "On"},                                                     {childWalletDesc});
  ...
  SettingsContext FillContext() {
    ...
    ctx.shuffleChildWallet   = (ShuffleChildWallet) ? 1 : 0;
  ...
  void UpdateSettings(std::unordered_map<RandomizerSettingKey, uint8_t> cvarSettings, std::set<RandomizerCheck> excludedLocations, std::set<RandomizerTrick> enabledTricks) {
    ...
    ShuffleChildWallet.SetSelectedIndex(cvarSettings[RSK_SHUFFLE_CHILD_WALLET]);
