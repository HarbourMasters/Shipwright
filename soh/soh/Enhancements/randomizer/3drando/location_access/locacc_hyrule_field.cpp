...
void AreaTable_Init_HyruleField() {
    areaTable[HYRULE_FIELD] =
        Area("Hyrule Field", "Hyrule Field", HYRULE_FIELD, DAY_NIGHT_CYCLE,
             {
                 // Events
        ...
        },
        {
            ...
            LocationAccess(LH_GS_LAB_WALL, { [] {
                               return IsChild &&
                                      (HookshotOrBoomerang ||
                                       (LogicLabWallGS &&
                                        (Sticks || KokiriSword || CanUse(MASTER_SWORD) || CanUse(BIGGORON_SWORD)))) &&
        ...
        },
        {
            ...
            Entrance(WATER_TEMPLE_ENTRYWAY, { [] {
                         return CanUse(HOOKSHOT) &&
                                ((CanUse(IRON_BOOTS) || (LogicWaterHookshotEntry && ProgressiveScale >= 2)) ||
                                 (IsAdult && CanUse(LONGSHOT) && ProgressiveScale >= 2));
                     } }),
    ...
    areaTable[LH_LAB] =
        Area("LH Lab", "LH Lab", NONE, NO_DAY_NIGHT_CYCLE,
             {
                 // Events
             ...
             {
                 // Locations
                 LocationAccess(LH_LAB_DIVE, { [] {
                                    return ProgressiveScale >= 2 ||
                                           (LogicLabDiving && CanUse(IRON_BOOTS) && CanUse(HOOKSHOT));
                                } }),
