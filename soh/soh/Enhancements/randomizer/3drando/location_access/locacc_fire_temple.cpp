...
void AreaTable_Init_FireTemple() {
  ...
  areaTable[FIRE_TEMPLE_MQ_UPPER] = Area("Fire Temple MQ Upper", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {
                  ...
                  LocationAccess(FIRE_TEMPLE_MQ_CHEST_ON_FIRE,               {[]{return ((IsAdult || (IsChild && (CanPlay(SongOfTime) || CanUse(HOVER_BOOTS)) && CanUse(SCARECROW))) && CanUse(HOOKSHOT) && CanUse(MEGATON_HAMMER)) || ((IsAdult || (IsChild && CanUse(SCARECROW)))) && LogicFireMQFlameMaze && (CanUse(MEGATON_HAMMER) || CanUse(HOOKSHOT) || (HasExplosives && (CanUseSword || CanUse(BOW) || CanUse(SLINGSHOT) || CanUse(BOOMERANG)))) && SmallKeys(FIRE_TEMPLE, 4);}}),
