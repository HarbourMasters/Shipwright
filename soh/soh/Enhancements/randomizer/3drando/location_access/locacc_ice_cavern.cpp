...
void AreaTable_Init_IceCavern() {
  ...
  areaTable[RR_ICE_CAVERN_MAIN] = Area("Ice Cavern", "Ice Cavern", RA_ICE_CAVERN, NO_DAY_NIGHT_CYCLE, {
                  //Events
                  EventAccess(&logic->BlueFireAccess, {[]{return logic->BlueFireAccess || (logic->CanBeAdult && logic->HasBottle);}}),
                }, {
                  //Locations
                  LocationAccess(RC_ICE_CAVERN_MAP_CHEST,               {[]{return logic->BlueFire && logic->CanBeAdult;}}),
                  ...
                  LocationAccess(RC_SHEIK_IN_ICE_CAVERN,                {[]{return logic->BlueFire && (logic->CanJumpslash || logic->CanUse(RG_FAIRY_SLINGSHOT) || logic->CanUse(RG_FAIRY_BOW) || logic->CanUse(RG_MEGATON_HAMMER) || logic->CanUse(RG_DINS_FIRE)) && logic->CanBeAdult;}}),
                  ...
                  LocationAccess(RC_ICE_CAVERN_GS_PUSH_BLOCK_ROOM,      {[]{return logic->BlueFire && (logic->HookshotOrBoomerang || (randoCtx->GetTrickOption(RT_ICE_BLOCK_GS) && logic->CanBeAdult && logic->CanUse(RG_HOVER_BOOTS)));}}),
  }, {});
  }
  ...
  areaTable[RR_ICE_CAVERN_MQ_BEGINNING] = Area("Ice Cavern MQ Beginning", "Ice Cavern", RA_ICE_CAVERN, NO_DAY_NIGHT_CYCLE, {
                  ...
                  Entrance(RR_ICE_CAVERN_MQ_COMPASS_ROOM,      {[]{return logic->CanBeAdult && logic->BlueFire;}}),
  ...
  areaTable[RR_ICE_CAVERN_MQ_IRON_BOOTS_REGION] = Area("Ice Cavern MQ Iron Boots Region", "Ice Cavern", RA_ICE_CAVERN, NO_DAY_NIGHT_CYCLE, {}, {
                  //Locations
                  LocationAccess(RC_ICE_CAVERN_MQ_IRON_BOOTS_CHEST, {[]{return logic->CanBeAdult && (logic->CanJumpslash || logic->CanUse(RG_MEGATON_HAMMER));}}),
                  LocationAccess(RC_SHEIK_IN_ICE_CAVERN,            {[]{return logic->CanBeAdult && (logic->CanJumpslash || logic->CanUse(RG_MEGATON_HAMMER));}}),
                  LocationAccess(RC_ICE_CAVERN_MQ_GS_ICE_BLOCK,     {[]{return logic->CanAdultAttack || logic->CanChildAttack;}}),
                  LocationAccess(RC_ICE_CAVERN_MQ_GS_SCARECROW,     {[]{return logic->CanUse(RG_SCARECROW) || (logic->HoverBoots && logic->CanUse(RG_LONGSHOT)) || (randoCtx->GetTrickOption(RT_ICE_MQ_SCARECROW) && logic->CanBeAdult);}}),
                    //Tricks: (logic->CanUse(RG_SCARECROW) || (logic->HoverBoots && logic->CanUse(RG_LONGSHOT)) || LogicIceMQScarecrow) && logic->CanBeAdult
  }, {});
  ...
  areaTable[RR_ICE_CAVERN_MQ_COMPASS_ROOM] = Area("Ice Cavern MQ Compass Room", "Ice Cavern", RA_ICE_CAVERN, NO_DAY_NIGHT_CYCLE, {}, {
                  ...
                  LocationAccess(RC_ICE_CAVERN_MQ_GS_RED_ICE,       {[]{return logic->CanPlay(logic->SongOfTime) || randoCtx->GetTrickOption(RT_ICE_MQ_RED_ICE_GS);}}),
                    //Trick: logic->CanPlay(logic->SongOfTime) || LogicIceMQRedIceGS
  }, {});
  }
}
