...
void AreaTable_Init_ZorasDomain() {
  ...
  areaTable[RR_ZORAS_RIVER] = Area("Zora River", "Zora River", RA_ZORAS_RIVER, DAY_NIGHT_CYCLE, {
                  ...
                  LocationAccess(RC_ZR_FROGS_OCARINA_GAME,                {[]{return logic->IsChild && logic->CanPlay(logic->ZeldasLullaby) && logic->CanPlay(logic->SariasSong) && logic->CanPlay(logic->SunsSong) && logic->CanPlay(logic->EponasSong) && logic->CanPlay(logic->SongOfTime) && logic->CanPlay(logic->SongOfStorms);}}),
                  LocationAccess(RC_ZR_FROGS_IN_THE_RAIN,                 {[]{return logic->IsChild && logic->CanPlay(logic->SongOfStorms);}}),
                  LocationAccess(RC_ZR_FROGS_ZELDAS_LULLABY,              {[]{return logic->IsChild && logic->CanPlay(logic->ZeldasLullaby);}}),
                  LocationAccess(RC_ZR_FROGS_EPONAS_SONG,                 {[]{return logic->IsChild && logic->CanPlay(logic->EponasSong);}}),
                  LocationAccess(RC_ZR_FROGS_SARIAS_SONG,                 {[]{return logic->IsChild && logic->CanPlay(logic->SariasSong);}}),
                  LocationAccess(RC_ZR_FROGS_SUNS_SONG,                   {[]{return logic->IsChild && logic->CanPlay(logic->SunsSong);}}),
                  LocationAccess(RC_ZR_FROGS_SONG_OF_TIME,                {[]{return logic->IsChild && logic->CanPlay(logic->SongOfTime);}}),
                  ...
                  Entrance(RR_ZR_BEHIND_WATERFALL, {[]{return logic->CanPlay(logic->ZeldasLullaby) || (logic->IsChild && randoCtx->GetTrickOption(RT_ZR_CUCCO)) || (logic->IsAdult && logic->CanUse(RG_HOVER_BOOTS) && randoCtx->GetTrickOption(RT_ZR_HOVERS));}}),
  });
