...
void AreaTable_Init_Kakariko() {
    areaTable[KAKARIKO_VILLAGE] = Area(
        ...
        },
        {
            ...
            LocationAccess(KAK_GS_WATCHTOWER, { [] {
                               return IsChild &&
                                      (Slingshot || HasBombchus || CanUse(BOW) || CanUse(LONGSHOT) ||
                                       (LogicKakarikoTowerGS && CanJumpslash)) &&
        ...
        },
        {
            ...
            Entrance(KAK_IMPAS_LEDGE,
                     { [] { return (IsChild && AtDay) || CanUse(HOOKSHOT) || (IsAdult && LogicVisibleCollision); } }),
            ...
            Entrance(KAK_IMPAS_ROOFTOP,
                     { [] { return CanUse(HOOKSHOT) || (LogicKakarikoRooftopGS && CanUse(HOVER_BOOTS)); } }),
    ...
    areaTable[KAK_WINDMILL] =
        Area("Kak Windmill", "Windmill and Dampes Grave", NONE, NO_DAY_NIGHT_CYCLE,
             {
                 // Events
             ...
             {
                 // Locations
                 LocationAccess(KAK_WINDMILL_FREESTANDING_POH, { [] {
                                    return CanUse(BOOMERANG) || DampesWindmillAccess ||
                                           (IsAdult && LogicAdultWindmillPoH) ||
                                           (IsChild && CanJumpslash && LogicChildWindmillPoH);
                                } }),
    ...
    areaTable[KAK_ODD_POTION_BUILDING] = Area(
        ...
        Area("Kak Redead Grotto", "Kak Redead Grotto", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 ...
                 LocationAccess(KAK_REDEAD_GROTTO_CHEST, { [] {
                                    return IsAdult ||
                                           (Sticks || KokiriSword || CanUse(DINS_FIRE) || CanUse(MEGATON_HAMMER) ||
        ...
        Area("Graveyard Composers Grave", "Graveyard Composers Grave", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 ...
                 LocationAccess(SONG_FROM_COMPOSERS_GRAVE, { [] {
                                    return IsAdult || (Slingshot || Boomerang || Sticks || HasExplosives ||
                                                       KokiriSword || CanUse(MEGATON_HAMMER) || CanUse(MASTER_SWORD) ||
                                                       CanUse(BIGGORON_SWORD) || CanUse(BOW) || CanUse(HOOKSHOT));
                                } }),
             },
             {
                 // Exits
    ...
    areaTable[GRAVEYARD_WARP_PAD_REGION] =
        Area("Graveyard Warp Pad Region", "Graveyard", THE_GRAVEYARD, NO_DAY_NIGHT_CYCLE,
             {
                 // Events
