...
static void WriteLocation(std::string sphere, const uint32_t locationKey, const bool withPadding = false) {
...
static void WriteShuffledEntrance(std::string sphereString, Entrance* entrance) {
    ...
    std::string text = entrance->GetConnectedRegion()->regionName + " from " +
...
static void WriteSettings(const bool printAll = false) {
    ...
    for (const Menu* menu : allMenus) {
        if (menu->name == "Cosmetic Settings" || menu->name == "Ingame Defaults" ||
            menu->name == "Item Usability Settings" || menu->name == "Multiplayer Settings")
            continue;
        ...
        if (menu->name == "Timesaver Settings") {
            for (const Option* setting : *menu->settingsList) {
                if (setting->GetName() == "Big Poe Target Count" || setting->GetName() == "Cuccos to return" ||
                    setting->GetName() == "Skip Epona Race" || setting->GetName() == "Skip Tower Escape" ||
                    setting->GetName() == "Skip Child Stealth" || setting->GetName() == "Complete Mask Quest" ||
...
static void WriteStartingInventory() {
    std::vector<std::vector<Option*>*> startingInventoryOptions = {
        &Settings::startingItemsOptions, &Settings::startingSongsOptions, &Settings::startingEquipmentOptions,
        &Settings::startingStonesMedallionsOptions, &Settings::startingOthersOptions
    };
    ...
    for (std::vector<Option*>* menu : startingInventoryOptions) {
        for (size_t i = 0; i < menu->size(); ++i) {
            ...
            // Starting Songs
            if (setting->GetName() == "Start with Zelda's Lullaby" || setting->GetName() == "Start with Epona's Song" ||
                setting->GetName() == "Start with Saria's Song" || setting->GetName() == "Start with Sun's Song" ||
                setting->GetName() == "Start with Song of Time" || setting->GetName() == "Start with Song of Storms" ||
    ...
    for (std::vector<Option*>* menu : startingInventoryOptions) {
        for (size_t i = 0; i < menu->size(); ++i) {
            ...
            if (setting->GetName() == "Start with Consumables" || setting->GetName() == "Start with Max Rupees" ||
                setting->GetName() == "Gold Skulltula Tokens" || setting->GetName() == "Start with Fairy Ocarina" ||
                setting->GetName() == "Start with Kokiri Sword" || setting->GetName() == "Start with Deku Shield") {
                jsonData["settings"][setting->GetName()] = setting->GetSelectedOptionText();
            }
        }
    }
...
static void WriteEnabledTricks(tinyxml2::XMLDocument& spoilerLog) {
    ...
    for (const auto& setting : Settings::trickOptions) {
        ...
        jsonData["enabledTricks"].push_back(
            RemoveLineBreaks(RandomizerTricks::GetRTName((RandomizerTrick)std::stoi(setting->GetName()))).c_str());
...
std::string AutoFormatHintTextString(std::string unformattedHintTextString) {
    ...
    if (textStr == "Erreur 0x69a504:&Traduction manquante^C'est de la faute à Purple Hato!&J'vous jure!" ||
        textStr == "Mon très cher @:&Viens vite au château, je t'ai préparé&un délicieux gâteau...^À bientôt, "
                   "Princesse Zelda" ||
        textStr == "What about Zelda makes you think&she'd be a better ruler than I?^I saved Lon Lon Ranch,&fed the "
        ...
        textStr == "Gamin, ton destin achève,&sous mon sort tu périras!&Cette partie ne fut pas brève,&et cette mort, "
                   "tu subiras!" ||
        ...
        textStr == "They say \"Forgive me, but-^Your script will not be used.&....After all...^The one writing the "
                   "rest of the script...&will be me.\"") {
...
ItemLocation* GetItemLocation(uint32_t item) {
    return Location(FilterFromPool(
