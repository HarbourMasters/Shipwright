    ...
    for (uint32_t startingItem : StartingInventory) {
        for (size_t i = 0; i < ItemPool.size(); i++) {
            if (startingItem == BIGGORON_SWORD) {
                ...
                continue;
            } else if (startingItem == ItemPool[i] ||
                       (ItemTable(startingItem).IsBottleItem() && ItemTable(ItemPool[i]).IsBottleItem())) {
...
static void ValidateWorldChecks(SearchMode& mode, bool checkPoeCollectorAccess, bool checkOtherEntranceAccess,
                                std::vector<uint32_t>& areaPool) {
    // Condition for validating Temple of Time Access
    if (mode == SearchMode::TempleOfTimeAccess &&
        ((Settings::ResolvedStartingAge == AGE_CHILD && AreaTable(TEMPLE_OF_TIME)->Adult()) ||
         (Settings::ResolvedStartingAge == AGE_ADULT && AreaTable(TEMPLE_OF_TIME)->Child()) ||
    ...
    if (mode == SearchMode::ValidStartingRegion) {
        ...
        if ((childAccess && (kokiri->Child() || kakariko->Child())) ||
            (adultAccess && (kokiri->Adult() || kakariko->Adult())) || !checkOtherEntranceAccess) {
    ...
    if (mode == SearchMode::PoeCollectorAccess &&
        (AreaTable(MARKET_GUARD_HOUSE)->Adult() || !checkPoeCollectorAccess)) {
...
static int GetMaxGSCount() {
    ...
    if (Location(KAK_100_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
        Location(KAK_100_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 100;
    } else if (Location(KAK_50_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
               Location(KAK_50_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 50;
    } else if (Location(KAK_40_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
               Location(KAK_40_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 40;
    } else if (Location(KAK_30_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
               Location(KAK_30_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 30;
    } else if (Location(KAK_20_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
               Location(KAK_20_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 20;
    } else if (Location(KAK_10_GOLD_SKULLTULA_REWARD)->GetPlacedItem().IsAdvancement() &&
               Location(KAK_10_GOLD_SKULLTULA_REWARD)->GetPlacedItem().GetItemType() != ITEMTYPE_TOKEN) {
        maxUseful = 10;
    }
...
std::vector<uint32_t> GetAccessibleLocations(const std::vector<uint32_t>& allowedLocations,
    ...
    int       gsCount = 0;
    const int maxGsCount =
        mode == SearchMode::GeneratePlaythrough
            ? GetMaxGSCount()
            : 0; // If generating playthrough want the max that's possibly useful, else doesn't matter
                    ...
                    if (!location->IsAddedToPool() && locPair.ConditionsMet()) {
                        ...
                        if (location->GetPlaceduint32_t() == NONE) {
                            ...
                            if (ignore != "") {
                                ...
                                // We want to ignore a specific Buy item name
                                    ...
                                    if ((type == ITEMTYPE_SHOP && ignore != GetShopItemBaseName(itemName)) ||
                                        type != ITEMTYPE_SHOP) {
                                        newItemLocations.push_back(location);
                                    }
                                }
                            ...
                            // If it doesn't, we can just add the location
                                ...
                                newItemLocations.push_back(
                                    location); // Add item to cache to be considered in logic next iteration
                            }
                        }
                        ...
                        if (mode == SearchMode::GeneratePlaythrough) {
                            ...
                            if (!playthroughBeatable && location->GetPlacedItem().IsAdvancement()) {
                                ...
                                // This preprocessing is done to reduce the amount of searches performed in
                                // PareDownPlaythrough Want to exclude: 1) Tokens after the last potentially useful one
                                // (the last one that gives an advancement item or last for token bridge) 2) Bombchus
                                // after the first (including buy bombchus) 3) Buy items of the same type, after the
                                // first (So only see Buy Deku Nut of any amount once)
                                bool exclude = true;
                                ...
                                // Handle buy items
                                // If ammo drops are off, don't do this step, since buyable ammo becomes logically
                                // important
                                else if (AmmoDrops.IsNot(AMMODROPS_NONE) && !(bombchus && bombchusFound) &&
                                         type == ITEMTYPE_SHOP) {
                                ...
                                else if (!bombchus && type != ITEMTYPE_TOKEN &&
                                         (AmmoDrops.Is(AMMODROPS_NONE) || type != ITEMTYPE_SHOP)) {
                                    exclude = false;
                                }
...
static void CalculateWotH() {
    ...
    for (int i = wothLocations.size() - 1; i >= 0; i--) {
        ...
        // so keep it Else, delete from wothLocations
        if (playthroughBeatable) {
...
static void RandomizeDungeonRewards() {
    ...
    if (ShuffleRewards.Is(REWARDSHUFFLE_END_OF_DUNGEON)) {
        ...
        std::vector<uint32_t> rewards = FilterAndEraseFromPool(
        ...
        std::vector<uint32_t> rewards =
            FilterFromPool(ItemPool, [](const auto i) { return ItemTable(i).GetItemType() == ITEMTYPE_DUNGEONREWARD; });
...
static void FillExcludedLocations() {
    ...
    std::vector<uint32_t> excludedLocations =
        FilterFromPool(allLocations, [](const auto loc) { return Location(loc)->IsExcluded(); });
...
static void RandomizeOwnDungeon(const Dungeon::DungeonInfo* dungeon) {
    ...
    if (Keysanity.Is(KEYSANITY_OWN_DUNGEON) && dungeon->GetSmallKey() != NONE) {
        std::vector<uint32_t> dungeonSmallKeys = FilterAndEraseFromPool(ItemPool, [dungeon](const uint32_t i) {
    ...
    if ((BossKeysanity.Is(BOSSKEYSANITY_OWN_DUNGEON) && dungeon->GetBossKey() != GANONS_CASTLE_BOSS_KEY) ||
        ...
        auto dungeonBossKey =
            FilterAndEraseFromPool(ItemPool, [dungeon](const uint32_t i) { return i == dungeon->GetBossKey(); });
    ...
    if (MapsAndCompasses.Is(MAPSANDCOMPASSES_OWN_DUNGEON) && dungeon->GetMap() != NONE &&
        dungeon->GetCompass() != NONE) {
        auto dungeonMapAndCompass = FilterAndEraseFromPool(
...
static void RandomizeDungeonItems() {
    ...
    std::vector<uint32_t> anyDungeonLocations =
        FilterFromPool(allLocations, [](const auto loc) { return Location(loc)->IsDungeon(); });
    ...
    for (auto dungeon : dungeonList) {
        if (Keysanity.Is(KEYSANITY_ANY_DUNGEON)) {
            auto dungeonKeys = FilterAndEraseFromPool(ItemPool, [dungeon](const uint32_t i) {
                return (i == dungeon->GetSmallKey()) || (i == dungeon->GetKeyRing());
            });
        ...
        if (GanonsBossKey.Is(GANONSBOSSKEY_ANY_DUNGEON)) {
            auto ganonBossKey =
                FilterAndEraseFromPool(ItemPool, [](const auto i) { return i == GANONS_CASTLE_BOSS_KEY; });
    ...
    for (auto dungeon : dungeonList) {
        if (MapsAndCompasses.Is(MAPSANDCOMPASSES_ANY_DUNGEON)) {
            auto mapAndCompassItems = FilterAndEraseFromPool(
                ItemPool, [dungeon](const uint32_t i) { return i == dungeon->GetMap() || i == dungeon->GetCompass(); });
...
int Fill() {
    ...
    while (retries < 5) {
        ...
        FilterAndEraseFromPool(ItemPool,
                               [](const auto item) { return ItemTable(item).GetItemType() == ITEMTYPE_SHOP; });
        ...
        if (Shopsanity.Is(SHOPSANITY_OFF)) {
            ...
            if (Shopsanity.IsNot(SHOPSANITY_ZERO)) { // Shopsanity 1-4, random
                ...
                for (size_t i = 0; i < ShopLocationLists.size(); i++) {
                    int num_to_replace =
                        GetShopsanityReplaceAmount(); // 1-4 shop items will be overwritten, depending on settings
        ...
        if (ShuffleSongs.IsNot(SONGSHUFFLE_ANYWHERE)) {
            ...
            // Get each song
            std::vector<uint32_t> songs = FilterAndEraseFromPool(
                ItemPool, [](const auto i) { return ItemTable(i).GetItemType() == ITEMTYPE_SONG; });

            // Get each song location
            std::vector<uint32_t> songLocations;
            if (ShuffleSongs.Is(SONGSHUFFLE_SONG_LOCATIONS)) {
                songLocations = FilterFromPool(
                    allLocations, [](const auto loc) { return Location(loc)->IsCategory(Category::cSong); });

            } else if (ShuffleSongs.Is(SONGSHUFFLE_DUNGEON_REWARDS)) {
                songLocations = FilterFromPool(allLocations, [](const auto loc) {
                    return Location(loc)->IsCategory(Category::cSongDungeonReward);
                });
            }
