...
void GenerateItemPool() {
    ...
    if (ctx->GetOption(RSK_SHUFFLE_ISG)) {
        AddItemToPool(RG_ABILITY_ISG, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_OI)) {
        AddItemToPool(RG_ABILITY_OI, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_QPA)) {
        AddItemToPool(RG_ABILITY_QPA, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_HESS)) {
        AddItemToPool(RG_ABILITY_HESS, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_SUPERSLIDE)) {
        AddItemToPool(RG_ABILITY_SUPERSLIDE, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_HOVER)) {
        AddItemToPool(RG_ABILITY_HOVER, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_EQUIP_SWAP)) {
        AddItemToPool(RG_ABILITY_EQUIP_SWAP, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_GROUND_JUMP)) {
        AddItemToPool(RG_ABILITY_GROUND_JUMP, 2, 1, 1, 1);
    }
    if (ctx->GetOption(RSK_SHUFFLE_WEIRDSHOT)) {
        AddItemToPool(RG_ABILITY_WEIRDSHOT, 2, 1, 1, 1);
    }
