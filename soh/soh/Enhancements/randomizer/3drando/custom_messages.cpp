...
constexpr std::array DungeonColors = {
    ...
    void CreateAlwaysIncludedMessages() {
        ...
        //Child's Wallet
        CreateMessage(0x09F8, 0, 2, 3,
                UNSKIPPABLE()+ITEM_OBTAINED(ITEM_WALLET_ADULT)+INSTANT_TEXT_ON()+"You got a "+COLOR(QM_RED)+"Child's Wallet"+COLOR(QM_WHITE)+"!"+INSTANT_TEXT_OFF()+NEWLINE()+"Now you can carry"+NEWLINE()+"up to "+COLOR(QM_YELLOW)+"99 "+COLOR(QM_WHITE)+COLOR(QM_YELLOW)+"Rupees"+COLOR(QM_WHITE)+"!"+MESSAGE_END(),
                UNSKIPPABLE()+ITEM_OBTAINED(ITEM_WALLET_ADULT)+INSTANT_TEXT_ON()+"Vous obtenez la "+COLOR(QM_RED)+"Bourse de Magnat"+COLOR(QM_WHITE)+"!"+INSTANT_TEXT_OFF()+NEWLINE()+"Elle peut contenir jusqu'à "+COLOR(QM_YELLOW)+"99 "+COLOR(QM_WHITE)+COLOR(QM_YELLOW)+"rubis"+COLOR(QM_WHITE)+"!"+MESSAGE_END(),//FIXME: still says tycoon
                UNSKIPPABLE()+ITEM_OBTAINED(ITEM_WALLET_ADULT)+INSTANT_TEXT_ON()+"¡Has conseguido una "+COLOR(QM_RED)+"bolsa para niños"+COLOR(QM_WHITE)+"!"+INSTANT_TEXT_OFF()+NEWLINE()+"Ya puedes llevar"+NEWLINE()+"hasta "+COLOR(QM_YELLOW)+"99 "+COLOR(QM_WHITE)+COLOR(QM_YELLOW)+"rupias"+COLOR(QM_WHITE)+"!"+MESSAGE_END());
