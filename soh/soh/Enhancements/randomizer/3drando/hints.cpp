...
std::unordered_map<HintType, std::string> hintTypeNames = {
    { HINT_TYPE_TRIAL, "Trial" },          { HINT_TYPE_ALWAYS, "Always" },     { HINT_TYPE_WOTH, "WotH" },
    { HINT_TYPE_BARREN, "Barren" },        { HINT_TYPE_ENTRANCE, "Entrance" }, { HINT_TYPE_SOMETIMES, "Sometimes" },
    { HINT_TYPE_RANDOM, "Random" },        { HINT_TYPE_ITEM, "Item" },         { HINT_TYPE_SONG, "Song" },
    { HINT_TYPE_OVERWORLD, "Overworld" },  { HINT_TYPE_DUNGEON, "Dungeon" },   { HINT_TYPE_JUNK, "Junk" },
...
static void AddHint(Text hint, const uint32_t gossipStone, const std::vector<uint8_t>& colors = {},
                    HintType hintType = HINT_TYPE_ITEM, const uint32_t hintedLocation = NONE) {
    ...
    Location(gossipStone)
        ->SetHintedRegion(
            GetHintRegion(Location(hintedLocation)->GetParentRegionKey())->GetHint().GetText().GetEnglish());
...
static void CreateWothHint(uint8_t* remainingDungeonWothHints) {
    ...
    std::vector<uint32_t> wothHintLocations = FilterFromPool(wothLocations, [remainingDungeonWothHints](uint32_t loc) {
...
void CreateGanonText() {
    ...
    auto lightArrowLocation = FilterFromPool(
        allLocations, [](const uint32_t loc) { return Location(loc)->GetPlaceduint32_t() == LIGHT_ARROWS; });
    ...
    if (lightArrowLocation.empty()) {
        ...
        ganonHintText =
            hint.GetText() + GetHintRegion(Location(lightArrowLocation[0])->GetParentRegionKey())->GetHint().GetText();
...
static Text BuildDungeonRewardText(const uint32_t itemKey) {
    uint32_t location = FilterFromPool(
        allLocations, [itemKey](const uint32_t loc) { return Location(loc)->GetPlaceduint32_t() == itemKey; })[0];
    ...
    return Text() + rewardString +
           GetHintRegion(Location(location)->GetParentRegionKey())->GetHint().GetText().Capitalize() + "...^";
}
...
void CreateAltarText() {
    ...
    if (AltarHintText) {
        childAltarText =
            ...
            (StartingGoronRuby.Value<uint8_t>() ? Text{ "##", "##", "##" } : BuildDungeonRewardText(GORON_RUBY)) +
            (StartingZoraSapphire.Value<uint8_t>() ? Text{ "##", "##", "##" } : BuildDungeonRewardText(ZORA_SAPPHIRE)) +
    ...
    CreateMessageFromTextObject(
        0x7088, 0, 2, 3,
        AddColorsAndFormat(adultAltarText, { QM_RED, QM_YELLOW, QM_GREEN, QM_RED, QM_BLUE, QM_YELLOW, QM_PINK, QM_RED,
                                             QM_RED, QM_RED, QM_RED }));
}
...
void CreateDampesDiaryText() {
    ...
    uint32_t location = FilterFromPool(
        allLocations, [item](const uint32_t loc) { return Location(loc)->GetPlaceduint32_t() == item; })[0];
    Text area = GetHintRegion(Location(location)->GetParentRegionKey())->GetHint().GetText();
    Text temp1 = Text{ "Whoever reads this, please enter %g", "Toi qui lit ce journal, rends-toi dans %g",
                       "Wer immer dies liest, der möge folgenden Ort aufsuchen: %g" };

    Text temp2 = { "%w. I will let you have my stretching, shrinking keepsake.^I'm waiting for you.&--Dampé",
...
void CreateGregRupeeHint() {
    ...
    uint32_t location = FilterFromPool(
        allLocations, [](const uint32_t loc) { return Location(loc)->GetPlacedItemKey() == GREG_RUPEE; })[0];
    Text area = GetHintRegion(Location(location)->GetParentRegionKey())->GetHint().GetText();

    Text temp1 = Text{ "By the way, if you're interested, I saw the shiniest %gGreen Rupee%w somewhere in%g ", "", "" };
...
void CreateAllHints() {
        ...
        auto alwaysHintLocations = FilterFromPool(allLocations, [](const uint32_t loc) {
            return ((Location(loc)->GetHint().GetType() == HintCategory::Always) ||
                    ...
                    (Bridge.Is(RAINBOWBRIDGE_GREG) && !GregHintText &&
                     Location(loc)->GetPlacedItemKey() == GREG_RUPEE)) &&
    ...
    while (FilterFromPool(gossipStoneLocations, [](const uint32_t loc) {
               return Location(loc)->GetPlaceduint32_t() == NONE;
           }).size() != 0) {
        ...
        if (type == HINT_TYPE_WOTH) {
            ...
            std::vector<uint32_t> sometimesHintLocations = FilterFromPool(allLocations, [](const uint32_t loc) {
            ...
            std::vector<uint32_t> songHintLocations = FilterFromPool(allLocations, [](const uint32_t loc) {
            ...
            std::vector<uint32_t> overworldHintLocations = FilterFromPool(allLocations, [](const uint32_t loc) {
            ...
            std::vector<uint32_t> dungeonHintLocations = FilterFromPool(allLocations, [](const uint32_t loc) {
    ...
    for (uint32_t gossipStone : FilterFromPool(
             gossipStoneLocations, [](const uint32_t loc) { return Location(loc)->GetPlaceduint32_t() == NONE; })) {
        const HintText junkHint = RandomElement(GetHintCategory(HintCategory::Junk));
