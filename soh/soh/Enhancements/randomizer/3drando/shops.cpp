...
std::vector<uint32_t> GetMinVanillaShopItems(int total_replaced) {
    std::vector<uint32_t> minShopItems = {
        ...
        BUY_ARROWS_30, BUY_ARROWS_50, BUY_BOMBCHU_10, BUY_BOMBCHU_10, BUY_BOMBCHU_10, BUY_BOMBCHU_20, BUY_BOMBS_525,
...
int GetPriceFromMax(int max) {
    return Random(1, max) *
           5; // random range of 1 - wallet max / 5, where wallet max is the highest it goes as a multiple of 5
}

// Get random price out of available "affordable prices", or just return 10 if Starter wallet is selected (no need to
// randomly select from a single element)
...
int GetRandomShopPrice() {
    ...
    if (Settings::ShopsanityPricesAffordable.Is(true) &&
        Settings::ShopsanityPrices.IsNot(RO_SHOPSANITY_PRICE_BALANCED)) {
...
void InitTrickNames() {
    trickNameTable[GI_SWORD_KOKIRI] = { Text{ "Korok Sword", "Épée Korok", "Espada Korok" },
    ...
    trickNameTable[0xB8] = { Text{ "Diamond Hearts", "Coeurs de Diamant", "Contenedor de diamante" },
                             Text{ "Double Damage", "Double Souffrance", "Doble daño receptivo" },
                             Text{ "Quadruple Defence", "Quadruple Défence", "Defensa cuádruple" } };

    trickNameTable[GI_POCKET_EGG] = { Text{ "Poached Egg", "oeuf à la coque", "Huevo pasado" },
                                      Text{ "Lon Lon Egg", "oeuf Lon Lon", "Huevo Lon Lon" },
                                      Text{ "Zora Egg", "oeuf Zora", "Huevo del Pez Viento" } };
    trickNameTable[GI_POCKET_CUCCO] = { Text{ "D.I.Y. Alarm Clock", "Réveille-matin improvisé",
                                              "Alarma emplumada portátil" },
                                        Text{ "Kakariko Cucco", "Cocotte Cocorico", "Cuco de Kakariko" },
                                        Text{ "Hatched Cucco", "Cocotte éclose", "Pollo de bolsillo" } };
    trickNameTable[GI_COJIRO] = { Text{ "Blucco", "Chair-Qui-Poule", "Cucazul" },
                                  Text{ "Grog's Cucco", "Cocotte de Grog", "Cuco de Grog" },
                                  Text{ "Corijo", "Cojiro", "Corijo" } };
    trickNameTable[GI_ODD_MUSHROOM] = { Text{ "Magic Mushroom", "Champignon magique", "Champiñón mágico" },
                                        Text{ "Endura Shroom", "Champi Vigueur", "Champiñón del bosque" },
                                        Text{ "Mushroom", "Champignon", "Seta" } };
    trickNameTable[GI_ODD_POTION] = { Text{ "Odd Medicine", "Élixir suspect", "Poción rara" },
    ...
    trickNameTable[GI_SAW] = { Text{ "Carpenter's Saw", "Scie du charpentier", "Sierra del carpintero" },
    ...
    trickNameTable[GI_PRESCRIPTION] = { Text{ "Biggoron's Prescription", "Ordonnance de Grogoron",
                                              "Receta de Biggoron" },
                                        Text{ "Eyedrop Prescription", "Ordonnance de gouttes", "Receta ocular" },
                                        Text{ "Urgent Prescription", "Ordonnance urgente", "Prescripción" } };
    trickNameTable[GI_FROG] = { Text{ "Don Gero", "Don Gero", "Don Gero" },
    ...
    trickNameTable[GI_CLAIM_CHECK] = { Text{ "Clay Check", "Certificat Grogoron", "Comprobante de Reclamación" },
    ...
    trickNameTable[0x81] = { Text{ "Progressive Glove", "Gant de puissance (prog.)", "Guanteletes progresivos" },
                             Text{ "Progressive Power Bracelet", "Bracelet de force (prog.)",
                                   "Brasaletes progresivos" },
    ...
    trickNameTable[0x0F] = { Text{ "Magic Bottle", "Flacon magique", "Frasco feérico" },
                             Text{ "Glass Bottle", "Flacon de verre", "Botella de cristal" },
                             Text{ "Bottle with Water", "Flacon d'eau", "Botella Tingle" } };
    trickNameTable[0x14] = { Text{ "Bottle with Chateau Romani", "Flacon de cuvée Romani",
                                   "Botella de Reserva Romani" },
    ...
    trickNameTable[0x8E] = { Text{ "Bottle with Blue Chu Jelly", "Flacon de gelée Chuchu bleue",
                                   "Jugo de Chuchu Azul" },
    ...
    trickNameTable[0xBB] = { Text{ "Saria's Karaoke", "Karaoké de Saria", "Dueto del bosque" },
                             Text{ "Sonata of Awakening", "Sonate de l'éveil", "Sonata del despertar" },
                             Text{ "Saria's Song", "Chant de Saria", "Canción de Saria" } };
    trickNameTable[0xBC] = { Text{ "Darunia's Tango", "Tango de Darunia", "Coro del fuego" },
                             Text{ "Goron Lullaby", "Berceuse des Gorons", "Nana goron" },
                             Text{ "Zelda's Lullaby", "Berceuse de Zelda", "Nana de Zelda" } };
    trickNameTable[0xBD] = { Text{ "Ruto's Blues", "Blues de Ruto", "Sonata del agua" },
    ...
    trickNameTable[0xCB] = { Text{ "Pendant of Courage", "Pendentif du courage", "Colgante del valor" },
    ...
    trickNameTable[0xCE] = { Text{ "Wind Medallion", "Médaillon du vent", "Medallón del Viento" },
    ...
    trickNameTable[0xD0] = { Text{ "Ice Medallion", "Médaillon de glace", "Medallón Helado" },
    ...
    trickNameTable[0xD3] = { Text{ "Ether Medallion", "Médaillon d'éther", "Medallón de Tesoro" },
    ...
    trickNameTable[GI_HEART] = { Text{ "Love", "Bisou", "Te amo" },
                                 Text{ "Heart Container", "Réceptacle de coeur", "Contenedor de corazón" },
                                 Text{ "Piece of Heart", "Quart de coeur", "Pieza de corazón" } };
    trickNameTable[GI_RUPEE_GREEN] = { Text{ "Green Rupy", "Rupee vert", "Rubia verde" },
                                       Text{ "One Rupee", "Un rubis", "Guaraní hyliano" },
                                       Text{ "Rupee (1)", "Rubis (1)", "Peso hyliano" } };
    trickNameTable[GI_RUPEE_BLUE] = { Text{ "Blue Rupy", "Rupee bleu", "Rubia azul" },
                                      Text{ "Five Rupees", "Cinq rubis", "Bolívar hyliano" },
                                      Text{ "Rupee (5)", "Rubis (5)", "Peso hyliano" } };
    trickNameTable[GI_RUPEE_RED] = { Text{ "Red Rupy", "Rupee rouge", "Rubia roja" },
                                     Text{ "Twenty Rupees", "Vingt rubis", "Colon hyliano" },
                                     Text{ "Rupee (20)", "Rubis (20)", "Peso hyliano" } };
    trickNameTable[GI_RUPEE_PURPLE] = { Text{ "Purple Rupy", "Rupee pourpre", "Rubia morada" },
                                        Text{ "Fifty Rupees", "Cinquante rubis", "Balboa hyliano" },
                                        Text{ "Rupee (50)", "Rubis (50)", "Peso hyliano" } };
    trickNameTable[GI_RUPEE_GOLD] = { Text{ "Huge Rupy", "Énorme Rupee", "Rubia gigante" },
                                      Text{ "Two Hundred Rupees", "Deux cent rubis", "Euro hyliano" },
                                      Text{ "Rupee (200)", "Rubis (200)", "Dólar hyliano" } };
    trickNameTable[GI_HEART_PIECE] = { Text{ "Piece of Health", "Quart d'énergie", "Pieza de amor" },
                                       Text{ "Recovery Heart", "Coeur d'énergie", "Corazón" },
                                       Text{ "Heart Container", "Réceptacle de coeur", "Contenedor de corazón" } };
    trickNameTable[GI_HEART_CONTAINER_2] = { Text{ "Health Container", "Réceptacle d'énergie", "Contenedor de amor" },
