...
void TimeSplitsPopUpContext() {
    if ((popupID != -1) && ImGui::BeginPopup("TimeSplitsPopUp")) {
        if (popupID == ITEM_SKULL_TOKEN) {
            ...
            ImGui::ImageButton(
                "QUEST_SKULL_TOKEN",
                std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->GetTextureByName("QUEST_SKULL_TOKEN"),
            ...
            for (auto item : popupList[popupID]) {
                ...
                auto ret = ImGui::ImageButton(
                    ...
                    std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->GetTextureByName(popupObject.splitImage),
    ...
    if (ImGui::BeginTable("Splits", 5, ImGuiTableFlags_Hideable | ImGuiTableFlags_Reorderable)) {
        ...
        for (auto& split : splitList) {
            ...
            auto ret = ImGui::ImageButton(
                ...
                std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->GetTextureByName(split.splitImage), imageSize,
        ...
        if (ImGui::BeginTabItem("Preview")) {
            ...
            for (auto& data : splitList) {
                ...
                auto ret = ImGui::ImageButton(
                    ...
                    std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->GetTextureByName(data.splitImage), imageSize,
...
void TimeSplitWindow::InitElement() {
    ...
    std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture("SPECIAL_TRIFORCE_PIECE_WHITE",
                                                                        gWTriforcePieceTex, ImVec4(1, 1, 1, 1));
    std::dynamic_pointer_cast<Fast::Fast3dGui>(Ship::Context::GetInstance()->GetWindow()->GetGui())->LoadGuiTexture("SPECIAL_SPLIT_ENTRANCE", gSplitEntranceTex,
                                                                        ImVec4(1, 1, 1, 1));
