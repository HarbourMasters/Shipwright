...
extern "C" void ValueViewer_Draw(GfxPrint* printer) {
    for (size_t i = 0; i < valueTable.size(); i++) {
        ...
        if (!element.isActive || !element.isPrinted || (gPlayState == NULL && element.requiresPlayState))
            continue;
        GfxPrint_SetColor(printer, static_cast<u32>(element.color.x * 255), static_cast<u32>(element.color.y * 255),
                          static_cast<u32>(element.color.z * 255), static_cast<u32>(element.color.w * 255));
