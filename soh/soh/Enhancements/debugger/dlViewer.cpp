...
void PerformDisplayListSearch() {
    auto result = ShipDK::Context::GetInstance()->GetResourceManager()->GetArchiveManager()->ListFiles("*" + std::string(searchString) + "*DL*");
...
void DLViewerWindow::DrawElement() {
    ...
    try {
        auto res = std::static_pointer_cast<LUS::DisplayList>(ShipDK::Context::GetInstance()->GetResourceManager()->LoadResource(activeDisplayList));
