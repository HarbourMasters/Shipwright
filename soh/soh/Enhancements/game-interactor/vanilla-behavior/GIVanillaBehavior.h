...
typedef enum {
  ...
  // &&
  // (gEntranceTable[((void)0, gSaveContext.entranceIndex)].scene ==
  ...
  // #### `result`
  // ```c
  // (this->cylinder1.base.ac != NULL) && (this->cylinder1.base.ac->id ==
  // ACTOR_EN_ICE_HONO)
  // ```
  // #### `args`
  // - `*BgIceShelter`
  ...
  // ((this->actor.params == DNS_TYPE_DEKU_NUT_UPGRADE) &&
  ...
  // #### `result`
  // ```c
  // gSaveContext.dayTime <= 0xC000 || gSaveContext.dayTime >= 0xE000 ||
  // LINK_IS_ADULT || play->sceneNum !=
  // SCENE_GRAVEYARD
  // ```
  // #### `args`
  // - `*EnTk`
  ...
  // #### `result`
  // ```c
  // (actorCategory == ACTORCAT_ENEMY) && CHECK_FLAG_ALL(actor->flags,
  // ACTOR_FLAG_ATTENTION_ENABLED |
  // ACTOR_FLAG_HOSTILE) && (actor->xyzDistToPlayerSq < SQ(500.0f)) &&
  ...
  // #### `result`
  // True if the next text position must be beyond the current position; false
  // otherwise
  // #### `args`
  ...
  //
  // ### Note:
  // When overriding this, ensure you're not in the intro cutscene as Mido's
  ...
  // - `int32_t` (entranceCutscene->entrance) (promoted from `uint16_t` by
  // va_arg)
  ...
  // #### `args`
  // - `*EnHeishi2`
  // - `bool` (clearCamera - true if the code clears a sub-camera, false
  // otherwise)
  ...
  // #### `result`
  // ##### In `z_demo.c` - `Cutscene_Command_Terminator` (setting
  // `shouldSkipCommand`)
  // ```c
  // true
  // ```
  // ##### In `z_demo.c` - `Cutscene_Command_Terminator` (inside if block and
  // switch)
  // ```c
  ...
  // Fired from Player_OverrideLimbDrawPause (pause/equipment screen character
  // only).
  // #### `args`
  // - `s32` limbIndex
  // - `Gfx**` dList (write to *dList to replace the resolved display list)
  ...
  // #### `result`
  // ```c
  // !((this->dyna.actor.params >> 6) & 1) && (Flags_GetSwitch(play,
  ...
  // #### `result`
  // ```c
  // camera->xzSpeed > 0.001f || <any release button pressed> ||
  // params->interfaceFlags & 0x8
  // ```
  // #### `args`
  // - `Camera*` (`camera`)
  ...
  // ```
  // #### `args`
  // - `*int16_t` - original next entrance index (`play->setupExitList[exitIndex
  // - 1]`)
  ...
  // ```c
  // (sFishingRecordLength >= 50.0f) && !(HIGH_SCORE(HS_FISHING) &
  // HS_FISH_PRIZE_CHILD)
  // ```
  // ##### In `Fishing_HandleOwnerDialog` - `!LINK_AGE_CHILD`
  // ```c
  // (sFishingRecordLength >= 60.0f) && !(HIGH_SCORE(HS_FISHING) &
  // HS_FISH_PRIZE_ADULT)
  // ```
  // #### `args`
  // - `*VBFishingData`
  ...
  // #### `result`
  // ```c
  // (this->stateFlags1 & PLAYER_STATE1_CARRYING_ACTOR) && (this->heldActor !=
  // NULL) &&
  ...
  // #### `result`
  // ```c
  // (this->modelAnimType != PLAYER_ANIMTYPE_3) && (play->shootingGalleryStatus
  // == 0)
  // ```
  // #### `args`
  // - `*Player`
  ...
  // !Flags_GetEventChkInf(EVENTCHKINF_ZELDA_FLED_HYRULE_CASTLE)) || (IS_RANDO
  // && !metZelda))
  ...
  // #### `result`
  // ```c
  // (CVarGetInteger(CVAR_GAMEPLAY_STATS("ShowIngameTimer"), 0) &&
  // gSaveContext.fileNum >= 0 && gSaveContext.fileNum
  // <= 2)
  // ```
  // #### `args`
  // - `*PlayState`
  ...
  // (this->dyna.actor.params >> 5 & 0x7F) == GI_ICE_TRAP && this->actionFunc ==
  // EnBox_Open &&
  ...
  //   (player->meleeWeaponAnimation == 22 || player->meleeWeaponAnimation ==
  //   23)
  // ```
  // #### `args`
  // - `*BgHidanDalm`
  ...
  // letting players escape the cutscene with A/B/start after a normal number of
  // playframes.
  // #### `args`
  // - none
  ...
  // ```
  // Whether the B button slot should be treated as holding an item when
  // entering the
  // horseback/minigame "temporary B" force path. Rando returns `true` for a
  // swordless
  ...
  // `buttonStatus[0] = buttonItems[0]` stash so rando can relocate it to its
  // swordless
  ...
  // ```
  // Whether the "temporary B" item should be restored to the B button. Rando
  // also returns
  ...
  // ```
  // Side-effect hook (return value ignored): fired right after the vanilla
  // `buttonItems[0] = buttonStatus[0]` restore so rando can convert its
  // swordless sentinel
