...
void RegisterSkipIntro() {
    bool shouldRegister = CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Intro"), 0) || IsRando();
    COND_VB_SHOULD(VB_PLAY_TRANSITION_CS, shouldRegister, {
        ...
        if ((CVarGetInteger(CVAR_ENHANCEMENT("TimeSavers.SkipCutscene.Intro"), IsRando()) ||
             (IsRando() && (adultStart || shuffleEntrances))) &&
