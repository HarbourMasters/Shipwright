...
void CrowdControl::Enable() {
    ...
    GameInteractor::Instance->RegisterRemoteDataHandler([&](char payload[512]) {
...
void CrowdControl::HandleRemoteData(char payload[512]) {
...
void CrowdControl::EmitMessage(uint32_t eventId, long timeRemaining, EffectResult status) {
    ...
    GameInteractor::Instance->TransmitDataToRemote(payload.dump().c_str());
}
...
CrowdControl::Effect* CrowdControl::ParseMessage(char payload[512]) {
    ...
    if (!dataReceived.contains("id") || !dataReceived.contains("code")) {
        SPDLOG_ERROR("Invalid CrowdControl message received {}", payload);
        return nullptr;
    }
