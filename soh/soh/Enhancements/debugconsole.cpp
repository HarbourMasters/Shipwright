...
const static std::map<std::string, uint16_t> ammoItems{ { "sticks", ITEM_STICK }, { "nuts", ITEM_NUT },
...
static bool GravityHandler(std::shared_ptr<LUS::Console> Console, const std::vector<std::string>& args,
    ...
    try {
        effect->parameters[0] =
            LUS::Math::clamp(std::stoi(args[1], nullptr, 10), GI_GRAVITY_LEVEL_LIGHT, GI_GRAVITY_LEVEL_HEAVY);
    } catch (std::invalid_argument const& ex) {
...
void DebugConsole_Init(void) {
    ...
    CMD_REGISTER("rupee", { RupeeHandler, "Set your rupee counter.", { { "amount", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("spawn",
                 { ActorSpawnHandler,
                   "Spawn an actor.",
                   { { "actor name/id", LUS::ArgumentType::NUMBER }, // TODO there should be an actor_id arg type
    ...
    CMD_REGISTER("bottle", { BottleHandler,
                             ...
                             { { "item", LUS::ArgumentType::TEXT }, { "slot", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("item", { ItemHandler,
                           ...
                           { { "slot", LUS::ArgumentType::NUMBER }, { "item id", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("giant_link",
                 { GiantLinkHandler, "Turn Link into a giant Lonky boi.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("minish_link",
                 { MinishLinkHandler, "Turn Link into a minish boi.", { { "value", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("gravity", { GravityHandler, "Set gravity level.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("no_ui", { NoUIHandler, "Disables the UI.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("freeze", { FreezeHandler, "Freezes Link in place" });

    CMD_REGISTER("defense_modifier",
                 { DefenseModifierHandler, "Sets the defense modifier.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("damage", { DamageHandler, "Deal damage to Link.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("heal", { HealHandler, "Heals Link.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("fill_magic", { FillMagicHandler, "Fills magic." });

    CMD_REGISTER("empty_magic", { EmptyMagicHandler, "Empties magic." });

    CMD_REGISTER("no_z", { NoZHandler, "Disables Z-button presses.", { { "value", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("paper_link",
                 { PaperLinkHandler, "Link but made out of paper.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("rainstorm", { RainstormHandler, "Activates rainstorm." });

    CMD_REGISTER("reverse_controls",
                 { ReverseControlsHandler, "Reverses the controls.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("update_rupees", { UpdateRupeesHandler, "Adds rupees.", { { "value", LUS::ArgumentType::NUMBER } } });

    CMD_REGISTER("speed_modifier",
                 { SpeedModifierHandler, "Sets the speed modifier.", { { "value", LUS::ArgumentType::NUMBER } } });
    ...
    CMD_REGISTER("knockback", { KnockbackHandler, "Knocks Link back.", { { "value", LUS::ArgumentType::NUMBER } } });
