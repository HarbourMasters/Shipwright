...
void sprite_load(sprite_t* sprite) {
    ...
    if (sprite->im_siz == G_IM_SIZ_16b) {
        gDPLoadTextureBlock(OVERLAY_DISP++, sprite->tex, sprite->im_fmt,
...
void sprite_draw(sprite_t* sprite, int left, int top, int width, int height) {
    ...
    gSPWideTextureRectangle(OVERLAY_DISP++, OTRGetRectDimensionFromRightEdge(left) << 2, top << 2,
                            (OTRGetRectDimensionFromRightEdge(left) + width) << 2, (top + height) << 2, G_TX_RENDERTILE,
