...
#define COSMETIC_OPTION(id, label, group, defaultColor, supportsAlpha, supportsRainbow, advancedOption)                \
    {                                                                                                                  \
        id, {                                                                                                          \
            "gCosmetics." id ".Value", "gCosmetics." id ".Rainbow", "gCosmetics." id ".Locked",                        \
                "gCosmetics." id ".Changed", label, group, defaultColor, defaultColor, supportsAlpha, supportsRainbow, \
                advancedOption                                                                                         \
        }                                                                                                              \
    }
...
/*
    ...
    To preface this, if you have any questions or concerns ping @ProxySaw on discord, if I'm no longer available ask
   around in the #development-chat

    # Silly Options
    Lets get this one out of the way, probably the only thing that will be consistent between silly options is how they
   are rendered on the ImGui tab. So when adding one just make sure it follows the same general pattern as the rest.
   Notably:
    ...
    # Finding your color
    So the first order of business is finding the source of the color you are trying to change. There are four scenarios
   to be aware of, in order of difficulty from easiest to hardest:
    1. Color is in code
    ...
    I would recommend first finding the draw function for whatever you are looking for. In most cases this will be an
   actor, and the actor's draw func will be at the bottom of their overlay file, `EnCow_Draw` for ACTOR_EN_COW is one
   example. There can also be additional nested draw methods like drawing each limb of an actor for instance that you
   will also want to inspect. What you are looking for is any sort of RGB values, or calls directly to
   gDPSetPrimColor/gDPSetEnvColor in code. If you find one, try changing the arguments and see if that's what you are
   looking for.

    If this fails, and you aren't able to find any colors within the source of the actor/whatever you will now need to
   investigate the DLists that are being rendered. The easiest way to do this is to use the experimental Display List
   Viewer in the developer tools options. An alternative to this is to dig through the source of the DLists after you
   have built the zeldaret/oot repository, but this will be much more manual, and I can't provide instructions for it.

    Assuming you are planning on using the Display List Viewer, you need to find the name of the DList to inspect. In
   the same areas you were looking for RGB values you now want to look for calls to gSPDisplayList, or variables that
   end in "DL". Once you have this name start typing parts of it into the dlist-viewer (in the developer dropdown) and
   select the desired dlist in the dropdown, there may be many. You will now see a list of commands associated with the
   DList you have selected. If you are lucky, there will be calls to gsDPSetPrimColor/gsDPSetEnvColor with the RGB
   values editable, and you can edit those to determine if that is the DList you are looking for. If it is, make note of
   the name and the index of the DList command you just edited, as you will need that going forward.

    If you are unlucky, this means it is very likely the color you want to change is embedded in a TLUT or the texture
   itself. We can work around this by using grayscale coloring, but this is advanced and I won't be providing a
   walkthrough for it here, you'll just have to read through the existing cosmetic options to get an understanding of
   how to do this.

    # Add your option to the editor
    This step should be as simple as adding a single line in the map below, ensure you add it to the appropriate place
   and with the default colors

    # Applying your color
    If you have determined your color is in code, this should just be as simple as replacing it, or the call it's used
   in if and only if it has been changed. Example from the moon cosmetic option:
    ...
    If instead what you found was that your color was set via a gsDPSetPrimColor command within a DList, you will need
   to follow the pattern displayed in `ApplyOrResetCustomGfxPatches`, using the name of the Dlist, and index of the
   command you want to replace appropriately.

    # Applying variants of the same color
    This applies to all of the different kinds of cosmetics, in some cases you will need to apply multiple shades of the
   same color, otherwise you end up with a weird color between the original color and the new color for example. One
   demonstration on how to handle this is shown above in the moon cosmetic, where for the gDPSetEnvColor color we are
   halving the RGB values, to make them a bit darker similar to how the original colors were darker than the
   gDPSetPrimColor. You will see many more examples of this below in the `ApplyOrResetCustomGfxPatches` method
*/
static std::map<std::string, CosmeticOption> cosmeticOptions = {
    ...
    COSMETIC_OPTION("Swords_KokiriBlade", "Kokiri Sword Blade", GROUP_SWORDS, ImVec4(255, 255, 255, 255), false, true,
                    false),
    // COSMETIC_OPTION("Swords_KokiriHilt",             "Kokiri Sword Hilt",    GROUP_SWORDS,       ImVec4(160, 100, 15,
    // 255), false, true, true), // Todo (Cosmetics): Broken, need a better way to grayscale
    COSMETIC_OPTION("Swords_MasterBlade", "Master Sword Blade", GROUP_SWORDS, ImVec4(255, 255, 255, 255), false, true,
                    false),
    // COSMETIC_OPTION("Swords_MasterHilt",             "Master Sword Hilt",    GROUP_SWORDS,       ImVec4( 80,  80,
    // 168, 255), false, true, true), // Todo (Cosmetics): Broken, need a better way to grayscale
    COSMETIC_OPTION("Swords_BiggoronBlade", "Biggoron Sword Blade", GROUP_SWORDS, ImVec4(255, 255, 255, 255), false,
                    true, false),
    // COSMETIC_OPTION("Swords_BiggoronHilt",           "Biggoron Sword Hilt",  GROUP_SWORDS,       ImVec4( 80,  80,
    // 168, 255), false, true, true), // Todo (Cosmetics): Broken, need a better way to grayscale

    COSMETIC_OPTION("Gloves_GoronBracelet", "Goron Bracelet", GROUP_GLOVES, ImVec4(255, 255, 170, 255), false, true,
                    false),
    COSMETIC_OPTION("Gloves_SilverGauntlets", "Silver Gauntlets", GROUP_GLOVES, ImVec4(255, 255, 255, 255), false, true,
                    false),
    COSMETIC_OPTION("Gloves_GoldenGauntlets", "Golden Gauntlets", GROUP_GLOVES, ImVec4(254, 207, 15, 255), false, true,
                    false),
    COSMETIC_OPTION("Gloves_GauntletsGem", "Gauntlets Gem", GROUP_GLOVES, ImVec4(255, 60, 100, 255), false, true, true),

    COSMETIC_OPTION("Equipment_BoomerangBody", "Boomerang Body", GROUP_EQUIPMENT, ImVec4(160, 100, 0, 255), false, true,
                    false),
    COSMETIC_OPTION("Equipment_BoomerangGem", "Boomerang Gem", GROUP_EQUIPMENT, ImVec4(255, 50, 150, 255), false, true,
                    true),
    // COSMETIC_OPTION("Equipment_SlingshotBody",       "Slingshot Body",       GROUP_EQUIPMENT,    ImVec4(160, 100, 0,
    // 255), false, true, true), // Todo (Cosmetics): Broken, need a better way to grayscale
    COSMETIC_OPTION("Equipment_SlingshotString", "Slingshot String", GROUP_EQUIPMENT, ImVec4(255, 255, 255, 255), false,
                    true, true),
    COSMETIC_OPTION("Equipment_HammerHead", "Hammer Head", GROUP_EQUIPMENT, ImVec4(155, 192, 201, 255), false, true,
                    false),
    COSMETIC_OPTION("Equipment_HammerHandle", "Hammer Handle", GROUP_EQUIPMENT, ImVec4(110, 60, 0, 255), false, true,
                    true),
    // COSMETIC_OPTION("Equipment_HookshotChain",       "Hookshot Chain",       GROUP_EQUIPMENT,    ImVec4(255, 255,
    // 255, 255), false, true, true), // Todo (Cosmetics): Implement COSMETIC_OPTION("Equipment_HookshotReticle",
    // "Hookshot Reticle",     GROUP_EQUIPMENT,    ImVec4(255, 255, 255, 255), false, true, true), // Todo (Cosmetics):
    // Implement COSMETIC_OPTION("Equipment_HookshotTip",         "Hookshot Tip",         GROUP_EQUIPMENT, ImVec4(255,
    // 255, 255, 255), false, true, false), // Todo (Cosmetics): Implement
    COSMETIC_OPTION("Equipment_BowTips", "Bow Tips", GROUP_EQUIPMENT, ImVec4(200, 0, 0, 255), false, true, true),
    COSMETIC_OPTION("Equipment_BowString", "Bow String", GROUP_EQUIPMENT, ImVec4(255, 255, 255, 255), false, true,
                    true),
    ...
    COSMETIC_OPTION("Consumable_HeartBorder", "Heart Border", GROUP_CONSUMABLE, ImVec4(50, 40, 60, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_DDHearts", "DD Hearts", GROUP_CONSUMABLE, ImVec4(200, 0, 0, 255), false, true, false),
    COSMETIC_OPTION("Consumable_DDHeartBorder", "DD Heart Border", GROUP_CONSUMABLE, ImVec4(255, 255, 255, 255), false,
                    true, true),
    COSMETIC_OPTION("Consumable_Magic", "Magic", GROUP_CONSUMABLE, ImVec4(0, 200, 0, 255), false, true, false),
    COSMETIC_OPTION("Consumable_MagicActive", "Magic Active", GROUP_CONSUMABLE, ImVec4(250, 250, 0, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_MagicBorder", "Magic Border", GROUP_CONSUMABLE, ImVec4(255, 255, 255, 255), false,
                    false, true),
    COSMETIC_OPTION("Consumable_MagicBorderActive", "Magic Border Active", GROUP_CONSUMABLE, ImVec4(255, 255, 255, 255),
                    false, false, true),
    COSMETIC_OPTION("Consumable_GreenRupee", "Green Rupee", GROUP_CONSUMABLE, ImVec4(50, 255, 50, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_BlueRupee", "Blue Rupee", GROUP_CONSUMABLE, ImVec4(50, 50, 255, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_RedRupee", "Red Rupee", GROUP_CONSUMABLE, ImVec4(255, 50, 50, 255), false, true, true),
    COSMETIC_OPTION("Consumable_PurpleRupee", "Purple Rupee", GROUP_CONSUMABLE, ImVec4(150, 50, 255, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_GoldRupee", "Gold Rupee", GROUP_CONSUMABLE, ImVec4(255, 190, 55, 255), false, true,
                    true),
    COSMETIC_OPTION("Consumable_SilverRupee", "Silver Rupee", GROUP_CONSUMABLE, ImVec4(255, 255, 255, 255), false, true,
                    true),
    ...
    // COSMETIC_OPTION("Title_FirePrimary",             "Title Fire Primary",   GROUP_TITLE,        ImVec4(255, 255,
    // 170, 255), false, true, false), // Todo (Cosmetics): Kinda complicated COSMETIC_OPTION("Title_FireSecondary",
    // "Title Fire Secondary", GROUP_TITLE,        ImVec4(255, 100,   0, 255), false, true, true), // Todo (Cosmetics):
    // Kinda complicated

    COSMETIC_OPTION("Arrows_NormalPrimary", "Normal Primary", GROUP_ARROWS, ImVec4(0, 150, 0, 0), false, true, false),
    COSMETIC_OPTION("Arrows_NormalSecondary", "Normal Secondary", GROUP_ARROWS, ImVec4(255, 255, 170, 255), false, true,
                    true),
    ...
    COSMETIC_OPTION("Arrows_IceSecondary", "Ice Secondary", GROUP_ARROWS, ImVec4(255, 255, 255, 255), false, true,
                    true),
    COSMETIC_OPTION("Arrows_LightPrimary", "Light Primary", GROUP_ARROWS, ImVec4(255, 255, 0, 255), false, true, false),
    COSMETIC_OPTION("Arrows_LightSecondary", "Light Secondary", GROUP_ARROWS, ImVec4(255, 255, 170, 0), false, true,
                    true),

    // COSMETIC_OPTION("Magic_DinsPrimary",             "Din's Primary",        GROUP_MAGIC,        ImVec4(255, 255,
    // 255, 255), false, true, false), // Todo (Cosmetics): Replace gDF_Col COSMETIC_OPTION("Magic_DinsSecondary",
    // "Din's Secondary",      GROUP_MAGIC,        ImVec4(255, 255, 255, 255), false, true, true), // Todo (Cosmetics):
    // Replace gDF_Env COSMETIC_OPTION("Magic_FaroresPrimary",          "Farore's Primary",     GROUP_MAGIC, ImVec4(255,
    // 255, 255, 255), false, true, false), // Todo (Cosmetics): Implement COSMETIC_OPTION("Magic_FaroresSecondary",
    // "Farore's Secondary",   GROUP_MAGIC,        ImVec4(255, 255, 255, 255), false, true, true), // Todo (Cosmetics):
    // Implement COSMETIC_OPTION("Magic_NayrusPrimary",           "Nayru's Primary",      GROUP_MAGIC, ImVec4(255, 255,
    // 255, 255), false, true, false), // Todo (Cosmetics): Replace gNL_Diamond_Col / gNL_Orb_Col
    // COSMETIC_OPTION("Magic_NayrusSecondary",         "Nayru's Secondary",    GROUP_MAGIC,        ImVec4(255, 255,
    // 255, 255), false, true, true), // Todo (Cosmetics): Replace gNL_Diamond_Env / gNL_Orb_Env

    COSMETIC_OPTION("SpinAttack_Level1Primary", "Level 1 Primary", GROUP_SPIN_ATTACK, ImVec4(170, 255, 255, 255), false,
                    true, true),
    COSMETIC_OPTION("SpinAttack_Level1Secondary", "Level 1 Secondary", GROUP_SPIN_ATTACK, ImVec4(0, 100, 255, 255),
                    false, true, false),
    COSMETIC_OPTION("SpinAttack_Level2Primary", "Level 2 Primary", GROUP_SPIN_ATTACK, ImVec4(255, 255, 170, 255), false,
                    true, true),
    COSMETIC_OPTION("SpinAttack_Level2Secondary", "Level 2 Secondary", GROUP_SPIN_ATTACK, ImVec4(255, 100, 0, 255),
                    false, true, false),
    ...
    COSMETIC_OPTION("Trails_BiggoronSword", "Biggoron Sword", GROUP_TRAILS, ImVec4(255, 255, 255, 255), false, true,
                    true),
    ...
    COSMETIC_OPTION("NPC_FireKeesePrimary", "Fire Keese Primary", GROUP_NPC, ImVec4(255, 255, 255, 255), false, true,
                    false),
    COSMETIC_OPTION("NPC_FireKeeseSecondary", "Fire Keese Secondary", GROUP_NPC, ImVec4(255, 255, 255, 255), false,
                    true, true),
    COSMETIC_OPTION("NPC_IceKeesePrimary", "Ice Keese Primary", GROUP_NPC, ImVec4(255, 255, 255, 255), false, true,
                    false),
    COSMETIC_OPTION("NPC_IceKeeseSecondary", "Ice Keese Secondary", GROUP_NPC, ImVec4(255, 255, 255, 255), false, true,
                    true),
    ...
    COSMETIC_OPTION("NPC_GoldenSkulltula", "Golden Skulltula", GROUP_NPC, ImVec4(255, 255, 255, 255), false, true,
                    false),
...
static const char* MarginCvarList[]{ "gHearts", "gHeartsCount", "gMagicBar", "gVSOA",  "gBBtn", "gABtn",    "gStartBtn",
                                     "gCBtnU",  "gCBtnD",       "gCBtnL",    "gCBtnR", "gDPad", "gMinimap", "gSKC",
                                     "gRC",     "gCarrots",     "gTimers",   "gAS",    "gTCM",  "gTCB",     "gIGT" };
...
void SetMarginAll(const char* ButtonName, bool SetActivated) {
    if (ImGui::Button(ButtonName)) {
        ...
        for (auto cvarName : MarginCvarList) {
            ...
            if (CVarGetInteger(cvarPosType.c_str(), 0) <= 2 && SetActivated) { // Our element is not Hidden or Non
                                                                               // anchor
                for (int i = 0; i < arrayLengthNonMargin; i++) {
                    if ((strcmp(cvarName, MarginCvarNonAnchor[i]) == 0) &&
                        (CVarGetInteger(cvarPosType.c_str(), 0) == 0)) { // Our element is both in original position and
                                                                         // do not have anchor by default so we skip it.
                        CVarSetInteger(cvarNameMargins.c_str(), false);  // force set off
                               ...
                               (CVarGetInteger(cvarPosType.c_str(), 0) !=
                                0)) { // Our element is not in original position regarless it has no anchor by default
                                      // since player made it anchored we can toggle margins
...
void ApplyOrResetCustomGfxPatches(bool manualChange) {
    ...
    if (manualChange || CVarGetInteger(linkGoronTunic.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { linkGoronTunic.defaultColor.x, linkGoronTunic.defaultColor.y,
                                            linkGoronTunic.defaultColor.z, linkGoronTunic.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(linkZoraTunic.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { linkZoraTunic.defaultColor.x, linkZoraTunic.defaultColor.y,
                                            linkZoraTunic.defaultColor.z, linkZoraTunic.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(linkHair.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { linkHair.defaultColor.x, linkHair.defaultColor.y, linkHair.defaultColor.z,
                                            linkHair.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(linkLinen.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { linkLinen.defaultColor.x, linkLinen.defaultColor.y,
                                            linkLinen.defaultColor.z, linkLinen.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(linkBoots.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { linkBoots.defaultColor.x, linkBoots.defaultColor.y,
                                            linkBoots.defaultColor.z, linkBoots.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(mirrorShieldBody.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { mirrorShieldBody.defaultColor.x, mirrorShieldBody.defaultColor.y,
                                            mirrorShieldBody.defaultColor.z, mirrorShieldBody.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(mirrorShieldMirror.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { mirrorShieldMirror.defaultColor.x, mirrorShieldMirror.defaultColor.y,
                                            mirrorShieldMirror.defaultColor.z, mirrorShieldMirror.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(mirrorShieldEmblem.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { mirrorShieldEmblem.defaultColor.x, mirrorShieldEmblem.defaultColor.y,
                                            mirrorShieldEmblem.defaultColor.z, mirrorShieldEmblem.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(swordsKokiriBlade.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { swordsKokiriBlade.defaultColor.x, swordsKokiriBlade.defaultColor.y,
                                            swordsKokiriBlade.defaultColor.z, swordsKokiriBlade.defaultColor.w };
    ...
    //     static Color_RGBA8 defaultColor = {swordsKokiriHilt.defaultColor.x, swordsKokiriHilt.defaultColor.y,
    //     swordsKokiriHilt.defaultColor.z, swordsKokiriHilt.defaultColor.w}; Color_RGBA8 color =
    //     CVarGetColor(swordsKokiriHilt.cvar, defaultColor); PATCH_GFX(gLinkChildLeftFistAndKokiriSwordNearDL,
    //     "Swords_KokiriHilt1",       swordsKokiriHilt.changedCvar,         4, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkChildLeftFistAndKokiriSwordFarDL,          "Swords_KokiriHilt2",
    //     swordsKokiriHilt.changedCvar,         4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkChildSwordAndSheathNearDL,                 "Swords_KokiriHilt3", swordsKokiriHilt.changedCvar,
    //     4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255)); PATCH_GFX(gLinkChildSwordAndSheathFarDL,
    //     "Swords_KokiriHilt4",       swordsKokiriHilt.changedCvar,         4,  gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkChildDekuShieldSwordAndSheathNearDL,       "Swords_KokiriHilt5",
    //     swordsKokiriHilt.changedCvar,         4,  gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathFarDL,        "Swords_KokiriHilt6", swordsKokiriHilt.changedCvar,
    //     4,  gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathNearDL,     "Swords_KokiriHilt7", swordsKokiriHilt.changedCvar,
    //     4,  gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathFarDL,      "Swords_KokiriHilt8", swordsKokiriHilt.changedCvar,
    //     4,  gsDPSetGrayscaleColor(color.r, color.g, color.b, 255)); PATCH_GFX(gGiKokiriSwordDL, "Swords_KokiriHilt9",
    ...
    //     PATCH_GFX(gGiKokiriSwordDL,                               "Swords_KokiriHilt12",
    //     swordsKokiriHilt.changedCvar,       164,  gsDPSetEnvColor(color.r / 3, color.g / 3, color.b / 3, 255));

    //     if (manualChange) {
    //     PATCH_GFX(gLinkChildLeftFistAndKokiriSwordNearDL,         "Swords_KokiriHilt13",
    //     swordsKokiriHilt.changedCvar,       108, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildLeftFistAndKokiriSwordNearDL,         "Swords_KokiriHilt14",
    //     swordsKokiriHilt.changedCvar,       134, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkChildLeftFistAndKokiriSwordFarDL,          "Swords_KokiriHilt15",
    //     swordsKokiriHilt.changedCvar,       106, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildLeftFistAndKokiriSwordFarDL,          "Swords_KokiriHilt16",
    //     swordsKokiriHilt.changedCvar,       126, gsSPGrayscale(false)); PATCH_GFX(gLinkChildSwordAndSheathNearDL,
    //     "Swords_KokiriHilt17",      swordsKokiriHilt.changedCvar,       100, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildSwordAndSheathNearDL,                 "Swords_KokiriHilt18",
    //     swordsKokiriHilt.changedCvar,       126, gsSPGrayscale(false)); PATCH_GFX(gLinkChildSwordAndSheathNearDL,
    //     "Swords_KokiriHilt19",      swordsKokiriHilt.changedCvar,       128, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkChildSwordAndSheathFarDL,                  "Swords_KokiriHilt20",
    ...
    //     PATCH_GFX(gLinkChildSwordAndSheathFarDL,                  "Swords_KokiriHilt22",
    //     swordsKokiriHilt.changedCvar,       120, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathNearDL,       "Swords_KokiriHilt23",
    //     swordsKokiriHilt.changedCvar,       166, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathNearDL,       "Swords_KokiriHilt24",
    //     swordsKokiriHilt.changedCvar,       192, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathNearDL,       "Swords_KokiriHilt25",
    //     swordsKokiriHilt.changedCvar,       194, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathFarDL,        "Swords_KokiriHilt26",
    //     swordsKokiriHilt.changedCvar,       156, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathFarDL,        "Swords_KokiriHilt27",
    //     swordsKokiriHilt.changedCvar,       176, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkChildDekuShieldSwordAndSheathFarDL,        "Swords_KokiriHilt28",
    //     swordsKokiriHilt.changedCvar,       178, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathNearDL,     "Swords_KokiriHilt29",
    //     swordsKokiriHilt.changedCvar,       162, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathNearDL,     "Swords_KokiriHilt30",
    //     swordsKokiriHilt.changedCvar,       188, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathNearDL,     "Swords_KokiriHilt31",
    //     swordsKokiriHilt.changedCvar,       190, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathFarDL,      "Swords_KokiriHilt32",
    //     swordsKokiriHilt.changedCvar,        98, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkChildHylianShieldSwordAndSheathFarDL,      "Swords_KokiriHilt33",
    //     swordsKokiriHilt.changedCvar,       118, gsSPGrayscale(false));
    //     }
    // }
    ...
    if (manualChange || CVarGetInteger(swordsMasterBlade.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { swordsMasterBlade.defaultColor.x, swordsMasterBlade.defaultColor.y,
                                            swordsMasterBlade.defaultColor.z, swordsMasterBlade.defaultColor.w };
        ...
        PATCH_GFX(object_toki_objects_DL_001BD0, "Swords_MasterBlade4", swordsMasterBlade.changedCvar, 14,
                  gsDPSetEnvColor(color.r / 2, color.g / 2, color.b / 2, 255));
        PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8, "Swords_MasterBlade5", swordsMasterBlade.changedCvar, 13,
                  gsDPSetPrimColor(0, 0, color.r, color.g, color.b, 255));
        PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8, "Swords_MasterBlade6", swordsMasterBlade.changedCvar, 14,
                  gsDPSetEnvColor(color.r / 2, color.g / 2, color.b / 2, 255));
    }
    ...
    //     static Color_RGBA8 defaultColor = {swordsMasterHilt.defaultColor.x, swordsMasterHilt.defaultColor.y,
    //     swordsMasterHilt.defaultColor.z, swordsMasterHilt.defaultColor.w}; Color_RGBA8 color =
    //     CVarGetColor(swordsMasterHilt.cvar, defaultColor); PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordNearDL,
    //     "Swords_MasterHilt1",       swordsMasterHilt.changedCvar,        20, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordFarDL,      "Swords_MasterHilt2",
    //     swordsMasterHilt.changedCvar,        20, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(object_toki_objects_DL_001BD0,                  "Swords_MasterHilt3", swordsMasterHilt.changedCvar,
    //     16, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255)); PATCH_GFX(gLinkAdultMasterSwordAndSheathNearDL,
    //     "Swords_MasterHilt4",       swordsMasterHilt.changedCvar,         4, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkAdultMasterSwordAndSheathFarDL,            "Swords_MasterHilt5",
    //     swordsMasterHilt.changedCvar,         4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathNearDL,     "Swords_MasterHilt6", swordsMasterHilt.changedCvar,
    //     4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathFarDL,      "Swords_MasterHilt7", swordsMasterHilt.changedCvar,
    //     4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathNearDL,     "Swords_MasterHilt8", swordsMasterHilt.changedCvar,
    //     4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathFarDL,      "Swords_MasterHilt9", swordsMasterHilt.changedCvar,
    //     4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255)); PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8,
    //     "Swords_MasterHilt10",      swordsMasterHilt.changedCvar,        16, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255));

    //     if (manualChange) {
    //     PATCH_GFX(gLinkAdultMasterSwordAndSheathFarDL,            "Swords_MasterHilt11",
    //     swordsMasterHilt.changedCvar,        38, gsSPGrayscale(true)); PATCH_GFX(gLinkAdultMasterSwordAndSheathFarDL,
    //     "Swords_MasterHilt12",      swordsMasterHilt.changedCvar,        64, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMasterSwordAndSheathFarDL,            "Swords_MasterHilt13",
    //     swordsMasterHilt.changedCvar,       106, gsSPGrayscale(true)); PATCH_GFX(gLinkAdultMasterSwordAndSheathFarDL,
    //     "Swords_MasterHilt14",      swordsMasterHilt.changedCvar,       120, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMasterSwordAndSheathNearDL,           "Swords_MasterHilt15",
    //     swordsMasterHilt.changedCvar,       104, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultMasterSwordAndSheathNearDL,           "Swords_MasterHilt16",
    //     swordsMasterHilt.changedCvar,       182, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMasterSwordAndSheathNearDL,           "Swords_MasterHilt17",
    //     swordsMasterHilt.changedCvar,       184, gsSPEndDisplayList());
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathFarDL,      "Swords_MasterHilt18",
    //     swordsMasterHilt.changedCvar,        80, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathFarDL,      "Swords_MasterHilt19",
    //     swordsMasterHilt.changedCvar,        94, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathFarDL,      "Swords_MasterHilt20",
    //     swordsMasterHilt.changedCvar,       162, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathFarDL,      "Swords_MasterHilt21",
    //     swordsMasterHilt.changedCvar,       180, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathNearDL,     "Swords_MasterHilt22",
    //     swordsMasterHilt.changedCvar,       154, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultHylianShieldSwordAndSheathNearDL,     "Swords_MasterHilt23",
    //     swordsMasterHilt.changedCvar,       232, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathFarDL,      "Swords_MasterHilt24",
    //     swordsMasterHilt.changedCvar,       112, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathFarDL,      "Swords_MasterHilt25",
    //     swordsMasterHilt.changedCvar,       130, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathFarDL,      "Swords_MasterHilt26",
    //     swordsMasterHilt.changedCvar,       172, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathFarDL,      "Swords_MasterHilt27",
    //     swordsMasterHilt.changedCvar,       186, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathNearDL,     "Swords_MasterHilt28",
    //     swordsMasterHilt.changedCvar,       220, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultMirrorShieldSwordAndSheathNearDL,     "Swords_MasterHilt29",
    //     swordsMasterHilt.changedCvar,       298, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordFarDL,      "Swords_MasterHilt30",
    //     swordsMasterHilt.changedCvar,        38, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordFarDL,      "Swords_MasterHilt31",
    //     swordsMasterHilt.changedCvar,       112, gsSPGrayscale(false));
    //     PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordNearDL,     "Swords_MasterHilt32",
    //     swordsMasterHilt.changedCvar,        86, gsSPGrayscale(true));
    //     PATCH_GFX(gLinkAdultLeftHandHoldingMasterSwordNearDL,     "Swords_MasterHilt33",
    //     swordsMasterHilt.changedCvar,       208, gsSPGrayscale(false)); PATCH_GFX(object_toki_objects_DL_001BD0,
    //     "Swords_MasterHilt34",      swordsMasterHilt.changedCvar,       112, gsSPGrayscale(true));
    //     PATCH_GFX(object_toki_objects_DL_001BD0,                  "Swords_MasterHilt35",
    ...
    //     PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8,                      "Swords_MasterHilt37",
    //     swordsMasterHilt.changedCvar,       112, gsSPGrayscale(true)); PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8,
    //     "Swords_MasterHilt38",      swordsMasterHilt.changedCvar,       278, gsSPGrayscale(false));
    //     PATCH_GFX(ovl_Boss_Ganon2_DL_0103A8,                      "Swords_MasterHilt39",
    //     swordsMasterHilt.changedCvar,       280, gsSPEndDisplayList());
    //     }
    // }
    ...
    if (manualChange || CVarGetInteger(swordsBiggoronBlade.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { swordsBiggoronBlade.defaultColor.x, swordsBiggoronBlade.defaultColor.y,
                                            swordsBiggoronBlade.defaultColor.z, swordsBiggoronBlade.defaultColor.w };
    ...
    //     static Color_RGBA8 defaultColor = {swordsBiggoronHilt.defaultColor.x, swordsBiggoronHilt.defaultColor.y,
    //     swordsBiggoronHilt.defaultColor.z, swordsBiggoronHilt.defaultColor.w}; Color_RGBA8 color =
    //     CVarGetColor(swordsBiggoronHilt.cvar, defaultColor); PATCH_GFX(gLinkAdultLeftHandHoldingBgsNearDL,
    //     "Swords_BiggoronHilt1",     swordsBiggoronHilt.changedCvar,      20, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkAdultLeftHandHoldingBgsFarDL,              "Swords_BiggoronHilt2",
    ...
    if (manualChange || CVarGetInteger(glovesGoronBracelet.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { glovesGoronBracelet.defaultColor.x, glovesGoronBracelet.defaultColor.y,
                                            glovesGoronBracelet.defaultColor.z, glovesGoronBracelet.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(glovesSilverGauntlets.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { glovesSilverGauntlets.defaultColor.x, glovesSilverGauntlets.defaultColor.y,
                                            glovesSilverGauntlets.defaultColor.z,
                                            glovesSilverGauntlets.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(glovesGoldenGauntlets.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { glovesGoldenGauntlets.defaultColor.x, glovesGoldenGauntlets.defaultColor.y,
                                            glovesGoldenGauntlets.defaultColor.z,
                                            glovesGoldenGauntlets.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(glovesGauntletsGem.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { glovesGauntletsGem.defaultColor.x, glovesGauntletsGem.defaultColor.y,
                                            glovesGauntletsGem.defaultColor.z, glovesGauntletsGem.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentBoomerangBody.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBoomerangBody.defaultColor.x,
                                            equipmentBoomerangBody.defaultColor.y,
                                            equipmentBoomerangBody.defaultColor.z,
                                            equipmentBoomerangBody.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentBoomerangGem.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBoomerangGem.defaultColor.x, equipmentBoomerangGem.defaultColor.y,
                                            equipmentBoomerangGem.defaultColor.z,
                                            equipmentBoomerangGem.defaultColor.w };
    ...
    //     static Color_RGBA8 defaultColor = {equipmentSlingshotBody.defaultColor.x,
    //     equipmentSlingshotBody.defaultColor.y, equipmentSlingshotBody.defaultColor.z,
    //     equipmentSlingshotBody.defaultColor.w}; Color_RGBA8 color = CVarGetColor(equipmentSlingshotBody.cvar,
    //     defaultColor); PATCH_GFX(gGiSlingshotDL,                                 "Equipment_SlingshotBody1",
    ...
    //     equipmentSlingshotBody.changedCvar,  12, gsDPSetEnvColor(MAX(color.r - 100, 0) / 3, MAX(color.g - 100, 0) /
    //     3, MAX(color.b - 100, 0) / 3, 255)); PATCH_GFX(gGiSlingshotDL, "Equipment_SlingshotBody3",
    ...
    //     equipmentSlingshotBody.changedCvar, 130, gsDPSetEnvColor(MAX(color.r - 100, 0) / 3, MAX(color.g - 100, 0) /
    //     3, MAX(color.b - 100, 0) / 3, 255)); PATCH_GFX(gLinkChildRightArmStretchedSlingshotDL,
    //     "Equipment_SlingshotBody7", equipmentSlingshotBody.changedCvar,   4, gsDPSetGrayscaleColor(color.r, color.g,
    //     color.b, 255)); PATCH_GFX(gLinkChildRightHandHoldingSlingshotNearDL,      "Equipment_SlingshotBody8",
    ...
    if (manualChange || CVarGetInteger(equipmentSlingshotString.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentSlingshotString.defaultColor.x,
                                            equipmentSlingshotString.defaultColor.y,
                                            equipmentSlingshotString.defaultColor.z,
                                            equipmentSlingshotString.defaultColor.w };
        ...
        PATCH_GFX(gGiSlingshotDL, "Equipment_SlingshotString2", equipmentSlingshotString.changedCvar, 76,
                  gsDPSetEnvColor(color.r / 2, color.g / 2, color.b / 2, 255));
        PATCH_GFX(gLinkChildSlinghotStringDL, "Equipment_SlingshotString3", equipmentSlingshotString.changedCvar, 9,
                  gsDPSetPrimColor(0, 0, color.r, color.g, color.b, 255));
    }
    ...
    if (manualChange || CVarGetInteger(equipmentBowTips.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBowTips.defaultColor.x, equipmentBowTips.defaultColor.y,
                                            equipmentBowTips.defaultColor.z, equipmentBowTips.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentBowString.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBowString.defaultColor.x, equipmentBowString.defaultColor.y,
                                            equipmentBowString.defaultColor.z, equipmentBowString.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentBowBody.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBowBody.defaultColor.x, equipmentBowBody.defaultColor.y,
                                            equipmentBowBody.defaultColor.z, equipmentBowBody.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentBowHandle.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentBowHandle.defaultColor.x, equipmentBowHandle.defaultColor.y,
                                            equipmentBowHandle.defaultColor.z, equipmentBowHandle.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentHammerHead.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentHammerHead.defaultColor.x, equipmentHammerHead.defaultColor.y,
                                            equipmentHammerHead.defaultColor.z, equipmentHammerHead.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentHammerHandle.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentHammerHandle.defaultColor.x, equipmentHammerHandle.defaultColor.y,
                                            equipmentHammerHandle.defaultColor.z,
                                            equipmentHammerHandle.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentChuFace.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentChuFace.defaultColor.x, equipmentChuFace.defaultColor.y,
                                            equipmentChuFace.defaultColor.z, equipmentChuFace.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(equipmentChuBody.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { equipmentChuBody.defaultColor.x, equipmentChuBody.defaultColor.y,
                                            equipmentChuBody.defaultColor.z, equipmentChuBody.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumableGreenRupee.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableGreenRupee.defaultColor.x, consumableGreenRupee.defaultColor.y,
                                            consumableGreenRupee.defaultColor.z, consumableGreenRupee.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumableBlueRupee.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableBlueRupee.defaultColor.x, consumableBlueRupee.defaultColor.y,
                                            consumableBlueRupee.defaultColor.z, consumableBlueRupee.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumableRedRupee.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableRedRupee.defaultColor.x, consumableRedRupee.defaultColor.y,
                                            consumableRedRupee.defaultColor.z, consumableRedRupee.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumablePurpleRupee.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumablePurpleRupee.defaultColor.x, consumablePurpleRupee.defaultColor.y,
                                            consumablePurpleRupee.defaultColor.z,
                                            consumablePurpleRupee.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumableGoldRupee.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableGoldRupee.defaultColor.x, consumableGoldRupee.defaultColor.y,
                                            consumableGoldRupee.defaultColor.z, consumableGoldRupee.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(consumableHearts.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableHearts.defaultColor.x, consumableHearts.defaultColor.y,
                                            consumableHearts.defaultColor.z, consumableHearts.defaultColor.w };
        Color_RGBA8        color = CVarGetColor(consumableHearts.cvar, defaultColor);
        // PATCH_GFX(gGiRecoveryHeartDL,                             "Consumable_Hearts1", consumableHearts.changedCvar,
        // 4, gsDPSetGrayscaleColor(color.r, color.g, color.b, 255)); PATCH_GFX(gGiRecoveryHeartDL,
        // "Consumable_Hearts2",       consumableHearts.changedCvar,        26, gsSPGrayscale(true));
        // PATCH_GFX(gGiRecoveryHeartDL,                             "Consumable_Hearts3", consumableHearts.changedCvar,
        // 72, gsSPGrayscale(false)); PATCH_GFX(gGiRecoveryHeartDL,                             "Consumable_Hearts4",
        // consumableHearts.changedCvar,        74, gsSPEndDisplayList());
    ...
    if (manualChange || CVarGetInteger(consumableMagic.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { consumableMagic.defaultColor.x, consumableMagic.defaultColor.y,
                                            consumableMagic.defaultColor.z, consumableMagic.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(npcGoldenSkulltula.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { npcGoldenSkulltula.defaultColor.x, npcGoldenSkulltula.defaultColor.y,
                                            npcGoldenSkulltula.defaultColor.z, npcGoldenSkulltula.defaultColor.w };
        Color_RGBA8        color = CVarGetColor(npcGoldenSkulltula.cvar, defaultColor);
        PATCH_GFX(gGiSkulltulaTokenDL, "NPC_GoldenSkulltula1", npcGoldenSkulltula.changedCvar, 5,
                  gsDPSetPrimColor(0, 0, color.r, color.g, color.b, 255));
        PATCH_GFX(gGiSkulltulaTokenDL, "NPC_GoldenSkulltula2", npcGoldenSkulltula.changedCvar, 6,
                  gsDPSetEnvColor(color.r / 2, color.g / 2, color.b / 2, 255));
        PATCH_GFX(gGiSkulltulaTokenFlameDL, "NPC_GoldenSkulltula3", npcGoldenSkulltula.changedCvar, 32,
                  gsDPSetPrimColor(0, 0, color.r, color.g, color.b, 255));
        PATCH_GFX(gGiSkulltulaTokenFlameDL, "NPC_GoldenSkulltula4", npcGoldenSkulltula.changedCvar, 33,
                  gsDPSetEnvColor(color.r / 2, color.g / 2, color.b / 2, 255));
        PATCH_GFX(object_st_DL_003FB0, "NPC_GoldenSkulltula5", npcGoldenSkulltula.changedCvar, 118,
                  gsDPSetPrimColor(0, 0, color.r, color.g, color.b, 255));
        PATCH_GFX(object_st_DL_003FB0, "NPC_GoldenSkulltula6", npcGoldenSkulltula.changedCvar, 119,
                  gsDPSetEnvColor(color.r / 4, color.g / 4, color.b / 4, 255));
    }

    static CosmeticOption& npcGerudo = cosmeticOptions.at("NPC_Gerudo");
    if (manualChange || CVarGetInteger(npcGerudo.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { npcGerudo.defaultColor.x, npcGerudo.defaultColor.y,
                                            npcGerudo.defaultColor.z, npcGerudo.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(npcMetalTrap.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { npcMetalTrap.defaultColor.x, npcMetalTrap.defaultColor.y,
                                            npcMetalTrap.defaultColor.z, npcMetalTrap.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(n64LogoRed.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { n64LogoRed.defaultColor.x, n64LogoRed.defaultColor.y,
                                            n64LogoRed.defaultColor.z, n64LogoRed.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(n64LogoBlue.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { n64LogoBlue.defaultColor.x, n64LogoBlue.defaultColor.y,
                                            n64LogoBlue.defaultColor.z, n64LogoBlue.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(n64LogoGreen.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { n64LogoGreen.defaultColor.x, n64LogoGreen.defaultColor.y,
                                            n64LogoGreen.defaultColor.z, n64LogoGreen.defaultColor.w };
    ...
    if (manualChange || CVarGetInteger(n64LogoYellow.rainbowCvar, 0)) {
        static Color_RGBA8 defaultColor = { n64LogoYellow.defaultColor.x, n64LogoYellow.defaultColor.y,
                                            n64LogoYellow.defaultColor.z, n64LogoYellow.defaultColor.w };
...
void DrawPositionSlider(const std::string CvarName, int MinY, int MaxY, int MinX, int MaxX) {
    ...
    UIWidgets::EnhancementSliderInt("Left <-> Right : %d", InvisibleLabelX.c_str(), PosXCvar.c_str(), MinX, MaxX, "",
                                    0);
...
void Draw_Placements() {
    if (ImGui::BeginTable("tableMargins", 1, FlagsTable)) {
        ...
        Table_InitHeader();
        UIWidgets::EnhancementSliderInt("Top : %dx", "##UIMARGINT", "gHUDMargin_T",
                                        (ImGui::GetWindowViewport()->Size.y / 2) * -1, 25, "", 0);
        UIWidgets::EnhancementSliderInt("Left: %dx", "##UIMARGINL", "gHUDMargin_L", -25,
                                        ImGui::GetWindowViewport()->Size.x, "", 0);
        UIWidgets::EnhancementSliderInt("Right: %dx", "##UIMARGINR", "gHUDMargin_R",
                                        (ImGui::GetWindowViewport()->Size.x) * -1, 25, "", 0);
        UIWidgets::EnhancementSliderInt("Bottom: %dx", "##UIMARGINB", "gHUDMargin_B",
                                        (ImGui::GetWindowViewport()->Size.y / 2) * -1, 25, "", 0);
    ...
    if (ImGui::CollapsingHeader("Hearts count position")) {
        if (ImGui::BeginTable("tableHeartsCounts", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gHeartsCount", -22, ImGui::GetWindowViewport()->Size.y, -125,
                               ImGui::GetWindowViewport()->Size.x);
            DrawScaleSlider("gHeartsCount", 0.7f);
            UIWidgets::EnhancementSliderInt("Heart line length : %d", "##HeartLineLength", "gHeartsLineLength", 0, 20,
                                            "", 10);
    ...
    if (ImGui::CollapsingHeader("Magic Meter position")) {
        if (ImGui::BeginTable("tablemmpos", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gMagicBar", 0, ImGui::GetWindowViewport()->Size.y / 2, -5,
    ...
    if (ImGui::CollapsingHeader("B Button position")) {
        if (ImGui::BeginTable("tablebbtn", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gBBtn", 0, ImGui::GetWindowViewport()->Size.y / 4 + 50, -1,
                               ImGui::GetWindowViewport()->Size.x - 50);
    ...
    if (ImGui::CollapsingHeader("A Button position")) {
        if (ImGui::BeginTable("tableabtn", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gABtn", -10, ImGui::GetWindowViewport()->Size.y / 4 + 50, -20,
                               ImGui::GetWindowViewport()->Size.x - 50);
    ...
    if (ImGui::CollapsingHeader("Start Button position")) {
        if (ImGui::BeginTable("tablestartbtn", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gStartBtn", 0, ImGui::GetWindowViewport()->Size.y / 2, 0,
                               ImGui::GetWindowViewport()->Size.x / 2 + 70);
    ...
    if (ImGui::CollapsingHeader("Minimaps position")) {
        if (ImGui::BeginTable("tableminimapspos", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gMinimap", (ImGui::GetWindowViewport()->Size.y / 3) * -1,
                               ImGui::GetWindowViewport()->Size.y / 3, ImGui::GetWindowViewport()->Size.x * -1,
                               ImGui::GetWindowViewport()->Size.x / 2);
    ...
    if (ImGui::CollapsingHeader("Small Keys counter position")) {
        if (ImGui::BeginTable("tablesmolekeys", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gSKC", 0, ImGui::GetWindowViewport()->Size.y / 3, -1,
                               ImGui::GetWindowViewport()->Size.x / 2);
    ...
    if (ImGui::CollapsingHeader("Rupee counter position")) {
        if (ImGui::BeginTable("tablerupeecount", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gRC", -2, ImGui::GetWindowViewport()->Size.y / 3, -3,
                               ImGui::GetWindowViewport()->Size.x / 2);
    ...
    if (ImGui::CollapsingHeader("Carrots position")) {
        if (ImGui::BeginTable("tableCarrots", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gCarrots", 0, ImGui::GetWindowViewport()->Size.y / 2, -50,
                               ImGui::GetWindowViewport()->Size.x / 2 + 25);
    ...
    if (ImGui::CollapsingHeader("Timers position")) {
        if (ImGui::BeginTable("tabletimers", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gTimers", 0, ImGui::GetWindowViewport()->Size.y / 2, -50,
                               ImGui::GetWindowViewport()->Size.x / 2 - 50);
    ...
    if (ImGui::CollapsingHeader("Archery Scores position")) {
        if (ImGui::BeginTable("tablearchery", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gAS", 0, ImGui::GetWindowViewport()->Size.y / 2, -50,
                               ImGui::GetWindowViewport()->Size.x / 2 - 50);
    ...
    if (ImGui::CollapsingHeader("Title cards (Maps) position")) {
        if (ImGui::BeginTable("tabletcmaps", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gTCM", 0, ImGui::GetWindowViewport()->Size.y / 2, -50,
    ...
    if (ImGui::CollapsingHeader("Title cards (Bosses) position")) {
        if (ImGui::BeginTable("tabletcbosses", 1, FlagsTable)) {
            ...
            DrawPositionSlider("gTCB", 0, ImGui::GetWindowViewport()->Size.y / 2, -50,
                               ImGui::GetWindowViewport()->Size.x / 2 + 10);
...
void DrawSillyTab() {
    ...
    if (UIWidgets::EnhancementSliderFloat("Link Body Scale: %f", "##Link_BodyScale", "gCosmetics.Link_BodyScale.Value",
                                          0.001f, 0.025f, "", 0.01f, true)) {
    ...
    if (UIWidgets::EnhancementSliderFloat("Link Head Scale: %f", "##Link_HeadScale", "gCosmetics.Link_HeadScale.Value",
                                          0.4f, 4.0f, "", 1.0f, false)) {
    ...
    if (UIWidgets::EnhancementSliderFloat("Link Sword Scale: %f", "##Link_SwordScale",
                                          "gCosmetics.Link_SwordScale.Value", 1.0f, 2.5f, "", 1.0f, false)) {
    ...
    UIWidgets::EnhancementSliderFloat("Bunny Hood Length: %f", "##BunnyHood_EarLength",
                                      "gCosmetics.BunnyHood_EarLength", -300.0f, 1000.0f, "", 0.0f, false);
    ImGui::SameLine();
    if (ImGui::Button("Reset##BunnyHood_EarLength")) {
        CVarClear("gCosmetics.BunnyHood_EarLength");
        LUS::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesOnNextTick();
    }
    UIWidgets::EnhancementSliderFloat("Bunny Hood Spread: %f", "##BunnyHood_EarSpread",
                                      "gCosmetics.BunnyHood_EarSpread", -300.0f, 500.0f, "", 0.0f, false);
    ImGui::SameLine();
    if (ImGui::Button("Reset##BunnyHood_EarSpread")) {
        CVarClear("gCosmetics.BunnyHood_EarSpread");
        LUS::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesOnNextTick();
    }
    UIWidgets::EnhancementSliderFloat("Goron Neck Length: %f", "##Goron_NeckLength", "gCosmetics.Goron_NeckLength",
                                      0.0f, 1000.0f, "", 0.0f, false);
    ImGui::SameLine();
    ...
    UIWidgets::EnhancementSliderFloat("Fairies Size: %f", "##Fairies_Size", "gCosmetics.Fairies_Size", 0.25f, 5.0f, "",
                                      1.0f, false);
    ImGui::SameLine();
    ...
    UIWidgets::EnhancementSliderFloat("N64 Logo Spin Speed: %f", "##N64Logo_SpinSpeed", "gCosmetics.N64Logo_SpinSpeed",
                                      0.25f, 5.0f, "", 1.0f, false);
    ImGui::SameLine();
    if (ImGui::Button("Reset##N64Logo_SpinSpeed")) {
        CVarClear("gCosmetics.N64Logo_SpinSpeed");
        LUS::Context::GetInstance()->GetWindow()->GetGui()->SaveConsoleVariablesOnNextTick();
    }
    UIWidgets::EnhancementSliderFloat("Moon Size: %f", "##Moon_Size", "gCosmetics.Moon_Size", 0.5f, 2.0f, "", 1.0f,
                                      false);
    ImGui::SameLine();
    ...
    if (UIWidgets::EnhancementSliderFloat("Kak Windmill Speed: %f", "##Kak_Windmill_Speed",
                                          "gCosmetics.Kak_Windmill_Speed.Value", 100.0f, 6000.0f, "", 100.0f, false)) {
...
void ResetColor(CosmeticOption& cosmeticOption) {
    Color_RGBA8 defaultColor = { cosmeticOption.defaultColor.x, cosmeticOption.defaultColor.y,
                                 cosmeticOption.defaultColor.z, cosmeticOption.defaultColor.w };
...
void DrawCosmeticRow(CosmeticOption& cosmeticOption) {
    if (ImGui::ColorEdit3(cosmeticOption.label.c_str(), (float*)&cosmeticOption.currentColor,
...
void CosmeticsEditorWindow::DrawElement() {
    ...
    UIWidgets::EnhancementSliderFloat("Rainbow Speed: %f", "##rainbowSpeed", "gCosmetics.RainbowSpeed", 0.03f, 1.0f, "",
                                      0.6f, false);
    ...
    if (ImGui::BeginTabBar("CosmeticsContextTabBar", ImGuiTabBarFlags_NoCloseWithMiddleMouseButton)) {
        ...
        if (ImGui::BeginTabItem("Effects")) {
            ...
            if (UIWidgets::EnhancementSliderInt("Trails Duration: %d", "##Trails_Duration",
                                                "gCosmetics.Trails_Duration.Value", 2, 20, "", 4)) {
