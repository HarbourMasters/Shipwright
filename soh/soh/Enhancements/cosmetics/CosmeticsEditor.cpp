...
namespace {
constexpr const char* CUSTOM_COSMETIC_GROUP = "Custom";
constexpr const char* CUSTOM_CVAR_PREFIX = "gCosmetics.Custom.";
...
std::vector<CustomCosmeticEntry> customCosmeticEntries;

bool IsCustomArchive(const std::shared_ptr<Ship::Archive>& archive) {
    if (archive == nullptr) {
        return false;
    }
...
int GetCustomMaterialSortOrder(const std::string& materialPath) {
...
std::string SanitizeCustomKey(const std::string& value) {
...
Color_RGBA8 GetCustomCosmeticColor(const CustomCosmeticEntry& entry) {
...
std::string GetCustomCosmeticCvar(const CustomCosmeticEntry& entry) {
    return entry.colorCvar.substr(0, entry.colorCvar.size() - std::string(".Value").size());
}

std::string GetCustomCosmeticRainbowCvar(const CustomCosmeticEntry& entry) {
    return GetCustomCosmeticCvar(entry) + ".Rainbow";
}

std::string GetCustomCosmeticLockedCvar(const CustomCosmeticEntry& entry) {
    return GetCustomCosmeticCvar(entry) + ".Locked";
}

void SetCustomCosmeticColor(const CustomCosmeticEntry& entry, Color_RGBA8 color) {
...
void ResetCustomCosmeticColor(const CustomCosmeticEntry& entry) {
...
void RandomizeCustomCosmeticColor(const CustomCosmeticEntry& entry) {
...
void DrawCustomCosmeticRow(const CustomCosmeticEntry& entry) {
    std::string cvar = GetCustomCosmeticCvar(entry);
    std::string rainbowCvar = GetCustomCosmeticRainbowCvar(entry);
    std::string lockedCvar = GetCustomCosmeticLockedCvar(entry);
