...
void SohInputEditorWindow::DrawButtonLineEditMappingButton(uint8_t port, uint16_t bitmask, std::string id) {
    ...
    GetButtonColorsForLUSDeviceIndex(mapping->GetShipDKDeviceIndex(), buttonColor, buttonHoveredColor);
...
#ifndef __WIIU__
    auto sdlAxisDirectionToButtonMapping = std::dynamic_pointer_cast<ShipDK::SDLAxisDirectionToButtonMapping>(mapping);
    auto indexMapping = ShipDK::Context::GetInstance()
                            ->GetControlDeck()
                            ->GetDeviceIndexMappingManager()
                            ->GetDeviceIndexMappingFromShipDKDeviceIndex(mapping->GetShipDKDeviceIndex());
    auto sdlIndexMapping = std::dynamic_pointer_cast<ShipDK::ShipDKDeviceIndexToSDLDeviceIndexMapping>(indexMapping);
...
void SohInputEditorWindow::DrawStickDirectionLineEditMappingButton(uint8_t port, uint8_t stick,
                                                                   ShipDK::Direction direction, std::string id) {
    std::shared_ptr<ShipDK::ControllerAxisDirectionMapping> mapping = nullptr;
    if (stick == ShipDK::LEFT) {
...
void SohInputEditorWindow::DrawStickDirectionLine(const char* axisDirectionName, uint8_t port, uint8_t stick,
                                                  ShipDK::Direction direction, ImVec4 color = CHIP_COLOR_N64_GREY) {
    ImGui::NewLine();
    ImGui::SameLine();
    ImGui::BeginDisabled();
    ImGui::PushStyleColor(ImGuiCol_Button, color);
...
void SohInputEditorWindow::DrawStickSection(uint8_t port, uint8_t stick, int32_t id,
                                            ImVec4 color = CHIP_COLOR_N64_GREY) {
    static int8_t sX, sY;
    std::shared_ptr<ShipDK::ControllerStick> controllerStick = nullptr;
    if (stick == ShipDK::LEFT) {
    ...
    ImGui::SameLine();
    ImGui::BeginGroup();
    DrawStickDirectionLine(ICON_FA_ARROW_UP, port, stick, ShipDK::UP, color);
    DrawStickDirectionLine(ICON_FA_ARROW_DOWN, port, stick, ShipDK::DOWN, color);
    DrawStickDirectionLine(ICON_FA_ARROW_LEFT, port, stick, ShipDK::LEFT, color);
    DrawStickDirectionLine(ICON_FA_ARROW_RIGHT, port, stick, ShipDK::RIGHT, color);
    ImGui::EndGroup();
...
void SohInputEditorWindow::UpdateStickDirectionToMappingIds(uint8_t port) {
    ...
    for (auto stick :
         { std::make_pair<uint8_t, std::shared_ptr<ShipDK::ControllerStick>>(
               ShipDK::LEFT, ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(port)->GetLeftStick()),
           std::make_pair<uint8_t, std::shared_ptr<ShipDK::ControllerStick>>(
               ShipDK::RIGHT,
               ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(port)->GetRightStick()) }) {
        for (auto direction : { ShipDK::LEFT, ShipDK::RIGHT, ShipDK::UP, ShipDK::DOWN }) {
...
void SohInputEditorWindow::DrawRemoveRumbleMappingButton(uint8_t port, std::string id) {
    ...
    if (ImGui::Button(StringHelper::Sprintf("%s###removeRumbleMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
        ...
        ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(port)->GetRumble()->ClearRumbleMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void SohInputEditorWindow::DrawButtonDeviceIcons(uint8_t portIndex, std::set<uint16_t> bitmasks) {
    std::set<ShipDK::ShipDKDeviceIndex> allLusDeviceIndices;
    allLusDeviceIndices.insert(ShipDK::ShipDKDeviceIndex::Keyboard);
    for (auto [lusIndex, mapping] : ShipDK::Context::GetInstance()
                                        ->GetControlDeck()
                                        ->GetDeviceIndexMappingManager()
    ...
    std::vector<std::pair<ShipDK::ShipDKDeviceIndex, bool>> lusDeviceIndiciesWithMappings;
    for (auto lusIndex : allLusDeviceIndices) {
        for (auto [bitmask, button] :
             ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->GetAllButtons()) {
                ...
                for (auto [id, mapping] : button->GetAllButtonMappings()) {
                    if (mapping->GetShipDKDeviceIndex() == lusIndex) {
                        lusDeviceIndiciesWithMappings.push_back(
                            std::pair<ShipDK::ShipDKDeviceIndex, bool>(lusIndex, mapping->PhysicalDeviceIsConnected()));
                        break;
                    }
                }
                break;
    ...
    for (auto [lusIndex, connected] : lusDeviceIndiciesWithMappings) {
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, buttonHoveredColor);
        ImGui::SameLine();
        if (lusIndex == ShipDK::ShipDKDeviceIndex::Keyboard) {
...
void SohInputEditorWindow::DrawAnalogStickDeviceIcons(uint8_t portIndex, ShipDK::Stick stick) {
    std::set<ShipDK::ShipDKDeviceIndex> allLusDeviceIndices;
    allLusDeviceIndices.insert(ShipDK::ShipDKDeviceIndex::Keyboard);
    for (auto [lusIndex, mapping] : ShipDK::Context::GetInstance()
                                        ->GetControlDeck()
    ...
    for (auto lusIndex : allLusDeviceIndices) {
        auto controllerStick =
            stick == ShipDK::Stick::LEFT_STICK
                ? ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->GetLeftStick()
                : ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->GetRightStick();
...
void SohInputEditorWindow::DrawGyroDeviceIcons(uint8_t portIndex) {
    auto mapping =
        ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->GetGyro()->GetGyroMapping();
    if (mapping == nullptr) {
        return;
    }
...
void SohInputEditorWindow::DrawOcarinaControlPanel() {
    ...
    if (CVarGetInteger("gCustomOcarinaControls", 0) == 1) {
        if (ImGui::BeginTable("tableCustomMainOcarinaControls", 2, ImGuiTableFlags_SizingStretchProp)) {
            ...
            ShipDK::GuiWindow::BeginGroupPanel("Notes", ImGui::GetContentRegionAvail());
            ...
            ShipDK::GuiWindow::EndGroupPanel(0);

            TableHelper::NextCol();

            ShipDK::GuiWindow::BeginGroupPanel("Modifiers", ImGui::GetContentRegionAvail());
            ...
            ShipDK::GuiWindow::EndGroupPanel(cursorY - ImGui::GetCursorPosY() + 2);
    ...
    ShipDK::GuiWindow::BeginGroupPanel("Alternate controls", ImGui::GetContentRegionAvail());
...
void SohInputEditorWindow::DrawCameraControlPanel() {
    ImVec2 cursor = ImGui::GetCursorPos();
    ImGui::SetCursorPos(ImVec2(cursor.x + 5, cursor.y + 5));
    ShipDK::GuiWindow::BeginGroupPanel("Aiming/First-Person Camera", ImGui::GetContentRegionAvail());
    ...
    ImGui::SetCursorPos(ImVec2(cursor.x + 5, cursor.y + 5));
    ShipDK::GuiWindow::BeginGroupPanel("Third-Person Camera", ImGui::GetContentRegionAvail());
...
void SohInputEditorWindow::DrawDpadControlPanel() {
    ImVec2 cursor = ImGui::GetCursorPos();
    ImGui::SetCursorPos(ImVec2(cursor.x + 5, cursor.y + 5));
    ShipDK::GuiWindow::BeginGroupPanel("D-Pad Options", ImGui::GetContentRegionAvail());
...
void SohInputEditorWindow::DrawMiscControlPanel() {
    ImVec2 cursor = ImGui::GetCursorPos();
    ImGui::SetCursorPos(ImVec2(cursor.x + 5, cursor.y + 5));
    ShipDK::GuiWindow::BeginGroupPanel("Misc Controls", ImGui::GetContentRegionAvail());
    ...
    UIWidgets::Tooltip("Hold the assigned button to change the maximum walking or swimming speed");
     if (CVarGetInteger("gEnableWalkModify", 0)) {
        UIWidgets::Spacer(5);
        ShipDK::GuiWindow::BeginGroupPanel("Speed Modifier", ImGui::GetContentRegionAvail());
        UIWidgets::PaddedEnhancementCheckbox("Toggle modifier instead of holding", "gWalkSpeedToggle", true, false);
        ShipDK::GuiWindow::BeginGroupPanel("Walk Modifier", ImGui::GetContentRegionAvail());
        ...
        ShipDK::GuiWindow::EndGroupPanel(0);
        ShipDK::GuiWindow::BeginGroupPanel("Swim Modifier", ImGui::GetContentRegionAvail());
...
void SohInputEditorWindow::DrawLinkTab() {
    ...
    if (ImGui::BeginTabItem(StringHelper::Sprintf("Link (P1)###port%d", portIndex).c_str())) {
        ...
        if (ImGui::CollapsingHeader("Analog Stick", NULL, ImGuiTreeNodeFlags_DefaultOpen)) {
            DrawAnalogStickDeviceIcons(portIndex, ShipDK::LEFT_STICK);
            DrawStickSection(portIndex, ShipDK::LEFT, 0);
        } else {
            DrawAnalogStickDeviceIcons(portIndex, ShipDK::LEFT_STICK);
        }

        if (ImGui::CollapsingHeader("Additional (\"Right\") Stick")) {
            DrawAnalogStickDeviceIcons(portIndex, ShipDK::RIGHT_STICK);
            DrawStickSection(portIndex, ShipDK::RIGHT, 1, CHIP_COLOR_N64_YELLOW);
        } else {
            DrawAnalogStickDeviceIcons(portIndex, ShipDK::RIGHT_STICK);
        }
...
void SohInputEditorWindow::DrawSetDefaultsButton(uint8_t portIndex) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        std::map<ShipDK::ShipDKDeviceIndex, std::pair<std::string, int32_t>> indexMappings;
        for (auto [lusIndex, mapping] : ShipDK::Context::GetInstance()
                                            ->GetControlDeck()
                                            ->GetDeviceIndexMappingManager()
                                            ->GetAllDeviceIndexMappings()) {
            auto wiiuIndexMapping = std::static_pointer_cast<ShipDK::ShipDKDeviceIndexToWiiUDeviceIndexMapping>(mapping);
        ...
        for (auto [lusIndex, info] : indexMappings) {
            ...
            if (ImGui::BeginPopupModal(StringHelper::Sprintf("Set Defaults for %s", name.c_str()).c_str(), NULL,
                ...
                if (ImGui::Button("Set defaults")) {
                    ...
                    ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->AddDefaultMappings(
                        lusIndex);
                    shouldClose = true;
                    ImGui::CloseCurrentPopup();
                }
...
void SohInputEditorWindow::DrawSetDefaultsButton(uint8_t portIndex) {
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        for (auto [lusIndex, mapping] : ShipDK::Context::GetInstance()
                                            ...
                                            ->GetDeviceIndexMappingManager()
                                            ->GetAllDeviceIndexMappings()) {
            auto sdlIndexMapping = std::static_pointer_cast<ShipDK::ShipDKDeviceIndexToSDLDeviceIndexMapping>(mapping);
        ...
        if (ImGui::BeginPopupModal("Set Defaults for Keyboard", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::Button("Set defaults")) {
                ShipDK::Context::GetInstance()
                    ->GetControlDeck()
                    ->GetControllerByPort(portIndex)
                    ->ClearAllMappingsForDevice(ShipDK::ShipDKDeviceIndex::Keyboard);
                ShipDK::Context::GetInstance()->GetControlDeck()->GetControllerByPort(portIndex)->AddDefaultMappings(
                    ShipDK::ShipDKDeviceIndex::Keyboard);
                shouldClose = true;
                ImGui::CloseCurrentPopup();
            }
            ImGui::EndPopup();
