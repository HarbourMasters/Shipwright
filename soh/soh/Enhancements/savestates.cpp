...
void SaveStateMgr::SetCurrentSlot(unsigned int slot) {
    ShipDK::Context::GetInstance()->GetWindow()->GetGui()->GetGameOverlay()->TextDrawNotification(1.0f, true, "slot %u set", slot);
    ...
    while (!this->requests.empty()) {
        ...
        switch (request.type) {
            case RequestType::SAVE:
                ...
                ShipDK::Context::GetInstance()->GetWindow()->GetGui()->GetGameOverlay()->TextDrawNotification(1.0f, true, "saved state %u", request.slot);
                break;
            case RequestType::LOAD:
                if (this->states.contains(request.slot)) {
                    this->states[request.slot]->Load();
                    ShipDK::Context::GetInstance()->GetWindow()->GetGui()->GetGameOverlay()->TextDrawNotification(1.0f, true, "loaded state %u", request.slot);
                } else {
...
SaveStateReturn SaveStateMgr::AddRequest(const SaveStateRequest request) {
    if (gPlayState == nullptr) {
        ...
        ShipDK::Context::GetInstance()->GetWindow()->GetGui()->GetGameOverlay()->TextDrawNotification(1.0f, true, "states not available here", request.slot);
    ...
    switch (request.type) { 
        ...
        case RequestType::LOAD:
            if (states.contains(request.slot)) {
                ...
                SPDLOG_ERROR("Invalid SaveState slot: {}", request.type);
                ShipDK::Context::GetInstance()->GetWindow()->GetGui()->GetGameOverlay()->TextDrawNotification(1.0f, true, "state slot %u empty", request.slot);
