...
void func_80A5455C(EnHeishi2* this, PlayState* play) {
    ...
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(play)) {
        ...
        bomb = (EnBom*)Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOM, pos.x, pos.y, pos.z, 0, rotY, 0, 0, false);
        if (bomb != NULL) {
...
void EnHeishi2_Draw(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, EnHeishi2_OverrideLimbDraw, EnHeishi2_PostLimbDraw, this);
