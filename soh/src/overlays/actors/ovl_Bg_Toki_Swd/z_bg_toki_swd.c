...
void func_808BAF40(BgTokiSwd* this, PlayState* play) {
    ...
    if (!LINK_IS_ADULT || (Flags_GetEventChkInf(EVENTCHKINF_LEARNED_PRELUDE_OF_LIGHT) && !IS_RANDO(gSaveContext)) || IS_RANDO(gSaveContext)) {
        if (Actor_HasParent(&this->actor, play)) {
            ...
            if (Actor_IsFacingPlayer(&this->actor, 0x2000) && 
                (!IS_RANDO(gSaveContext) || (IS_RANDO(gSaveContext) && player->getItemId == GI_NONE))) {
