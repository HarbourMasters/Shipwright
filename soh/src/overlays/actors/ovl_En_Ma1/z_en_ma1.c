...
u16 EnMa1_GetText(PlayState* play, Actor* thisx) {
    ...
    if (IS_RANDO(gSaveContext) && play->sceneNum == SCENE_HYRULE_CASTLE) {
...
s32 func_80AA08C4(EnMa1* this, PlayState* play) {
    ...
    if ((this->actor.shape.rot.z == 3) && IS_DAY && (Flags_GetEventChkInf(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE)) && 
        ((IS_RANDO(gSaveContext) && Randomizer_ObtainedMalonHCReward()) || !IS_RANDO(gSaveContext))) {
        return 1;
    }
    return 0;
}
...
void EnMa1_Init(Actor* thisx, PlayState* play) {
    ...
    if (IS_RANDO(gSaveContext)) { // Skip Malon's multiple textboxes before getting an item
   ...
   // 3. We are not Randomized and have obtained Epona's Song
    if (!Flags_GetEventChkInf(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE) || (IS_RANDO(gSaveContext) && !Randomizer_ObtainedMalonHCReward()) || (CHECK_QUEST_ITEM(QUEST_SONG_EPONA) && !IS_RANDO(gSaveContext)) ||
        (IS_RANDO(gSaveContext) && Flags_GetTreasure(play, 0x1F))) {
        this->actionFunc = func_80AA0D88;
        EnMa1_ChangeAnim(this, ENMA1_ANIM_2);
    // If none of the above conditions were true, set Malon up to teach Epona's Song.
    } else {
        if (IS_RANDO(gSaveContext)) { // Skip straight to "let's sing it together" textbox in the ranch
            Flags_SetEventChkInf(EVENTCHKINF_INVITED_TO_SING_WITH_CHILD_MALON);
        }
...
void func_80AA0D88(EnMa1* this, PlayState* play) {
    ...
    // disappear.
    if ((play->sceneNum == SCENE_HYRULE_CASTLE) && (!IS_RANDO(gSaveContext) && Flags_GetEventChkInf(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE))) {
        Actor_Kill(&this->actor);
    ...
    } else if ((!Flags_GetEventChkInf(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE) || (IS_RANDO(gSaveContext) && !Randomizer_ObtainedMalonHCReward())) || (CHECK_QUEST_ITEM(QUEST_SONG_EPONA) && !IS_RANDO(gSaveContext))) {
...
void func_80AA0F44(EnMa1* this, PlayState* play) {
    ...
    if (Flags_GetEventChkInf(EVENTCHKINF_INVITED_TO_SING_WITH_CHILD_MALON)) {
        ...
        if (player->stateFlags2 & 0x1000000) {
            ...
            this->actionFunc = IS_RANDO(gSaveContext) ? func_80AA1150 : func_80AA106C;
        ...
        if (IS_RANDO(gSaveContext) && (Actor_TextboxIsClosing(&this->actor, play) && play->msgCtx.textId == 0x2049) &&
...
void func_80AA1150(EnMa1* this, PlayState* play) {
    ...
    if (IS_RANDO(gSaveContext) && (Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING)) {
