...
void EnSyatekiMan_Init(Actor* thisx, PlayState* play) {
    ...
    if (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_INTERIOR_ENTRANCES)) {
        ...
        if (gallery->actor.update != NULL) {
            ...
            switch (this->gameResult) {
                case SYATEKI_RESULT_WINNER:
                    ...
                    if (!LINK_IS_ADULT) {
                        if(IS_RANDO(gSaveContext) && !Flags_GetTreasure(play, 0x1E)) {
                        ...
                        } else if (!IS_RANDO(gSaveContext) && !Flags_GetItemGetInf(ITEMGETINF_0D)) {
                        ...
                        } else if (!IS_RANDO(gSaveContext) && !Flags_GetItemGetInf(ITEMGETINF_0E)) {
