...
s32 EnTk_ChooseReward(EnTk* this) {
    ...
    if ((IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) && !Flags_GetCollectible(gPlayState, 0x1F) && this->heartPieceSpawned == 0) {
        return 3;
    }
...
void EnTk_Dig(EnTk* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame == 32.0f) {
        ...
        if (this->validDigHere == 1 || IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) {
            rewardOrigin.x = 0.0f;
            rewardOrigin.y = 0.0f;
            ...
            if (IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) {
                if (this->currentReward == 3) {
                    /*
                    * Upgrade the purple rupee reward to the heart piece if this
                    * is the first grand prize dig.
                    */
                    if (!Flags_GetItemGetInf(ITEMGETINF_1C) && !(IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0))) {
                        Flags_SetItemGetInf(ITEMGETINF_1C);
                        this->currentReward = 4;
                    } else if ((IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) && !Flags_GetCollectible(gPlayState, 0x1F) && this->heartPieceSpawned == 0) {
                        this->currentReward = 4;
                    }
                }

                if ((IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) && this->currentReward == 4) {
    ...
    if (this->skelAnime.curFrame >= 32.0f && this->rewardTimer == 10) {
        ...
        if (!(IS_RANDO(gSaveContext) || CVarGetInteger("gDampeWin", 0)) && this->validDigHere == 0) {
            /* Bad dig spot */
