...
void func_809FE890(EnDu* this, PlayState* play) {
    ...
    if (play->csCtx.state == CS_STATE_IDLE || IS_RANDO(gSaveContext)) {
        if (IS_RANDO(gSaveContext)) {
...
void func_809FEB08(EnDu* this, PlayState* play) {
    ...
    if ((!IS_RANDO(gSaveContext) && CUR_UPG_VALUE(UPG_STRENGTH) <= 0) ||
         (IS_RANDO(gSaveContext) && !Flags_GetTreasure(play, 0x1E))) {
        if (IS_RANDO(gSaveContext)) {
