        ...
        switch (play->msgCtx.choiceIndex) {
            case 0: // yes
                if (gSaveContext.rupees <
                    ((IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MAGIC_BEANS))
                         ? 60
                         : sPrices[BEANS_BOUGHT])) {
                    Message_ContinueTextbox(play, 0x4069); // not enough rupees text
                    return;
                }
                if (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MAGIC_BEANS)) {
...
void EnMs_Sell(EnMs* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        Rupees_ChangeBy((IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MAGIC_BEANS)) ? -60 : -sPrices[BEANS_BOUGHT]);
        this->actor.parent = NULL;
        this->actionFunc =
            (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MAGIC_BEANS)) ? EnMs_Wait : EnMs_TalkAfterPurchase;
    } else {
        if (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MAGIC_BEANS)) {
