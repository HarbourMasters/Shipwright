...
void func_808BA018(BgTokiHikari* this, PlayState* play) {
    ...
    if (LINK_IS_ADULT) {
        ...
        gSPSegment(POLY_XLU_DISP++, 8, Gfx_TexScrollEx(play->state.gfxCtx, 0, play->gameplayFrames % 128, 64, 32, 0, 1));

        gSPSegment(POLY_XLU_DISP++, 9, Gfx_TexScrollEx(play->state.gfxCtx, 0, play->gameplayFrames % 128, 64, 32, 0, 1));
