...
void EnTp_Init(Actor* thisx, PlayState* play2) {
    ...
    if (this->actor.params <= TAILPASARAN_HEAD) {
        ...
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_UPDATE_WHILE_CULLED;
        ...
        for (i = 0; i <= 6; i++) {
            ...
            if (next != NULL) {
                ...
                if (i == 2) {
                    next->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE | ACTOR_FLAG_UPDATE_WHILE_CULLED;
                    next->unk_150 = 1; // Why?
                }
