...
s32 func_80B4F45C(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx,
    ...
    if (limbIndex == 14) {
        ...
        Matrix_ToMtx(&sp74[0], __FILE__, __LINE__);
        Matrix_Get(&sp34);
        Matrix_MtxFToYXZRotS(&sp34, &sp2C, 0);
        if (!FrameAdvance_IsEnabled(globalCtx)) {
        ...
        Matrix_ToMtx(&sp74[1], __FILE__, __LINE__);
        Matrix_Get(&sp34);
        Matrix_MtxFToYXZRotS(&sp34, &sp2C, 0);
        if (!FrameAdvance_IsEnabled(globalCtx)) {
        ...
        Matrix_ToMtx(&sp74[2], __FILE__, __LINE__);
        Matrix_Pop();
        Matrix_Push();
        ...
        Matrix_ToMtx(&sp74[3], __FILE__, __LINE__);
        Matrix_Get(&sp34);
        Matrix_MtxFToYXZRotS(&sp34, &sp2C, 0);
        if (!FrameAdvance_IsEnabled(globalCtx)) {
        ...
        Matrix_ToMtx(&sp74[4], __FILE__, __LINE__);
        Matrix_Pop();
        Matrix_Push();
        Matrix_Translate(467.0f, 265.0f, -389.0f, MTXMODE_APPLY);
        Matrix_Get(&sp34);
        ...
        Matrix_Translate(-427.0f, -1.0f, 3.0f, MTXMODE_APPLY);
        Matrix_ToMtx(&sp74[5], __FILE__, __LINE__);
        Matrix_Get(&sp34);
        Matrix_MtxFToYXZRotS(&sp34, &sp2C, 0);
        if (!FrameAdvance_IsEnabled(globalCtx)) {
        ...
        Matrix_Translate(-446.0f, -52.0f, -84.0f, MTXMODE_APPLY);
        Matrix_ToMtx(&sp74[6], __FILE__, __LINE__);
        Matrix_Pop();
        Matrix_Pop();
        this->unk_24C = 1;
    }
