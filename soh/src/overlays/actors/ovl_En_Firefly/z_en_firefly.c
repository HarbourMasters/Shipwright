...
void EnFirefly_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    ...
    Color_RGBA8 customFireAuraPrimColor = CVarGetColor(COSMETIC_CVAR("NPC_FireKeesePrimary.Value"), fireAuraPrimColor);
    Color_RGBA8 customFireAuraEnvColor = CVarGetColor(COSMETIC_CVAR("NPC_FireKeeseSecondary.Value"), fireAuraEnvColor);
    Color_RGBA8 customIceAuraPrimColor = CVarGetColor(COSMETIC_CVAR("NPC_IceKeesePrimary.Value"), iceAuraPrimColor);
    Color_RGBA8 customIceAuraEnvColor = CVarGetColor(COSMETIC_CVAR("NPC_IceKeeseSecondary.Value"), iceAuraEnvColor);
        ...
        if ((this->auraType == KEESE_AURA_FIRE) || (this->auraType == KEESE_AURA_ICE)) {
            if ((limbIndex == 15) || (limbIndex == 21)) {
                ...
                if (this->auraType == KEESE_AURA_FIRE) {
                    if (CVarGetInteger(COSMETIC_CVAR("NPC_FireKeesePrimary.Changed"), 0)) {
                        effPrimColor = &customFireAuraPrimColor;
                    } else {
                        effPrimColor = &fireAuraPrimColor;
                    }
                    if (CVarGetInteger(COSMETIC_CVAR("NPC_FireKeeseSecondary.Changed"), 0)) {
                        effEnvColor = &customFireAuraEnvColor;
                    } else {
                        effEnvColor = &fireAuraEnvColor;
                    }
                } else {
                    if (CVarGetInteger(COSMETIC_CVAR("NPC_IceKeesePrimary.Changed"), 0)) {
                        effPrimColor = &customIceAuraPrimColor;
                    } else {
                        effPrimColor = &iceAuraPrimColor;
                    }
                    if (CVarGetInteger(COSMETIC_CVAR("NPC_IceKeeseSecondary.Changed"), 0)) {
