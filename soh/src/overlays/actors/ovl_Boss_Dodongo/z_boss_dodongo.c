...
void BossDodongo_Init(Actor* thisx, PlayState* play) {
    ...
    if (Flags_GetClear(play, play->roomCtx.curRoom.num)) { // KD is dead
        ...
        Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_DOOR_WARP1, -890.0f, -1523.76f, -3304.0f, 0, 0, 0,
                           WARP_DUNGEON_CHILD);
        Actor_Spawn(&play->actorCtx, play, ACTOR_BG_BREAKWALL, -890.0f, -1523.76f, -3304.0f, 0, 0, 0, 0x6000, false);
        if (GameInteractor_Should(VB_SPAWN_HEART_CONTAINER, true)) {
            Actor_Spawn(&play->actorCtx, play, ACTOR_ITEM_B_HEART, -690.0f, -1523.76f, -3304.0f, 0, 0, 0, 0, false);
        }
    }
            ...
            if (this->unk_1DA == 820) {
                ...
                if (GameInteractor_Should(VB_SPAWN_HEART_CONTAINER, true)) {
                    Actor_Spawn(
                        ...
                        Math_CosS(this->actor.shape.rot.y) * -50.0f + this->actor.world.pos.z, 0, 0, 0, 0, false);
                }
            }
            if (this->unk_1DA == 600) {
                camera = Play_GetCamera(play, MAIN_CAM);
