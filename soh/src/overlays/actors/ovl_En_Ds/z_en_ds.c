...
void EnDs_BrewOddPotion2(EnDs* this, PlayState* play) {
    if (this->brewTimer > 0) {
        ...
        this->brewTimer = IS_RANDO(gSaveContext) ? 0 : 60;
...
void EnDs_BrewOddPotion1(EnDs* this, PlayState* play) {
    if (this->brewTimer > 0) {
        ...
        this->brewTimer = IS_RANDO(gSaveContext) ? 0 : 20;
    }
...
u8 EnDs_RandoCanGetGrannyItem() {
    return IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_MERCHANTS) != RO_SHUFFLE_MERCHANTS_OFF &&
...
void EnDs_Wait(EnDs* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, play)) {
        if (func_8002F368(play) == EXCH_ITEM_ODD_MUSHROOM) {
            ...
            (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_SHUFFLE_ADULT_TRADE) == RO_GENERIC_OFF) ||
