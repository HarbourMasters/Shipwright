...
void EnGe1_Init(Actor* thisx, PlayState* play) {
    ...
    if (IS_RANDO(gSaveContext) && (Randomizer_GetSettingValue(RSK_SHUFFLE_GERUDO_MEMBERSHIP_CARD) ||
    ...
    switch (this->actor.params & 0xFF) {
        ...
        case GE1_TYPE_TRAINING_GROUNDS_GUARD:
            ...
            if (EnGe1_CheckCarpentersFreed()) {
                ...
                if (IS_RANDO(gSaveContext) && gSaveContext.sceneFlags[93].swch & 0x00000004) {
                    this->actionFunc = EnGe1_SetNormalText;
                } else {
...
void EnGe1_WaitTillItemGiven_Archery(EnGe1* this, PlayState* play) {
    ...
    if (Actor_HasParent(&this->actor, play)) {
        if (IS_RANDO(gSaveContext) && gSaveContext.minigameScore >= 1500 && !Flags_GetInfTable(INFTABLE_190)) {
