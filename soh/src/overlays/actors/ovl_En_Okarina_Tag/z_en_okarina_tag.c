...
void func_80ABF4C8(EnOkarinaTag* this, PlayState* play) {
    ...
    if (play->msgCtx.ocarinaMode == OCARINA_MODE_04) {
        ...
        if (!IS_RANDO(gSaveContext) || (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_DOOR_OF_TIME) != RO_DOOROFTIME_CLOSED)) {
            func_80078884(NA_SE_SY_CORRECT_CHIME);
        }
        if (this->switchFlag >= 0) {
            Flags_SetSwitch(play, this->switchFlag);
...
void func_80ABF708(EnOkarinaTag* this, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(&this->actor, play)) {
        ...
        if (!(this->actor.xzDistToPlayer > 120.0f)) {
            if (CHECK_QUEST_ITEM(QUEST_SONG_SUN) || IS_RANDO(gSaveContext)) {
                this->actor.textId = 0x5021;
            }
...
void func_80ABF7CC(EnOkarinaTag* this, PlayState* play) {
    ...
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && Message_ShouldAdvance(play)) {
        ...
        if (!IS_RANDO(gSaveContext) && !CHECK_QUEST_ITEM(QUEST_SONG_SUN)) {
            play->csCtx.segment = SEGMENTED_TO_VIRTUAL(&gSunSongGraveSunSongTeachCs);
            gSaveContext.cutsceneTrigger = 1;
        } else if (IS_RANDO(gSaveContext) && !Flags_GetTreasure(play, 0x1F)) {
