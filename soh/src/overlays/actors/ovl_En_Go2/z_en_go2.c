...
u16 EnGo2_GetTextIdGoronCityRollingBig(PlayState* play, EnGo2* this) {
    if (Flags_GetInfTable(INFTABLE_11E)) {
        return 0x3013;
    } else if ((CUR_CAPACITY(UPG_BOMB_BAG) >= 20 || IS_RANDO(gSaveContext)) && this->waypoint > 7 && this->waypoint < 12) {
        return 0x3012;
    } else {
        return 0x3011;
    }
...
u16 EnGo2_GetTextIdGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (((!IS_RANDO(gSaveContext) && CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) ||
         (IS_RANDO(gSaveContext) && Flags_GetRandomizerInf(RAND_INF_DUNGEONS_DONE_FIRE_TEMPLE))) && LINK_IS_ADULT) {
        return 0x3043;
    } else if ((!IS_RANDO(gSaveContext) && CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) ||
               (IS_RANDO(gSaveContext) && Flags_GetRandomizerInf(RAND_INF_DUNGEONS_DONE_DODONGOS_CAVERN))) {
        return 0x3027;
    } else {
...
u16 EnGo2_GetTextIdGoronDmtBiggoron(PlayState* play, EnGo2* this) {
    ...
    if (!IS_RANDO(gSaveContext) && gSaveContext.bgsFlag) {
    ...
    switch (EnGo2_GetDialogState(this, play)) {
        case TEXT_STATE_DONE:
            if (this->actor.textId == 0x305E) {
                if((!IS_RANDO(gSaveContext) && gSaveContext.bgsFlag) || (IS_RANDO(gSaveContext) && Flags_GetTreasure(play, 0x1F))) {
                    return NPC_TALK_STATE_IDLE;
                }
...
void EnGo2_BiggoronSetTextId(EnGo2* this, PlayState* play, Player* player) {
    ...
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        if ((!IS_RANDO(gSaveContext) && gSaveContext.bgsFlag)) {
            ...
            if (func_8002F368(play) == EXCH_ITEM_CLAIM_CHECK) {
                if (IS_RANDO(gSaveContext) && Flags_GetTreasure(play, 0x1F)) {
                    textId = 0x3003;
...
s32 EnGo2_IsCameraModified(EnGo2* this, PlayState* play) {
    ...
    if ((this->actor.params & 0x1F) == GORON_FIRE_GENERIC || (this->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG ||
        ...
        return true;
    } else if (((!IS_RANDO(gSaveContext) && !CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) ||
                (IS_RANDO(gSaveContext) && !Flags_GetRandomizerInf(RAND_INF_DUNGEONS_DONE_FIRE_TEMPLE))) &&
...
void EnGo2_BiggoronEyedrops(EnGo2* this, PlayState* play) {
    switch (this->goronState) {
        case 0:
            ...
            this->trackingMode = NPC_TRACKING_NONE;
            this->animTimer = IS_RANDO(gSaveContext) ? 0 : (this->skelAnime.endFrame + 60.0f + 60.0f); // eyeDrops animation timer
