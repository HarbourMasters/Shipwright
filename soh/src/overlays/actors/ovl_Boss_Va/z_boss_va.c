...
s32 BossVa_BodyOverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 20) {
        ...
        gSPSegment(
            POLY_OPA_DISP++, 0x08,
            Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 8, 16, 1, 0, (play->gameplayFrames * -2) % 64, 16, 16, 0, 0, 0, -2));
        ...
        case BOSSVA_BODY:
            ...
            if (!this->isDead) {
                gSPSegment(POLY_OPA_DISP++, 0x08,
                           Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 8, 16, 1, 0, (play->gameplayFrames * -10) % 16,
                                            16, 16, 0, 0, 0, -10));
                gSPSegment(POLY_OPA_DISP++, 0x09,
                           Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, (play->gameplayFrames * -10) % 32, 16, 0x20, 1, 0,
                                            (play->gameplayFrames * -5) % 32, 16, 32, 0, -10, 0, -5));
