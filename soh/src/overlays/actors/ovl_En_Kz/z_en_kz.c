...
u16 EnKz_GetTextNoMaskChild(PlayState* play, EnKz* this) {
    ...
    if ((IS_RANDO(gSaveContext) && Flags_GetRandomizerInf(RAND_INF_DUNGEONS_DONE_JABU_JABUS_BELLY)) ||
        (!IS_RANDO(gSaveContext) && CHECK_QUEST_ITEM(QUEST_ZORA_SAPPHIRE))) {
        // Allow turning in Ruto's letter even if you have already rescued her
        if (IS_RANDO(gSaveContext) && !Flags_GetEventChkInf(EVENTCHKINF_KING_ZORA_MOVED)) {
...
void func_80A9CB18(EnKz* this, PlayState* play) {
    ...
    if (func_80A9C95C(play, this, &this->interactInfo.talkState, 340.0f, EnKz_GetText, func_80A9C6C0)) {
        if (((IS_RANDO(gSaveContext) && LINK_IS_CHILD) || this->actor.textId == 0x401A) && !Flags_GetEventChkInf(EVENTCHKINF_KING_ZORA_MOVED)) {
...
void EnKz_StartTimer(EnKz* this, PlayState* play) {
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_DONE) && Message_ShouldAdvance(play)) {
        if (INV_CONTENT(ITEM_TRADE_ADULT) == ITEM_FROG && !IS_RANDO(gSaveContext)) {
