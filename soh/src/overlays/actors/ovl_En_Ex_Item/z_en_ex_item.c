        ...
        switch (this->type) {
            ...
            case EXITEM_BOMB_BAG_BOWLING:
                ...
                if (onCounter == 0) {
                    ...
                    this->actor.shape.yOffset = IS_RANDO(gSaveContext) ? -10.0f : -18.0f;
                }
                break;
            case EXITEM_HEART_PIECE_COUNTER:
                onCounter = true;
            ...
            case EXITEM_14:
                ...
                this->unkFloat = 0.5f;
                this->actor.velocity.y = 10.0f;
                if (!IS_RANDO(gSaveContext) || !Randomizer_GetSettingValue(RSK_SHUFFLE_CHEST_MINIGAME)) {
                    switch (this->type) {
                        case EXITEM_GREEN_RUPEE_CHEST:
...
void EnExItem_TargetPrizeApproach(EnExItem* this, PlayState* play) {
    ...
    if (!IS_RANDO(gSaveContext) && this->timer != 0) {
        if (this->prizeRotateTimer != 0) {
            tmpf1 = play->view.lookAt.x - play->view.eye.x;
