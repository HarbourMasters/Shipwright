...
void DemoEffect_InitJewel(PlayState* play, DemoEffect* this) {
    ...
    if (IS_RANDO(gSaveContext) && play->sceneNum == SCENE_JABU_JABU) {
    ...
    this->csActionId = 1;
    this->actor.shape.rot.x = (IS_RANDO(gSaveContext) && play->sceneNum == SCENE_JABU_JABU) ? 0 : 16384;
...
void DemoEffect_UpdateGetItem(DemoEffect* this, PlayState* play) {
    ...
    if (play->csCtx.state != CS_STATE_IDLE && play->csCtx.npcActions[this->csActionId] != NULL) {
        ...
        if (gSaveContext.entranceIndex == 0x0053 || (IS_RANDO(gSaveContext) && gSaveContext.entranceIndex == 0x05F4)) {
        ...
        switch (play->csCtx.npcActions[this->csActionId]->action) {
            case 2:
                if (gSaveContext.entranceIndex == 0x0053 ||
                    (IS_RANDO(gSaveContext) && gSaveContext.entranceIndex == 0x05F4)) {
