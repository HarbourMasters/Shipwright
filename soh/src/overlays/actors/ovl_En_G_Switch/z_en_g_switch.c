    ...
    if (!this->broken) {
        ...
        if (CVarGetInteger("gNewDrops", 0)) {
            ...
            if (this->type == ENGSWITCH_TARGET_RUPEE) {
                ...
                Color_RGB8 silverRupeeColor = CVarGetColor24(COSMETIC_CVAR("Consumable_SilverRupee.Value"), (Color_RGB8){ 255, 255, 255 });
                Gfx_SetupDL_25Opa(play->state.gfxCtx);
                gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx),
            ...
            switch (this->colorIdx) {
                ...
                case 3: // pink rupee texture corresponds to the gold rupee (authentic bug)
                    ...
                    break;
                case 5:
                    rupeeColor = CVarGetColor24(COSMETIC_CVAR("Consumable_SilverRupee.Value"), (Color_RGB8){ 255, 255, 255 });
                    shouldColor = CVarGetInteger(COSMETIC_CVAR("Consumable_SilverRupee.Changed"), 0);
                    break;
            }
