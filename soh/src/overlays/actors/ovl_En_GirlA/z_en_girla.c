...
s32 EnGirlA_CanBuy_Bombs(PlayState* play, EnGirlA* this) {
    if (!IS_RANDO(gSaveContext) && !CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        return CANBUY_RESULT_CANT_GET_NOW;
    }
...
s32 EnGirlA_CanBuy_GoronTunic(PlayState* play, EnGirlA* this) {
    if (LINK_AGE_IN_YEARS == YEARS_CHILD && 
        (!IS_RANDO(gSaveContext) || Randomizer_GetSettingValue(RSK_SHOPSANITY) == RO_SHOPSANITY_OFF)) {
        return CANBUY_RESULT_CANT_GET_NOW;
    }
...
void EnGirlA_BuyEvent_ObtainBombchuPack(PlayState* play, EnGirlA* this) {
    ...
    if (IS_RANDO(gSaveContext) && Randomizer_GetSettingValue(RSK_BOMBCHUS_IN_LOGIC)) {
        return;
    }
