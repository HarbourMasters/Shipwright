...
void EnArrow_Init(Actor* thisx, PlayState* play) {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_NormalPrimary.Changed"), 0)) {
        blureNormal.altEnvColor = CVarGetColor(COSMETIC_CVAR("Arrows_NormalPrimary.Value"), (Color_RGBA8){ 0, 150, 0, 0 });
    } else {
        blureNormal.altEnvColor = (Color_RGBA8){ 0, 150, 0, 0 };
    }
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_NormalSecondary.Changed"), 0)) {
        blureNormal.altPrimColor = CVarGetColor(COSMETIC_CVAR("Arrows_NormalSecondary.Value"), (Color_RGBA8){ 255, 255, 170, 255 });
    } else {
        blureNormal.altPrimColor = (Color_RGBA8){ 255, 255, 170, 255 };
    }

    if (CVarGetInteger(COSMETIC_CVAR("Arrows_FirePrimary.Changed"), 0)) {
        blureFire.altEnvColor = CVarGetColor(COSMETIC_CVAR("Arrows_FirePrimary.Value"), (Color_RGBA8){ 255, 200, 0, 0 });
    } else {
        blureFire.altEnvColor = (Color_RGBA8){ 255, 200, 0, 0 };
    }
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_FireSecondary.Changed"), 0)) {
        blureFire.altPrimColor = CVarGetColor(COSMETIC_CVAR("Arrows_FireSecondary.Value"), (Color_RGBA8){ 255, 0, 0, 255 });
    } else {
        blureFire.altPrimColor = (Color_RGBA8){ 255, 0, 0, 255 };
    }

    if (CVarGetInteger(COSMETIC_CVAR("Arrows_IcePrimary.Changed"), 0)) {
        blureIce.altEnvColor = CVarGetColor(COSMETIC_CVAR("Arrows_IcePrimary.Value"), (Color_RGBA8){ 0, 0, 255, 255 });
    } else {
        blureIce.altEnvColor = (Color_RGBA8){ 0, 0, 255, 255 };
    }
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_IceSecondary.Changed"), 0)) {
        blureIce.altPrimColor = CVarGetColor(COSMETIC_CVAR("Arrows_IceSecondary.Value"), (Color_RGBA8){ 170, 255, 255, 0 });
    } else {
        blureIce.altPrimColor = (Color_RGBA8){ 170, 255, 255, 0 };
    }

    if (CVarGetInteger(COSMETIC_CVAR("Arrows_LightPrimary.Changed"), 0)) {
        blureLight.altEnvColor = CVarGetColor(COSMETIC_CVAR("Arrows_LightPrimary.Value"), (Color_RGBA8){ 255, 255, 0, 255 });
    } else {
        blureLight.altEnvColor = (Color_RGBA8){ 255, 255, 0, 255 };
    }
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_LightSecondary.Changed"), 0)) {
        blureLight.altPrimColor = CVarGetColor(COSMETIC_CVAR("Arrows_LightSecondary.Value"), (Color_RGBA8){ 255, 255, 170, 0 });
    } else {
