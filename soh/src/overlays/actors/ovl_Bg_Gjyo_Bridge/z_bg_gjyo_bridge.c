...
void BgGjyoBridge_Init(Actor* thisx, PlayState* play) {
    ...
    if (Flags_GetEventChkInf(EVENTCHKINF_RAINBOW_BRIDGE_BUILT) || (IS_RANDO(gSaveContext) && bridge == RO_BRIDGE_ALWAYS_OPEN)) {
...
void BgGjyoBridge_SpawnBridge(BgGjyoBridge* this, PlayState* play) {
    if (IS_RANDO(gSaveContext) || (play->csCtx.state != CS_STATE_IDLE) && (play->csCtx.npcActions[2] != NULL) &&
