...
void ArrowLight_Draw(Actor* thisx, PlayState* play) {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_LightPrimary.Changed"), 0)) {
        primaryColor = CVarGetColor24(COSMETIC_CVAR("Arrows_LightPrimary.Value"), primaryColor);
    }
    Color_RGB8 secondaryColor = {255, 255, 0};
    if (CVarGetInteger(COSMETIC_CVAR("Arrows_LightSecondary.Changed"), 0)) {
        secondaryColor = CVarGetColor24(COSMETIC_CVAR("Arrows_LightSecondary.Value"), secondaryColor);
    }
