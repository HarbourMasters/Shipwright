...
void KaleidoScope_DrawAmmoCount(PauseContext* pauseCtx, GraphicsContext* gfxCtx, s16 item, int slot) {
    ...
    if (modId == 0 ? !CHECK_AGE_REQ_SLOT(SLOT(item)) : !CHECK_AGE_REQ_MODDED_ITEM(modId, item)) {
...
void KaleidoScope_DrawItemCycleExtras(PlayState* play, u8 slot, u8 canCycle, u8 leftModId, u8 leftItem, u8 rightModId, u8 rightItem) {
...
void KaleidoScope_HandleItemCycleExtras(PlayState* play, u8 slot, bool canCycle, u8 leftModId, u8 leftItem, u8 rightModId, u8 rightItem, bool replaceCButtons) {
...
void KaleidoScope_DrawItemSelect(PlayState* play) {
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_ITEM)) {
        ...
        cursorItemModId = pauseCtx->cursorItemModId[PAUSE_ITEM];
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            if ((ABS(pauseCtx->stickRelX) > 30) || (dpad && CHECK_BTN_ANY(input->press.button, BTN_DLEFT | BTN_DRIGHT))) {
                ...
                if (moveCursorResult == 1) {
                    ...
                    cursorItemModId = gSaveContext.inventory.itemModIds[pauseCtx->cursorPoint[PAUSE_ITEM]];
                }
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            if (cursorItemModId == 0 ? !CHECK_AGE_REQ_SLOT(cursorSlot) : !CHECK_AGE_REQ_MODDED_ITEM(cursorItemModId, cursorItem)) {
    ...
    for (i = j = 0; i < 24; i++, j += 4) {
        ...
        if (gSaveContext.inventory.items[i] != ITEM_NONE || gSaveContext.inventory.itemModIds[i] != 0) {
            if ((pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_ITEM) && (pauseCtx->cursorSpecialPos == 0)) {
                if (gSaveContext.inventory.itemModIds[i] == 0 ? CHECK_AGE_REQ_SLOT(i) : CHECK_AGE_REQ_MODDED_ITEM(gSaveContext.inventory.itemModIds[i], gSaveContext.inventory.items[i])) {
            ...
            bool not_acquired = modId == 0 ? !CHECK_AGE_REQ_ITEM(itemId) : !CHECK_AGE_REQ_MODDED_ITEM(modId, itemId);
            if (not_acquired) {
                gDPSetGrayscaleColor(POLY_KAL_DISP++, 109, 109, 109, 255);
                gSPGrayscale(POLY_KAL_DISP++, true);
            }
...
void KaleidoScope_SetupItemEquip(PlayState* play, u16 item, u16 modId, u16 slot, s16 animX, s16 animY) {
    Input* input = &play->state.input[0];
    PauseContext* pauseCtx = &play->pauseCtx;
    ...
    pauseCtx->equipTargetModId = modId;
    ...
    sEquipMoveTimer = 10;
    if (
        pauseCtx->equipTargetModId == 0 &&
        (
            (pauseCtx->equipTargetItem == ITEM_ARROW_FIRE) ||
...
void KaleidoScope_UpdateItemEquip(PlayState* play) {
    ...
    if (sEquipState == 2) {
        ...
        if (D_8082A488 == 0 && pauseCtx->equipTargetModId == 0) {
    ...
    if (pauseCtx->equipTargetModId == 0 && (pauseCtx->equipTargetItem >= 0xBF) && (pauseCtx->equipAnimAlpha < 254)) {
        pauseCtx->equipAnimAlpha += 14;
        if (pauseCtx->equipAnimAlpha > 255) {
            pauseCtx->equipAnimAlpha = 254;
        }
    ...
    if (sEquipAnimTimer == 0) {
        ...
        if (sEquipMoveTimer == 0) {
            ...
            for (uint16_t otherSlotIndex = 0; otherSlotIndex < ARRAY_COUNT(gSaveContext.equips.cButtonSlots);
                ...
                if (pauseCtx->equipTargetSlot == gSaveContext.equips.cButtonSlots[otherSlotIndex]) {
                    ...
                    if (gSaveContext.equips.buttonItems[targetButtonIndex] != ITEM_NONE) {
                        ...
                        gSaveContext.equips.buttonModIds[otherButtonIndex] =
                            gSaveContext.equips.buttonModIds[targetButtonIndex];
                        ...
                        gSaveContext.equips.buttonModIds[otherButtonIndex] = 0;
                ...
                //Fix for Equip Dupe
                if (pauseCtx->equipTargetModId == 0 && pauseCtx->equipTargetItem == ITEM_BOW) {
            ...
            gSaveContext.equips.buttonModIds[targetButtonIndex] = pauseCtx->equipTargetModId;
