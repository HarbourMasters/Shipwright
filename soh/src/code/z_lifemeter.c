...
void HealthMeter_Init(PlayState* play) {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Consumable_Hearts.Changed"), 0)) {
        mainColor = CVarGetColor24(COSMETIC_CVAR("Consumable_Hearts.Value"), mainColor);
    }
    Color_RGB8 mainBorder = {HEARTS_ENV_R, HEARTS_ENV_G, HEARTS_ENV_B};
    if (CVarGetInteger(COSMETIC_CVAR("Consumable_HeartBorder.Changed"), 0)) {
        mainBorder = CVarGetColor24(COSMETIC_CVAR("Consumable_HeartBorder.Value"), mainBorder);
    }
    Color_RGB8 ddColor = {HEARTS_DD_ENV_R, HEARTS_DD_ENV_G, HEARTS_DD_ENV_B};
    if (CVarGetInteger(COSMETIC_CVAR("Consumable_DDHearts.Changed"), 0)) {
        ddColor = CVarGetColor24(COSMETIC_CVAR("Consumable_DDHearts.Value"), ddColor);
    }
    Color_RGB8 ddBorder = {HEARTS_DD_PRIM_R, HEARTS_DD_PRIM_G, HEARTS_DD_PRIM_B};
    if (CVarGetInteger(COSMETIC_CVAR("Consumable_DDHeartBorder.Changed"), 0)) {
        ddBorder = CVarGetColor24(COSMETIC_CVAR("Consumable_DDHeartBorder.Value"), ddBorder);
    }
...
void HealthMeter_Update(PlayState* play) {
    ...
    Top_LM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginTop"), 0);
    Left_LM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginLeft"), 0);
    Right_LM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginRight"), 0);
    Bottom_LM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginBottom"), 0);
...
s16 getHealthMeterXOffset() {
    s16 X_Margins;
    if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.UseMargins"), 0) != 0)
        X_Margins = Left_LM_Margin;
    else
        X_Margins = 0;

    if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) != 0) {
        if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 1) {//Anchor Left
            return OTRGetDimensionFromLeftEdge(CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+X_Margins+70.0f);
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 2) {//Anchor Right
            X_Margins = Right_LM_Margin;
            return OTRGetDimensionFromRightEdge(CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+X_Margins+70.0f);
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 3) {//Anchor None
            return CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+70.0f;;
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 4) {//Hidden
            return -9999;
        }
    } else {
...
s16 getHealthMeterYOffset() {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) != 0) {
        HeartsScale = CVarGetFloat(COSMETIC_CVAR("Hud_Hearts.Scale"), 0.7f);
        return CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosY"), 0)+Y_Margins+(HeartsScale*15);
    } else {
...
/*
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) != 0) {
        offsetY = CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosY"), 0)+Y_Margins+(HeartsScale*15);
        if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 1) {//Anchor Left
            offsetX = OTRGetDimensionFromLeftEdge(CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+X_Margins+70.0f);
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 2) {//Anchor Right
            X_Margins = Right_LM_Margin;
            offsetX = OTRGetDimensionFromRightEdge(CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+X_Margins+70.0f);
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 3) {//Anchor None
            offsetX = CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.PosX"), 0)+70.0f;
        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Hearts.AnchorType"), 0) == 4) {//Hidden
            offsetX = -9999;
        }
    } else {
