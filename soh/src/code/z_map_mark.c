            ...
            if ((mapMarkIconData->markType != MAP_MARK_CHEST) || !Flags_GetTreasure(play, markPoint->chestFlag)) {
                ...
                if (CVarGetInteger("gMinimapPosType", 0) != 0) {
                    ...
                    if (CVarGetInteger("gMinimapPosType", 0) == 1) { // Anchor Left
                        ...
                        if (play->sceneNum == SCENE_YDAN || play->sceneNum == SCENE_DDAN ||
                            play->sceneNum == SCENE_BDAN || play->sceneNum == SCENE_BMORI1 ||
                            play->sceneNum == SCENE_HIDAN || play->sceneNum == SCENE_MIZUSIN ||
                            play->sceneNum == SCENE_JYASINZOU || play->sceneNum == SCENE_HAKADAN ||
                            play->sceneNum == SCENE_HAKADANCH || play->sceneNum == SCENE_ICE_DOUKUTO) {
                            rectLeft = OTRGetRectDimensionFromLeftEdge(markPointX + CVarGetInteger("gMinimapPosX", 0) +
                                                                       204 + X_Margins_Minimap_ic);
                        } else {
                            rectLeft = OTRGetRectDimensionFromLeftEdge(markPointX + CVarGetInteger("gMinimapPosX", 0) +
                                                                       204 + X_Margins_Minimap_ic);
                        }
                        ...
                        rectLeft = OTRGetRectDimensionFromRightEdge(markPointX + CVarGetInteger("gMinimapPosX", 0) +
                                                                    204 + X_Margins_Minimap_ic);
