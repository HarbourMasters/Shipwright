...
void Player_InitAimCueCollision(Player* this, PlayState* play) {
    for (int i = 0; i < AIMCUE_COLLIDER_COUNT; i++) {
        Collider_InitQuad(play, &sAimCueCollider[i]);
        Collider_SetQuad(play, &sAimCueCollider[i], NULL, &sSensingColliderInit);
    }
}
...
void Player_UpdateAimCue(Player* this, PlayState* play) {
    ...
    for (int i = 0; i < AIMCUE_COLLIDER_COUNT; i++) {
        if (sAimCueCollider[i].base.atFlags & AT_HIT) {
            ...
            if (i != 0 && (play->gameplayFrames % 1) != 0) {
                continue;
            }
...
void Player_ComputeHandAimCue(Player* this, PlayState* play, s32 limbIndex) {
    ...
    if (BgCheck_AnyLineTest3(&play->colCtx, &vecCenter, &vecCenterFar, &vecResult, &polyResult, 1, 1, 1, 1,
                             &bgId)) {
        sAimSurfaceHookshotable =
            isHoldingHookshotType && SurfaceType_IsHookshotSurface(&play->colCtx, polyResult, bgId);
    ...
    CollisionCheck_SetAT(play, &play->colChkCtx, &sAimCueCollider[0].base);

    Matrix_Scale(500.0f, 500.0f, 1.0f, MTXMODE_APPLY);

    f32 rotateOffset = (play->gameplayFrames % 4) * M_PI / 16;
    ...
    for (int i = 1; i < AIMCUE_COLLIDER_COUNT; i++) {
        ...
        CollisionCheck_SetAT(play, &play->colChkCtx, &sAimCueCollider[i].base);
    }
