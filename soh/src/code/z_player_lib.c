    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) 
    {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            if ((sLeftHandType == 4) && (gSaveContext.swordHealth <= 0.0f)) {
                dLists += 4;
            } else if ((sLeftHandType == 6) && (this->stateFlags1 & PLAYER_STATE1_THREW_BOOMERANG)) {
                dLists = &gPlayerLeftHandOpenDLs[gSaveContext.linkAge];
                sLeftHandType = 0;
            } else if ((this->leftHandType == 0) && (this->actor.speedXZ > 2.0f) && !(this->stateFlags1 & PLAYER_STATE1_IN_WATER)) {
            ...
            *dList = ResourceMgr_LoadGfxByName(dLists[sDListsLodOffset]);
            ...
            } else if ((this->rightHandType == 8) && (this->actor.speedXZ > 2.0f) && !(this->stateFlags1 & PLAYER_STATE1_IN_WATER)) {
