...
void Inventory_SwapAgeEquipment(void) {
    ...
    if (LINK_AGE_IN_YEARS == YEARS_CHILD) {
        ...
        // When using enhancements, set swordless flag if player doesn't have kokiri sword or hasn't equipped a sword
        // yet. Then set the child equips button items to item none to ensure kokiri sword is not equipped
        if ((CVarGetInteger("gSwitchAge", 0) || CVarGetInteger("gSwitchTimeline", 0)) &&
            ((1 << 0 & gSaveContext.inventory.equipment) == 0 || Flags_GetInfTable(INFTABLE_SWORDLESS))) {
            Flags_SetInfTable(INFTABLE_SWORDLESS);
        ...
        if (CVarGetInteger("gSwitchAge", 0) || CVarGetInteger("gSwitchTimeline", 0)) {
            ...
            // Equips kokiri sword in the inventory screen only if kokiri sword exists in inventory and a sword has been
            // equipped already
    ...
    if ((item >= ITEM_MEDALLION_FOREST) && (item <= ITEM_MEDALLION_LIGHT)) {
        ...
        if (item == ITEM_SWORD_BGS) {
            ...
            if (ALL_EQUIP_VALUE(EQUIP_SWORD) == 0xF ||
                (gSaveContext.n64ddFlag && ALL_EQUIP_VALUE(EQUIP_SWORD) == 0xE)) {
...
void Interface_UpdateMagicBar(PlayState* play) {
    ...
    if (CVarGetInteger("gCosmetics.Consumable_MagicBorderActive.Changed",
        ...
        sMagicBorderColors[2][0] =
            CVarGetColor24("gCosmetics.Consumable_MagicBorderActive.Value", MagicBorder_2).r / 2.5;
        sMagicBorderColors[2][1] =
            CVarGetColor24("gCosmetics.Consumable_MagicBorderActive.Value", MagicBorder_2).g / 2.5;
        sMagicBorderColors[2][2] =
            CVarGetColor24("gCosmetics.Consumable_MagicBorderActive.Value", MagicBorder_2).b / 2.5;
    ...
    switch (gSaveContext.magicState) {
        ...
        case 7:
            if ((play->pauseCtx.state == 0) && (play->pauseCtx.debugState == 0) && (msgCtx->msgMode == MSGMODE_NONE) &&
                (play->gameOverCtx.state == GAMEOVER_INACTIVE) && (play->sceneLoadFlag == 0) &&
                (play->transitionMode == 0) && !Play_InCsMode(play)) {
                bool hasLens = false;
                for (int buttonIndex = 1; buttonIndex < (CVarGetInteger("gDpadEquips", 0) != 0)
...
void Interface_DrawEnemyHealthBar(TargetContext* targetCtx, PlayState* play) {
    ...
    if (targetCtx->unk_48 != 0 && actor != NULL && actor->category == ACTORCAT_ENEMY) {
        ...
        Interface_CreateQuadVertexGroup(&sEnemyHealthVtx[0], -floorf(halfBarWidth), -texHeight / 2, endTexWidth,
                                        texHeight, 0);
        ...
        Interface_CreateQuadVertexGroup(&sEnemyHealthVtx[8], ceilf(halfBarWidth) - endTexWidth, -texHeight / 2,
                                        endTexWidth, texHeight, 1);
        // Health bar fill
...
void Interface_DrawItemButtons(PlayState* play) {
    ...
    if (CVarGetInteger("gStartBtnPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gStartBtnPosType", 0) == 1) { // Anchor Left
            ...
            PosX_StartBtn = OTRGetDimensionFromLeftEdge(CVarGetInteger("gStartBtnPosX", 0) - (Start_BTN_Scale * 13) +
                                                        X_Margins_StartBtn);
        } else if (CVarGetInteger("gStartBtnPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gStartBtnUseMargins", 0) != 0) {
                X_Margins_StartBtn = Right_HUD_Margin;
            };
            PosX_StartBtn = OTRGetDimensionFromRightEdge(CVarGetInteger("gStartBtnPosX", 0) - (Start_BTN_Scale * 13) +
                                                         X_Margins_StartBtn);
...
void Interface_DrawItemIconTexture(PlayState* play, void* texture, s16 button) {
    ...
    if (CVarGetInteger("gDPadPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gDPadPosType", 0) == 1) { // Anchor Left
            ...
            ItemIconPos[4][0] = OTRGetDimensionFromLeftEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                            DPad_ItemsOffset[0][0]);
            ItemIconPos[5][0] = OTRGetDimensionFromLeftEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                            DPad_ItemsOffset[1][0]);
            ItemIconPos[6][0] = OTRGetDimensionFromLeftEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                            DPad_ItemsOffset[2][0]);
            ItemIconPos[7][0] = OTRGetDimensionFromLeftEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                            DPad_ItemsOffset[3][0]);
        } else if (CVarGetInteger("gDPadPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gDPadUseMargins", 0) != 0) {
                X_Margins_DPad_Items = Right_HUD_Margin;
            };
            ItemIconPos[4][0] = OTRGetDimensionFromRightEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                             DPad_ItemsOffset[0][0]);
            ItemIconPos[5][0] = OTRGetDimensionFromRightEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                             DPad_ItemsOffset[1][0]);
            ItemIconPos[6][0] = OTRGetDimensionFromRightEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                             DPad_ItemsOffset[2][0]);
            ItemIconPos[7][0] = OTRGetDimensionFromRightEdge(CVarGetInteger("gDPadPosX", 0) + X_Margins_DPad_Items +
                                                             DPad_ItemsOffset[3][0]);
...
void Interface_DrawAmmoCount(PlayState* play, s16 button, s16 alpha) {
    ...
    if (CVarGetInteger("gBBtnPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gBBtnPosType", 0) == 1) { // Anchor Left
            ...
            ItemIconPos[0][0] =
                OTRGetDimensionFromLeftEdge(CVarGetInteger("gBBtnPosX", 0) + X_Margins_BtnB + PosX_adjust);
        } else if (CVarGetInteger("gBBtnPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gBBtnUseMargins", 0) != 0) {
                X_Margins_BtnB = Right_HUD_Margin;
            };
            ItemIconPos[0][0] =
                OTRGetDimensionFromRightEdge(CVarGetInteger("gBBtnPosX", 0) + X_Margins_BtnB + PosX_adjust);
        } else if (CVarGetInteger("gBBtnPosType", 0) == 3) { // Anchor None
    ...
    if (CVarGetInteger("gCBtnLPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gCBtnLPosType", 0) == 1) { // Anchor Left
            ...
            ItemIconPos[1][0] =
                OTRGetDimensionFromLeftEdge(CVarGetInteger("gCBtnLPosX", 0) + X_Margins_CL + PosX_adjust);
        } else if (CVarGetInteger("gCBtnLPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gCBtnLUseMargins", 0) != 0) {
                X_Margins_CL = Right_HUD_Margin;
            };
            ItemIconPos[1][0] =
                OTRGetDimensionFromRightEdge(CVarGetInteger("gCBtnLPosX", 0) + X_Margins_CL + PosX_adjust);
        } else if (CVarGetInteger("gCBtnLPosType", 0) == 3) { // Anchor None
    ...
    if (CVarGetInteger("gCBtnDPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gCBtnDPosType", 0) == 1) { // Anchor Left
            ...
            ItemIconPos[2][0] =
                OTRGetDimensionFromLeftEdge(CVarGetInteger("gCBtnDPosX", 0) + X_Margins_CD + PosX_adjust);
        } else if (CVarGetInteger("gCBtnDPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gCBtnDUseMargins", 0) != 0) {
                X_Margins_CD = Right_HUD_Margin;
            };
            ItemIconPos[2][0] =
                OTRGetDimensionFromRightEdge(CVarGetInteger("gCBtnDPosX", 0) + X_Margins_CD + PosX_adjust);
        } else if (CVarGetInteger("gCBtnDPosType", 0) == 3) { // Anchor None
    ...
    if (CVarGetInteger("gCBtnRPosType", 0) != 0) {
        ...
        if (CVarGetInteger("gCBtnRPosType", 0) == 1) { // Anchor Left
            ...
            ItemIconPos[3][0] =
                OTRGetDimensionFromLeftEdge(CVarGetInteger("gCBtnRPosX", 0) + X_Margins_CR + PosX_adjust);
        } else if (CVarGetInteger("gCBtnRPosType", 0) == 2) { // Anchor Right
            if (CVarGetInteger("gCBtnRUseMargins", 0) != 0) {
                X_Margins_CR = Right_HUD_Margin;
            };
            ItemIconPos[3][0] =
                OTRGetDimensionFromRightEdge(CVarGetInteger("gCBtnRPosX", 0) + X_Margins_CR + PosX_adjust);
        } else if (CVarGetInteger("gCBtnRPosType", 0) == 3) { // Anchor None
...
void Interface_Draw(PlayState* play) {
    ...
    if (pauseCtx->debugState == 0) {
            ...
            switch (play->sceneNum) {
                ...
                case SCENE_TREASURE_BOX_SHOP:
                    if (gSaveContext.inventory.dungeonKeys[gSaveContext.mapIndex] >= 0) {
                        ...
                        OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gSmallKeyCounterIconTex, 16, 16, PosX_SKC, PosY_SKC,
                                                      16, 16, 1 << 10, 1 << 10);
                        ...
                        if (interfaceCtx->counterDigits[2] != 0) {
                            OVERLAY_DISP =
                                Gfx_TextureI8(OVERLAY_DISP, ((u8*)((u8*)digitTextures[interfaceCtx->counterDigits[2]])),
                                              8, 16, PosX_SKC + 8, PosY_SKC, 8, 16, 1 << 10, 1 << 10);
                        }

                        OVERLAY_DISP = Gfx_TextureI8(OVERLAY_DISP, ((u8*)digitTextures[interfaceCtx->counterDigits[3]]),
                                                     8, 16, PosX_SKC + 16, PosY_SKC, 8, 16, 1 << 10, 1 << 10);
                    }
                    break;
                default:
                    break;
        ...
        if (!(interfaceCtx->unk_1FA)) {
            ...
            if (CVarGetInteger("gBBtnPosType", 0) != 0) {
                ...
                if (CVarGetInteger("gBBtnPosType", 0) == 1) { // Anchor Left
                    ...
                    BbtnPosX = OTRGetDimensionFromLeftEdge(CVarGetInteger("gBBtnPosX", 0) + X_Margins_BtnB_label +
                                                           PosX_adjust);
                } else if (CVarGetInteger("gBBtnPosType", 0) == 2) { // Anchor Right
                    if (CVarGetInteger("gBBtnUseMargins", 0) != 0) {
                        X_Margins_BtnB_label = Right_HUD_Margin;
                    };
                    BbtnPosX = OTRGetDimensionFromRightEdge(CVarGetInteger("gBBtnPosX", 0) + X_Margins_BtnB_label +
                                                            PosX_adjust);
                } else if (CVarGetInteger("gBBtnPosType", 0) == 3) { // Anchor None
            ...
            if (gSaveContext.minigameState != 1) {
                ...
                if (interfaceCtx->unk_1EE == 8 && !CVarGetInteger("gInfiniteEpona", 0)) {
                    ...
                    if (CVarGetInteger("gCarrotsPosType", 0) != 0) {
                        ...
                        if (CVarGetInteger("gCarrotsPosType", 0) == 1) { // Anchor Left
                            if (CVarGetInteger("gCarrotsUseMargins", 0) != 0) {
                                CarrotsMargins_X = Left_HUD_Margin;
                            };
                            CarrotsPosX =
                                OTRGetDimensionFromLeftEdge(CVarGetInteger("gCarrotsPosX", 0) + CarrotsMargins_X);
                        } else if (CVarGetInteger("gCarrotsPosType", 0) == 2) { // Anchor Right
                            if (CVarGetInteger("gCarrotsUseMargins", 0) != 0) {
                                CarrotsMargins_X = Right_HUD_Margin;
                            };
                            CarrotsPosX =
                                OTRGetDimensionFromRightEdge(CVarGetInteger("gCarrotsPosX", 0) + CarrotsMargins_X);
                ...
                if (CVarGetInteger("gASPosType", 0) != 0) {
                    ...
                    if (CVarGetInteger("gASPosType", 0) == 1) { // Anchor Left
                        ...
                        ArcheryPos_X =
                            OTRGetRectDimensionFromLeftEdge(CVarGetInteger("gASPosX", 0) + X_Margins_Archery);
                    } else if (CVarGetInteger("gASPosType", 0) == 2) { // Anchor Right
                        if (CVarGetInteger("gASUseMargins", 0) != 0) {
                            X_Margins_Archery = Right_HUD_Margin;
                        };
                        ArcheryPos_X =
                            OTRGetRectDimensionFromRightEdge(CVarGetInteger("gASPosX", 0) + X_Margins_Archery);
        ...
        if ((play->pauseCtx.state == 0) && (play->pauseCtx.debugState == 0) &&
            ...
            !(player->stateFlags2 & 0x01000000) && (play->sceneLoadFlag == 0) && (play->transitionMode == 0) &&
...
void Interface_DrawTotalGameplayTimer(PlayState* play) {
    ...
    if ((gSaveContext.isBossRush && gSaveContext.bossRushOptions[BR_OPTIONS_TIMER] == BR_CHOICE_TIMER_YES) ||
        (CVarGetInteger("gGameplayStats.ShowIngameTimer", 0) && gSaveContext.fileNum >= 0 &&
...
void Interface_Update(PlayState* play) {
    ...
    if ((play->pauseCtx.state == 0) && (play->pauseCtx.debugState == 0) && (msgCtx->msgMode == MSGMODE_NONE) &&
        (play->sceneLoadFlag == 0) && (play->gameOverCtx.state == GAMEOVER_INACTIVE) && (play->transitionMode == 0) &&
