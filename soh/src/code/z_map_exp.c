...
void Minimap_DrawCompassIcons(PlayState* play) {
    ...
    if (play->interfaceCtx.minimapAlpha >= 0xAA) {
        ...
        s16 mapWidth = 0;
        s16 mapStartPosX = 0;
        if (SCENEDB_ISOVERWORLD(entry)) { // Overworld
            mapStartPosX = entry->worldData.minimapX;
            mapWidth = entry->worldData.minimapWidth;
        } else if (SCENEDB_ISDUNGEON(entry)) { // Dungeons
            mapStartPosX = R_DGN_MINIMAP_X;
            mapWidth = 96;
        }

        s16 compassOffsetX = SCENEDB_ISDUNGEON(entry) ? entry->dungeonData.rooms[play->roomCtx.curRoom.num].compassOffsetX : entry->compassInfo.x;
        s16 compassOffsetY = SCENEDB_ISDUNGEON(entry) ? entry->dungeonData.rooms[play->roomCtx.curRoom.num].compassOffsetY : entry->compassInfo.y;
        ...
        s16 mirrorOffset = ((mapWidth / 2) - ((compassOffsetX / 10) - (mapStartPosX - SCREEN_WIDTH / 2))) * 2 * 10;

        tempX = player->actor.world.pos.x;
        tempZ = player->actor.world.pos.z;
        tempX /= entry->compassInfo.scaleX * (CVarGetInteger(CVAR_ENHANCEMENT("MirroredWorld"), 0) ? -1 : 1);
        tempZ /= entry->compassInfo.scaleY;

        s16 tempXOffset = compassOffsetX + (CVarGetInteger(CVAR_ENHANCEMENT("MirroredWorld"), 0) ? mirrorOffset : 0);
        if (CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) != 0) {
            if (CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) == 1) {//Anchor Left
                ...
                Matrix_Translate(
                    ...
                    (compassOffsetY + ((Y_Margins_Minimap*10)*-1) - tempZ + ((CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            ...
            Matrix_Translate(OTRGetDimensionFromRightEdge((tempXOffset+(X_Margins_Minimap*10) + tempX) / 10.0f), (compassOffsetY+((Y_Margins_Minimap*10)*-1) - tempZ) / 10.0f, 0.0f, MTXMODE_NEW);
        }
        ...
        if (CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) != 0) {
            if (CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) == 1) {//Anchor Left
                ...
                Matrix_Translate(
                    (tempXOffset + tempX + (CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosX"), 0)*10) / 10.0f),
                    (compassOffsetY - tempZ + ((CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            }
        } else {
            Matrix_Translate(OTRGetDimensionFromRightEdge((tempXOffset+(X_Margins_Minimap*10) + tempX) / 10.0f), (compassOffsetY+((Y_Margins_Minimap*10)*-1) - tempZ) / 10.0f, 0.0f, MTXMODE_NEW);
        }
        ...
        if (SCENEDB_ISDUNGEON(entry)) {
            ...
            if (CHECK_BTN_ALL(play->state.input[0].press.button, BTN_L) && !Play_InCsMode(play) && enableMapToggle) {
                ...
                if (!R_MINIMAP_DISABLED) {
                    Audio_PlaySoundGeneral(NA_SE_SY_CAMERA_ZOOM_UP, &D_801333D4, 4,
                        &D_801333E0, &D_801333E0, &D_801333E8);
                } else {
                    Audio_PlaySoundGeneral(NA_SE_SY_CAMERA_ZOOM_DOWN, &D_801333D4, 4,
                        &D_801333E0, &D_801333E0, &D_801333E8);
                }
                // clang-format on
            ...
            if (!R_MINIMAP_DISABLED && CVarGetInteger(CVAR_COSMETIC("HUD.Minimap.PosType"), 0) != 4) { // Not Hidden
                ...
                if (((play->sceneNum != SCENE_KAKARIKO_VILLAGE) && (play->sceneNum != SCENE_KOKIRI_FOREST) &&
                    ...
                    s16 newY = sOwEntranceIconPosY & 0x3FF;
