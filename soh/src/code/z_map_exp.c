...
void Minimap_DrawCompassIcons(PlayState* play) {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Hud_MinimapEntrance.Changed"), 0)) {
        lastEntranceColor = CVarGetColor24(COSMETIC_CVAR("Hud_MinimapEntrance.Value"), lastEntranceColor);
    }
    Color_RGB8 currentPositionColor = { 200, 255, 0 };
    if (CVarGetInteger(COSMETIC_CVAR("Hud_MinimapPosition.Changed"), 0)) {
        currentPositionColor = CVarGetColor24(COSMETIC_CVAR("Hud_MinimapPosition.Value"), currentPositionColor);
    }
    s16 X_Margins_Minimap;
    s16 Y_Margins_Minimap;
    if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {
        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 0) {X_Margins_Minimap = Right_MM_Margin;};
    ...
    if (play->interfaceCtx.minimapAlpha >= 0xAA) {
        ...
        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) {//Anchor Left
                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Left_MM_Margin;};
                Matrix_Translate(
                    OTRGetDimensionFromLeftEdge((tempXOffset + (X_Margins_Minimap*10) + tempX + (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)*10)) / 10.0f),
                    (R_COMPASS_OFFSET_Y + ((Y_Margins_Minimap*10)*-1) - tempZ + ((CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 2) {//Anchor Right
                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Right_MM_Margin;};
                Matrix_Translate(
                    OTRGetDimensionFromRightEdge((tempXOffset + (X_Margins_Minimap*10) + tempX + (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)*10)) / 10.0f),
                    (R_COMPASS_OFFSET_Y +((Y_Margins_Minimap*10)*-1) - tempZ + ((CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 3) {//Anchor None
                Matrix_Translate(
                    (tempXOffset + tempX + (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)*10) / 10.0f),
                    (R_COMPASS_OFFSET_Y + ((Y_Margins_Minimap*10)*-1) - tempZ + ((CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            }
        } else {
        ...
        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) {//Anchor Left
                ...
                Matrix_Translate(
                    (tempXOffset + tempX + (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)*10) / 10.0f),
                    (R_COMPASS_OFFSET_Y - tempZ + ((CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)*10)*-1)) / 10.0f, 0.0f, MTXMODE_NEW);
            }
        } else {
            Matrix_Translate(OTRGetDimensionFromRightEdge((tempXOffset+(X_Margins_Minimap*10) + tempX) / 10.0f), (R_COMPASS_OFFSET_Y+((Y_Margins_Minimap*10)*-1) - tempZ) / 10.0f, 0.0f, MTXMODE_NEW);
        }
...
void Minimap_Draw(PlayState* play) {
    ...
    if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.Changed"), 0)) {
        minimapColor = CVarGetColor24(COSMETIC_CVAR("Hud_Minimap.Value"), minimapColor);
    }
    ...
    if (play->pauseCtx.state < 4) {
        ...
        switch (play->sceneNum) {
            ...
            case SCENE_BOTTOM_OF_THE_WELL:
            case SCENE_ICE_CAVERN:
                if (!R_MINIMAP_DISABLED && CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 4) { // Not Hidden
                    ...
                    if (CHECK_DUNGEON_ITEM(DUNGEON_MAP, mapIndex)) {
                        ...
                        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
                            dgnMiniMapY = R_DGN_MINIMAP_Y+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)+Y_Margins_Minimap;
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) {//Anchor Left
                                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Left_MM_Margin;};
                                dgnMiniMapX = OTRGetDimensionFromLeftEdge(R_DGN_MINIMAP_X+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)+X_Margins_Minimap);
                            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 2) {//Anchor Right
                                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Right_MM_Margin;};
                                dgnMiniMapX = OTRGetDimensionFromRightEdge(R_DGN_MINIMAP_X+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)+X_Margins_Minimap);
                            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 3) {//Anchor None
                                dgnMiniMapX = CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0);
                            }
                        }
            ...
            case SCENE_OUTSIDE_GANONS_CASTLE:
                if (!R_MINIMAP_DISABLED && CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 4) { // Not Hidden
                    ...
                    if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
                        oWMiniMapY = R_OW_MINIMAP_Y+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0)+Y_Margins_Minimap;
                        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) {//Anchor Left
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Left_MM_Margin;};
                            oWMiniMapX = OTRGetDimensionFromLeftEdge(R_OW_MINIMAP_X+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)+X_Margins_Minimap);
                        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 2) {//Anchor Right
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Right_MM_Margin;};
                            oWMiniMapX = OTRGetDimensionFromRightEdge(R_OW_MINIMAP_X+CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0)+X_Margins_Minimap);
                        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 3) {//Anchor None
                            oWMiniMapX = CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0);
                        }
                    }
                    ...
                    if (((play->sceneNum != SCENE_KAKARIKO_VILLAGE) && (play->sceneNum != SCENE_KOKIRI_FOREST) &&
                        ...
                        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
                            entranceY = newY + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0) + Y_Margins_Minimap;
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) { // Anchor Left
                                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Left_MM_Margin;};
                                entranceX = OTRGetRectDimensionFromLeftEdge(newX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0));
                            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 2) { // Anchor Right
                                if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Right_MM_Margin;};
                                entranceX = OTRGetRectDimensionFromRightEdge(newX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0));
                            } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 3) { // Anchor None
                                entranceX = newX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0);
                            }
                        }
                    ...
                    if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) != 0) {
                        entranceY = 154 + Y_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosY"), 0);
                        if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 1) {//Anchor Left
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Left_MM_Margin;};
                            entranceX = OTRGetRectDimensionFromLeftEdge(origX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0));
                        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 2) {//Anchor Right
                            if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.UseMargins"), 0) != 0) {X_Margins_Minimap = Right_MM_Margin;};
                            entranceX = OTRGetRectDimensionFromRightEdge(origX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0));
                        } else if (CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.AnchorType"), 0) == 3) {//Anchor None
                            entranceX = origX + X_Margins_Minimap + CVarGetInteger(COSMETIC_CVAR("Hud_Minimap.PosX"), 0);
                        }
                    }
...
void Map_Update(PlayState* play) {
    ...
    Top_MM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginTop"), 0);
    Left_MM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginLeft"), 0);
    Right_MM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginRight"), 0);
    Bottom_MM_Margin = CVarGetInteger(COSMETIC_CVAR("Hud_MarginBottom"), 0);
