...
void Play_Init(GameState* thisx) {
    ...
    if (GetGlobalShipSaveContextData()->stats.sceneNum != gPlayState->sceneNum) {
        ...
        GetGlobalShipSaveContextData()->stats.sceneTimer = 0;
    ...
    GetGlobalShipSaveContextData()->stats.sceneNum = gPlayState->sceneNum;
    GetGlobalShipSaveContextData()->stats.roomNum = gPlayState->roomCtx.curRoom.num;
    // #endregion
    ...
    if (FrameAdvance_Update(&play->frameAdvCtx, &input[1])) {
        ...
        if (!GetGlobalShipSaveContextData()->stats.gameComplete) {
            if (CHECK_BTN_ALL(input[0].press.button, BTN_A)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_A]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_B)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_B]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_CUP)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_CUP]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_CRIGHT)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_CRIGHT]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_CLEFT)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_CLEFT]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_CDOWN)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_CDOWN]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_DUP)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_DUP]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_DRIGHT)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_DRIGHT]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_DDOWN)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_DDOWN]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_DLEFT)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_DLEFT]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_L)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_L]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_R)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_R]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_Z)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_Z]++;
            }
            if (CHECK_BTN_ALL(input[0].press.button, BTN_START)) {
                GetGlobalShipSaveContextData()->stats.count[COUNT_BUTTON_PRESSES_START]++;
            }

            // Start RTA timing on first non-c-up input after intro cutscene
            if (!GetGlobalShipSaveContextData()->stats.fileCreatedAt && !Player_InCsMode(play) &&
                ((input[0].press.button && input[0].press.button != 0x8) || input[0].rel.stick_x != 0 ||
                 input[0].rel.stick_y != 0)) {
                GetGlobalShipSaveContextData()->stats.fileCreatedAt = GetUnixTimestamp();
            }
        }
        // #endregion
        ...
        if (1 && (gTrnsnUnkState != 3)) {
            ...
            if (!isPaused && (IREG(72) == 0)) {
                ...
                if (!GetGlobalShipSaveContextData()->stats.gameComplete &&
                    ...
                    GetGlobalShipSaveContextData()->stats.playTimer++;
                    GetGlobalShipSaveContextData()->stats.sceneTimer++;
                    GetGlobalShipSaveContextData()->stats.roomTimer++;

                    if (CVarGetInteger(CVAR_ENHANCEMENT("MMBunnyHood"), BUNNY_HOOD_VANILLA) != BUNNY_HOOD_VANILLA &&
                        Player_GetMask(play) == PLAYER_MASK_BUNNY) {
                        GetGlobalShipSaveContextData()->stats.count[COUNT_TIME_BUNNY_HOOD]++;
                    }
                }
