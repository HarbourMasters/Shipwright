...
Vec3s Camera_Update(Camera* camera) {
    ...
    if (CVarGetInteger(CVAR_DEVELOPER_TOOLS_DEBUG_ENABLED, 0) && CHECK_BTN_ALL(D_8015BD7C->state.input[2].press.button, BTN_START)) {
        gDbgCamEnabled ^= 1;
        if (gDbgCamEnabled) {
    ...
    if (camera->timer != -1 && CHECK_BTN_ALL(D_8015BD7C->state.input[0].press.button, BTN_DRIGHT) &&
        CVarGetInteger(CVAR_DEVELOPER_TOOLS_DEBUG_ENABLED, 0)) {
        camera->timer = 0;
    }
