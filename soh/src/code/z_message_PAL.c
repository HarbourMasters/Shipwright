...
void Message_DrawTextCharCustom(MessageContext* msgCtx, void* textureImage, Gfx** p) {
    Gfx* gfx = *p;
...
void Message_DrawTextCustom(MessageContext* msgCtx, Gfx** gfxP, u16 xPos, u16 yPos) {
    ...
    msgCtx->textPosX = xPos;
    msgCtx->textPosY = yPos;
    ...
    for (i = 0; i < msgCtx->textDrawPos; i++) {
        ...
        switch (character) {
            ...
            case ' ':
                ...
                break;
            default:
                Message_DrawTextCharCustom(msgCtx, &font->charTexBuf[charTexIdx], &gfx);
...
void Message_DecodeCustom(MessageContext* msgCtx, Gfx** gfxP) {
    ...
    for (u32 i = 0; i < FONT_CHAR_TEX_SIZE * 120; i += FONT_CHAR_TEX_SIZE) {
        if (&font->charTexBuf[i] != NULL) {
            gSPInvalidateTexCache(gfx++, &font->charTexBuf[i]);
        }
    }
...
void Message_OpenTextCustom(MessageContext* msgCtx, u16 textId) {
    ...
    if (!CustomMessage_RetrieveIfExistsMsgCtx(msgCtx)) {
        return;
    }
