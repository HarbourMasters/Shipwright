...
void Message_LoadItemIcon(PlayState* play, u16 itemId, s16 y) {
    ...
    const char* itemIconPath = (const char*)gItemIcons[itemId];
    if (ResourceMgr_OTRSigCheck((char*)itemIconPath) == 0) {
        itemIconPath = (const char*)gItemIcons[ITEM_NONE];
    }
    ...
    if (itemId < ITEM_MEDALLION_FOREST) {
        ...
        memcpy((uintptr_t)msgCtx->textboxSegment + MESSAGE_STATIC_TEX_SIZE, itemIconPath,
               strlen(itemIconPath) + 1);
        // "Item 32-0"
