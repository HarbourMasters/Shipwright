...
void Font_LoadChar(Font* font, u8 character, u16 codePointIndex) {
    ...
    if (character < 0x8B)
        memcpy(&font->charTexBuf[codePointIndex], GetResourceDataByName(fntTbl[character]), FONT_CHAR_TEX_SIZE);
}
...
void Font_LoadMessageBoxIcon(Font* font, u16 icon) {
    memcpy(font->iconBuf, GetResourceDataByName(msgStaticTbl[4 + icon]), FONT_CHAR_TEX_SIZE);
}
...
void Font_LoadOrderedFont(Font* font) {
    ...
    for (fontBufIndex = 0, codePointIndex = 0; font->msgBuf[codePointIndex] != MESSAGE_END; codePointIndex++) {
        ...
        if (font->msgBuf[codePointIndex] != MESSAGE_NEWLINE) {
            ...
            memcpy(fontBuf, GetResourceDataByName(fntTbl[offset / FONT_CHAR_TEX_SIZE]), FONT_CHAR_TEX_SIZE);
