...
void Sram_InitSave(FileChooseContext* fileChooseCtx) {
    ...
    if (fileChooseCtx->buttonIndex != 0 || !CVarGetInteger(CVAR_DEVELOPER_TOOLS_DEBUG_ENABLED, 0)) {
        Sram_InitNewSave();
    } else {
        Sram_InitDebugSave();
    }
    ...
    if ((fileChooseCtx->buttonIndex == 0 && CVarGetInteger(CVAR_DEVELOPER_TOOLS_DEBUG_ENABLED, 0)) || CVarGetInteger("gNaviSkipCutscene", 0)) {
