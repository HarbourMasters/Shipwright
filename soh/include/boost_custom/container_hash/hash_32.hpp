...
template <class T, bool bigger_than_size_t = (sizeof(T) > sizeof(uint32_t)),
          bool is_unsigned = boost::is_unsigned<T>::value, std::size_t size_t_bits = sizeof(uint32_t) * CHAR_BIT,
          std::size_t type_bits = sizeof(T) * CHAR_BIT>
struct hash_integral_impl_32;

template <class T, bool is_unsigned, std::size_t size_t_bits, std::size_t type_bits>
struct hash_integral_impl_32<T, false, is_unsigned, size_t_bits, type_bits> {
...
struct hash_integral_impl_32<T, true, false, size_t_bits, type_bits> {
    static uint32_t fn(T v) {
...
template <typename T> typename boost::enable_if_<boost::is_integral<T>::value, uint32_t>::type hash_value_32(T v) {
...
#if BOOST_VERSION_HAS_HASH_RANGE
template <typename T>
typename boost::enable_if_<container_hash::is_contiguous_range<T>::value, uint32_t>::type hash_value_32(T const& v) {
    return boost::hash_range_32(v.data(), v.data() + v.size());
}
#else
template <class Ch, class A>
inline uint32_t hash_value_32(std::basic_string<Ch, std::BOOST_HASH_CHAR_TRAITS<Ch>, A> const& v) {
    return boost::hash_range_32(v.data(), v.data() + v.size());
}
#endif
...
template <class T> inline void hash_combine_32(uint32_t& seed, T const& v) {
...
template <class It> inline void hash_range_32(uint32_t& seed, It first, It last) {
    seed = hash_detail::hash_range_32(seed, first, last);
}

template <class It> inline uint32_t hash_range_32(It first, It last) {
    uint32_t seed = 0;
