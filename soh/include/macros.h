...
#define GERUDO_FORTRESS_SMALL_KEY_MAX (OTRGlobals::Instance->gRandoContext->GetOption(RSK_GERUDO_FORTRESS).Is(RO_GF_CARPENTERS_FAST) ? 1 : OTRGlobals::Instance->gRandoContext->GetOption(RSK_GERUDO_FORTRESS).Is(RO_GF_CARPENTERS_FREE) ? 0 : 4)
#define THIEVES_HIDEOUT_DOOR_FLAGS (OTRGlobals::Instance->gRandoContext->GetOption(RSK_GERUDO_FORTRESS).Is(RO_GF_CARPENTERS_FAST) ? std::vector<uint8_t>{ 1 } : OTRGlobals::Instance->gRandoContext->GetOption(RSK_GERUDO_FORTRESS).Is(RO_GF_CARPENTERS_FREE) ? std::vector<uint8_t>{} : std::vector<uint8_t>{ 1, 2, 3, 4 })
