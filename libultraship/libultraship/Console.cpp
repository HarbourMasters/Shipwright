		...
		if (args.size() > 2) {
			...
			for (size_t k = 0; k < std::size(io->KeysData); k++) {
				...
				if (toLowerCase(args[1]) == toLowerCase(key)) {
					...
					Console->Bindings[(ImGuiKey)k] = imploded.str();
					Console->SendInfoMessage("Binding '%s' to %s", args[1].c_str(), Console->Bindings[(ImGuiKey)k].c_str());
					break;
				}
			}
		}
	...
	bool Console::BindToggleCommand(std::shared_ptr<Console> Console, const std::vector<std::string>& args) {
		if (args.size() > 2) {
			...
			for (size_t k = 0; k < std::size(io->KeysData); k++) {
				...
				if (toLowerCase(args[1]) == toLowerCase(key)) {
					Console->BindingToggle[(ImGuiKey)k] = args[2];
					Console->SendInfoMessage("Binding toggle '%s' to %s", args[1].c_str(), Console->BindingToggle[(ImGuiKey)k].c_str());
					break;
				}
			}
		}
