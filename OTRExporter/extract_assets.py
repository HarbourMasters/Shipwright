...
def BuildOTR(xmlPath, rom, zapd_exe=None, portVer=None, genHeaders=None):
    ...
    if genHeaders:
        ...
        exec_cmd.extend(["-gsf", "0", "-se", "OTR", "--otrfile",
                "oot-mq.otr" if Z64Rom.isMqRom(rom) else "oot.otr"] + ["--portVer", portVer] if portVer else [])
...
def BuildSohOtr(zapd_exe=None, portVer=None):
    shutil.copytree("assets", "Extract/assets")

    if not zapd_exe:
        zapd_exe = "x64\\Release\\ZAPD.exe" if sys.platform == "win32" else "../ZAPDTR/ZAPD.out"

    exec_cmd = [zapd_exe, "botr", "-se", "OTR", "--norom"] + ["--portVer", portVer] if portVer else []
...
def main():
    ...
    parser.add_argument("--portVer", help="Store the port version in the otr", dest="portVer", type=str)
    ...
    if args.norom:
        ...
        BuildSohOtr(args.zapd_exe, portVer=args.portVer)
        return
    ...
    for rom in roms:
        ...
        BuildOTR("../soh/assets/xml/" + rom.version.xml_ver + "/", rom.file_path, zapd_exe=args.zapd_exe, portVer=args.portVer, genHeaders=args.gen_headers)
